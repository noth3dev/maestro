<p align="center">
  <a href="docs/README.md">
    <picture>
      ...
      <img alt="Maestro" src="docs/assets/logo-light.svg" width="280" style="max-width: 100%;">
    </picture>
  </a>
</p>
...
- **Worker path:** `ExecutionKernelPort` uses the authenticated Model Gateway transport; Ensemble Router production selector/native-admission wiring is implemented and is the default routing mode. Every admission carries host context, capability grant, model policy, account binding, and idempotency. The production `ToolRegistry` registers the IPython host tool; other host tools are not registered yet, and unregistered tools fail closed.
...
The repository contains the routing artifact contracts plus a production selector wired into worker admission (default routing mode). The verified boundary is:
...
[`routing-selector.ts`](packages/domain/src/routing-selector.ts) implements a pure selector with explicit A↔D weakest-link checks and B/C hard filters. [`0072_ensemble_router_artifacts.sql`](packages/persistence/migrations/0072_ensemble_router_artifacts.sql) and [`ensemble-router-artifacts.ts`](packages/persistence/src/ensemble-router-artifacts.ts) provide durable C overlay/Goal snapshot and append-only routing-evidence storage; the real PostgreSQL gate is [`ensemble-router-artifacts.integration.test.ts`](packages/persistence/src/ensemble-router-artifacts.integration.test.ts). Production selector/native-admission wiring is implemented (`apps/control-plane/src/composition/execution-services.ts`'s `composeExecutionServices`, `apps/control-plane/src/worker-service.ts`'s `assertWorkerRoutingMode`/`resolveWorkerModelForRouting`) and is the default routing mode. Fixed-model evidence migration and live host-tool acceptance remain unimplemented; the production `ToolRegistry` registers the IPython host tool only, and unregistered host-tool writes/effects fail closed. `MAESTRO_MODEL_ROUTING_MODE=pin` with `MAESTRO_NATIVE_MODEL` remains an explicit fixed-model pin/routing-off input where required. See the [canonical registry](roadmap/_meta/naming-registry.md) and [Ensemble Router design](roadmap/act-1-foundation/active/2026-09-08-ensemble-router-routing-design.md).
