...
export function Persona() {
  ...
  const propose = (axis: keyof PersonaInspection["profile"], value: number) => {
    ...
    if (prior === undefined || prior.candidate === undefined || typeof prior.candidate !== "object" || Array.isArray(prior.candidate)) { setError("A durable candidate proposal is required before editing this axis."); return; }
    const source = prior.candidate as Record<string, unknown>;
    ...
    const candidate = { ...source, changes };
    void window.maestro.api.editPersonaCandidate(prior.candidateId, { projectId: config.projectId, goalId: selectedGoalId, candidate }, globalThis.crypto.randomUUID()).then(() => window.maestro.api.getPersona({ projectId: config.projectId, goalId: selectedGoalId, roleId: model.roleId, taskClass: model.taskClass })).then(setModel).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : "Persona proposal rejected by the control plane"));
  };
