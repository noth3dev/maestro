...
export function Setup() {
  ...
  const safeError = error === undefined ? undefined : safeBootstrapErrorMessage(error);
  const displayError =
    safeError === LONG_BOOTSTRAP_ERROR_SUMMARY
      ? t.setup.longDiagnosticError
      : safeError === LOCAL_DATABASE_LOCK_SUMMARY
        ? t.setup.databaseLockedError
        : safeError;
  ...
  const showRecoveryHint =
    ...
    (formError === undefined || formError === setupError) &&
    error !== LONG_BOOTSTRAP_ERROR_SUMMARY &&
    error !== LOCAL_DATABASE_LOCK_SUMMARY;
  const retryProgressText = retryingBootstrap
    ? bootstrapProgressText(bootstrap, t.setup.retryingLocal, t.setup.progressSteps, t.setup.completedProgressSteps, t.setup.retryFailed)
    : t.setup.retryFailed;
  ...
  const retryLocalSetup = async () => {
    ...
    try {
      ...
      setFormError(safeBootstrapErrorMessage(cause, "Could not retry local setup"));
    }
  };
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setFormError(safeBootstrapErrorMessage(cause, "Could not save the connection"));
    } finally {
      setConnecting(false);
    }
  };
  ...
  return (
    <main className="home-main setup-main" aria-label={t.setup.title}>
      <h1 id="setup-title" className="home-title setup-title" tabIndex={-1}>
        {t.setup.title}
      </h1>
      <div className="home-composer setup-card">
        ...
        <details className="setup-manual">
          <summary className="setup-manual-title">{t.setup.manualTitle}</summary>
          <p className="form-hint setup-manual-hint">{t.setup.manualHint}</p>
          <form className="setup-manual-form" onSubmit={(event) => void submit(event)}>
            ...
            <button
              className={canRetryLocal ? "btn setup-submit" : "btn btn-primary setup-submit"}
              type="submit"
              disabled={connecting || retryingBootstrap}
            >
