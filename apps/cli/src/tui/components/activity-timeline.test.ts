...
describe("renderActivityTimeline", () => {
  ...
  it("derives actionable pending decisions from durable activity", () => {
    const pending = toActivityTimelineEvent({
      ...
      payload: { effect: { actor: "worker-3", action: "git.remote.push", target: "origin main", classification: "critical", outcome: "awaiting user", kind: "warning" } },
    });
  ...
  it("keeps concurrent identical effects distinct by durable identity", () => {
    ...
    const pendingOne = toActivityTimelineEvent({ ...baseEvent, cursor: "1", eventId: "event-1", payload: { effect: { effectId: "effect-1", actor: "worker", action: "git.remote.push", target: "main", classification: "critical", outcome: "awaiting user", kind: "warning" } } });
    const pendingTwo = toActivityTimelineEvent({ ...baseEvent, cursor: "2", eventId: "event-2", payload: { effect: { effectId: "effect-2", actor: "worker", action: "git.remote.push", target: "main", classification: "critical", outcome: "awaiting user", kind: "warning" } } });
    const completedOne = toActivityTimelineEvent({ ...baseEvent, cursor: "3", eventId: "event-3", eventType: "effect.completed", payload: { effect: { effectId: "effect-1", actor: "worker", action: "git.remote.push", target: "main", classification: "critical", outcome: "approved", kind: "success" } } });
