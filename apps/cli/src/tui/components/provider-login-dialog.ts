...
export function renderProviderLoginDialog(width: number, selected: AccountLoginProviderSelection, state: "selecting" | "opening" | "waiting", authUrl?: string): string[] {
  ...
  const lines = [
    ...
    "",
    state === "selecting" ? tuiTheme.dim("↑/↓ choose · Enter continue · Esc cancel") : state === "opening" ? tuiTheme.dim("Opening the provider sign-in page…") : tuiTheme.dim("Finish sign-in in your browser · Alt+C copy link · Esc cancels"),
