...
export interface EvidenceReadModel { id: string; kind: string; createdAt?: string }
export function renderEvidencePanel(state: PanelState<readonly EvidenceReadModel[]>, width: number): string[] {
  ...
  return ["Evidence", ...state.value.map((evidence) => panelLine(`• ${evidence.id} · ${evidence.kind}${evidence.createdAt === undefined ? "" : ` · ${evidence.createdAt}`}`, width))];
}
