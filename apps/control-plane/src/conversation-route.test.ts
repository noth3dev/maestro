...
describe("conversation routes", () => {
  it("creates a conversation and runs a turn through the authenticated service", async () => {
    ...
    const turn = vi.fn(async () => ({ conversation: { ...conversation, status: "succeeded" as const, version: 2 }, turn: { turnId: "018f3c9b-7e71-7b44-ae23-3b5d4e8c9f04", conversationId, role: "assistant" as const, content: "Hello from Carnegie", status: "completed" as const, cursor: "2", createdAt: "2025-01-01T00:00:00.000Z" } }));
    ...
    expect(response.json().turn.content).toBe("Hello from Carnegie");
