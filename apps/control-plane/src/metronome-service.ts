...
export interface WorkerOverlayChallengeInput {
  ...
  readonly taskClassTemplate?: Readonly<Partial<Record<PersonaAxis, number>>>;
...
interface DurableWorkerOverlayRow { status: string; department_id: string; bundle_content_hash: string; content_hash: string; persona: unknown; worker_profile_derivation: unknown; expires_at: Date | string; }
  ...
  return {
    ...
    async challengeWorkerOverlay(goalId, input, commandId) {
      ...
      const worker = await deps.pool.query<DurableWorkerOverlayRow>(
        `SELECT w.status, w.department_id, w.bundle_content_hash, b.content_hash, o.persona, o.expires_at, w.worker_profile_derivation
           FROM workers w
      ...
      let assignmentProfile = profile;
      let taskClassTemplate = profile;
      if (durableWorker.worker_profile_derivation !== null && durableWorker.worker_profile_derivation !== undefined) {
        try {
          const assignment = durableWorker.worker_profile_derivation as { profile?: unknown; taskClassTemplate?: unknown };
          assignmentProfile = parsePersonaProfile(assignment.profile);
          taskClassTemplate = parsePersonaProfile(assignment.taskClassTemplate);
        } catch { throw new WorkerOverlayChallengeError("stored worker derivation is invalid"); }
      }
      ...
      const violations = [...findUnsafeWorkerOverlay({ ...input, profile: assignmentProfile, taskClassTemplate, roleFloors, roleCeilings })];
      if (PERSONA_AXES.some((axis) => profile[axis] !== assignmentProfile[axis])) violations.push("persisted Mission Persona Overlay differs from the worker assignment profile");
