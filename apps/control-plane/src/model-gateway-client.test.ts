...
const model: ModelCatalogEntry = {
  identity: { provider: "openai", id: "gpt-test" },
...
describe("Control Plane model gateway client", () => {
  ...
  it("sanitizes gateway stream errors and bounds incomplete records", async () => {
    ...
    const oversized = createModelGatewayClient({
      baseUrl: "http://127.0.0.1:4321",
      token: "gateway-secret",
      fetch: async () => new Response("x".repeat(128_001), { status: 200, headers: { "content-type": "text/event-stream" } }),
    });
    await expect(oversized.turn(input)).rejects.toMatchObject({
      code: "gateway_request_failed",
      message: "model gateway stream record is too large",
    });
  });
  ...
  it("preserves a local spawn detail from a streamed provider failure", async () => {
    ...
    await expect(client.turn(input)).rejects.toMatchObject({
      code: "provider_unavailable",
      detail: "local codex executable could not be spawned",
    });
  });
  ...
  it("rejects a stale streamed result with a different request identity", async () => {
    ...
    await expect(client.turn(input)).rejects.toMatchObject({
      code: "gateway_request_failed",
      message: "model gateway returned a mismatched request",
    });
  });
  ...
  it("forwards provider stream events and returns the terminal result", async () => {
    ...
    const fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      return new Response(
        [
          `event: thinking-delta\ndata: ${JSON.stringify({ kind: "thinking-delta", cursor: 1 })}\n\n`,
    ...
    const events: string[] = [];
    const client = createModelGatewayClient({ baseUrl: "http://127.0.0.1:4321", token: "gateway-secret", fetch });
    const actual = await client.turn({
      binding: {
        bindingId: "binding-1",
        gatewayInstanceId: "gateway-1",
        provider: model.identity,
  ...
  it("fails closed for malformed typed stream events", async () => {
    ...
    const malformedEvents = [
      { kind: "thinking-delta", cursor: 1, text: "hidden" },
      { kind: "tool-proposed", cursor: 1, call: { id: "call-1", name: "readGoal", arguments: { state: "valid" } } },
      { kind: "tool-completed", cursor: 1, callId: "call-1", toolName: "readGoal", status: "cancelled" },
      { kind: "provider-error", cursor: 1, code: "provider-failed", retryable: true, detail: "secret" },
      { kind: "terminal", cursor: 1, status: "done" },
    ];
    for (const event of malformedEvents) {
      ...
      await expect(client.turn(input)).rejects.toMatchObject({
        code: "gateway_request_failed",
        message: "model gateway returned malformed stream event",
      });
    }
  });
});
...
it("preserves the gateway lost-login code for restart recovery", async () => {
  const fetch = async () =>
    response({ error: { code: "account_login_session_unknown", message: "account login session is unknown" } }, 409);
  const client = createModelGatewayClient({ baseUrl: "http://127.0.0.1:4321", token: "gateway-secret", fetch });
  await expect(
    client.accountLoginStatus?.({ requestId: "request-unknown", operatorId: "operator-1", providerId: "openai-codex", loginId: "login-1" }),
  ).rejects.toMatchObject({ code: "account_login_session_unknown", message: "account login session is unknown" });
});
...
it("starts and polls managed account login through the narrow gateway RPC", async () => {
  ...
  const fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (String(input).endsWith("/account-logins/start"))
      return response({ providerId: "openai-codex", loginId: "login-1", authUrl: "https://chatgpt.com/login" });
    return response({ providerId: "openai-codex", loginId: "login-1", state: "succeeded" });
  };
  const client = createModelGatewayClient({ baseUrl: "http://127.0.0.1:4321", token: "gateway-secret", fetch });
  await expect(
    client.startAccountLogin?.({ requestId: "request-1", operatorId: "operator-1", providerId: "openai-codex" }),
  ).resolves.toEqual({ providerId: "openai-codex", loginId: "login-1", authUrl: "https://chatgpt.com/login" });
  await expect(
    client.accountLoginStatus?.({ requestId: "request-2", operatorId: "operator-1", providerId: "openai-codex", loginId: "login-1" }),
  ).resolves.toEqual({ providerId: "openai-codex", loginId: "login-1", state: "succeeded" });
...
it("rejects reasoning stream events that carry hidden text", async () => {
  ...
  const fetch = async () =>
    new Response(
      [
        `event: thinking-delta\ndata: ${JSON.stringify({ kind: "thinking-delta", cursor: 1, text: "must-not-cross" })}\n\n`,
        `event: result\ndata: ${JSON.stringify(result)}\n\n`,
      ].join(""),
      { status: 200, headers: { "content-type": "text/event-stream" } },
    );
