...
export const GATEWAY_OPERATOR_ID = "gateway-operator";
export const PROVIDER_ACCOUNT_REF = "acceptance-account-1";
    ...
    async turn(request) {
      ...
      const repairRequested =
        ...
        request.messages.some(
          (message, index) => index > lastToolIndex && JSON.stringify(message.content).toLowerCase().includes("repair"),
        );
      if (hasIpPython && (!hasToolResult || repairRequested)) {
        ...
        if (worker !== undefined && !preparedWorkers.has(worker.worker_id)) {
          const bundleResult = await pool.query<{ substance: { environment?: unknown } }>(
            "SELECT substance FROM mission_bundles WHERE council_id = $1 AND item_id = 'exec-1' ORDER BY created_at DESC LIMIT 1",
            [worker.council_id],
          );
          const environmentId = Array.isArray(bundleResult.rows[0]?.substance.environment)
            ? bundleResult.rows[0]?.substance.environment[0]
            : undefined;
          ...
          await pool.query(
            ...
            [
              ...
              JSON.stringify({ network: ["none"], filesystem: ["/tmp"], processes: ["python3", "node", "npm"], browsers: [], devices: [] }),
              JSON.stringify({ cpuMillis: 1000, memoryMb: 128, diskMb: 256, processCount: 2, durationSeconds: 60 }),
              JSON.stringify({ status: "healthy", checkedAt: null, summary: null }),
              "e".repeat(64),
              JSON.stringify({ status: "not_scheduled", scheduledAt: null, completedAt: null, ownedResources: [], retainedEvidence: [] }),
            ],
          );
          preparedWorkers.add(worker.worker_id);
        }
        const worktreeResult = repairRequested
          ? await pool.query<{ worktree_path: string }>("SELECT worktree_path FROM worker_worktrees WHERE worker_id = $1", [
              worker?.worker_id,
            ])
          : undefined;
        ...
        return {
          ...
          toolCalls: [
            {
              id: repairRequested ? "acceptance-ipython-repair" : "acceptance-ipython-test",
              name: "ipython",
              arguments: { state: "valid", value: { code } },
            },
          ],
          stopReason: "tool_use",
          usage: { state: "unknown" },
      ...
      const toolText = request.messages
        ...
        .join("\n");
      const localEffectObserved =
        toolText.includes("worker local effect complete") || toolText.includes("run_test") || toolText.includes('state":"unknown');
      const complete = localEffectObserved || request.messages.some((message) => message.role === "tool");
      return {
        requestId: request.requestId,
        model: identity,
        text: complete ? "worker ipython local effect complete" : "acceptance run incomplete",
        toolCalls: [],
        stopReason: "end_turn",
        usage: { state: "unknown" },
      };
...
function acceptanceRoutingEvidence(input: { goalId: string; projectId: string; contractId: string; bindingId: string; index: number }) {
  const taskDemand = {
    ...
    requirements: Object.fromEntries(ROUTING_AXES.map((axis) => [axis, { level: 0, rationale: "release acceptance" }])) as Record<
      (typeof ROUTING_AXES)[number],
      { level: number; rationale: string }
    >,
  ...
  const routing = {
    ...
    routeRef: `acceptance-route:${input.goalId}:${input.index}`,
    mode: "pin" as const,
    selectedModelRef: "test/model-a",
    accountBinding: PROVIDER_ACCOUNT_REF,
    candidateRefs: ["test-model-a"],
    selectedCandidateRef: "test-model-a",
    rejections: [],
    taskDemandHash: taskDemandContentHash(taskDemand),
    pressure: 1,
    pressureBand: "low" as const,
    decisionLayer: "automatic progress" as const,
    overlayVersion: 1,
    admissionBindingRef: input.bindingId,
    rationale: "real PostgreSQL acceptance route",
    ...
    modelProfile: {
      ...
      capability: {
        ...
        axes: Object.fromEntries(
          ROUTING_AXES.map((axis) => [axis, { status: "scored" as const, score: 180, rationale: "acceptance", evidence: ["acceptance"] }]),
        ),
      },
      providerFacts: {
        schemaVersion: 1 as const,
    ...
    operationalOverlaySnapshot: {
      schemaVersion: 1 as const,
      installationRef: "release-acceptance",
      projectRef: input.projectId,
      goalRef: input.goalId,
      overlayVersion: 1,
      observations: [
...
export async function grantGitAction(
  pool: Pool,
  input: { projectId: string; goalId: string; actorId: string; action: string; target: readonly string[] },
): Promise<void> {
...
describeDatabase("real Control Plane + PostgreSQL + Model Gateway Worker acceptance", () => {
  ...
  it("admits a real native Head and Worker through authenticated HTTP, a real Model Gateway process, and real PostgreSQL", async () => {
    ...
    writeFileSync(
      `${repositoryPath}/src/discount.test.js`,
    ...
    execFileSync("git", ["-c", "user.name=acceptance", "-c", "user.email=acceptance@example.com", "commit", "-m", "initial"], {
      cwd: repositoryPath,
    });
    ...
    const substance = {
      ...
      expectedDepartments: ["Product Department", "Quality Department", "Safety Compliance Department"],
    ...
    try {
      ...
      await grantGitAction(pool, {
        projectId,
        goalId,
        actorId: operatorId,
        action: "git.local.branch.create",
        target: [repositoryPath, "goal/integration", immutableBaseRevision],
      });
      const goalBranch = await send(`/v1/goals/${goalId}/git/integration-branch`, "POST", {
        projectId,
        repositoryPath,
        branchName: "goal/integration",
        baseRevision: immutableBaseRevision,
      });
      ...
      const qualityActivation = {
        projectId,
        departmentId: "quality",
        contractId,
        requestedContribution: "independently validate",
        urgency: "normal",
        contextScope: ["confirmed contract"],
        budgetEffect: "within envelope",
      ...
      const safetyActivation = {
        projectId,
        departmentId: "safety-compliance",
        contractId,
        requestedContribution: "validate safety boundary",
        urgency: "normal",
        contextScope: ["confirmed contract"],
        budgetEffect: "within envelope",
      ...
      expect(
        (
          await send(`/v1/councils/${council.councilId}/briefs/quality`, "POST", {
            projectId,
            brief: { ...brief, contribution: "independently validate" },
          })
        ).status,
      ).toBe(204);
      expect(
        (
          await send(`/v1/councils/${council.councilId}/briefs/safety-compliance`, "POST", {
            projectId,
            brief: { ...brief, contribution: "validate safety boundary" },
          })
        ).status,
      ).toBe(204);
      ...
      const observedBody = (await observed.json()) as {
        ...
        observability: { toolEvents: { state: string; events?: ReadonlyArray<{ toolName?: string }> } };
      };
      ...
      const observability = (
        observedBody as {
          observability: { capabilityJournal: Array<{ details: Record<string, unknown> }>; ipythonSessionJournal: unknown[] };
        }
      ).observability;
      ...
      const initialWorktree = await pool.query<{ worktree_path: string }>(
        "SELECT worktree_path FROM worker_worktrees WHERE worker_id = $1",
        [worker.workerId],
      );
      ...
      const workerBeforeRepair = await pool.query<{ execution_ref: string; invocation_ref: string }>(
        "SELECT execution_ref, invocation_ref FROM workers WHERE worker_id = $1",
        [worker.workerId],
      );
      const repairResponse = await send(`/v1/workers/${worker.workerId}/messages`, "POST", {
        projectId,
        message: "tests failed; repair the defect before re-testing",
      });
      ...
      const workerAfterRepair = await pool.query<{ execution_ref: string; invocation_ref: string }>(
        "SELECT execution_ref, invocation_ref FROM workers WHERE worker_id = $1",
        [worker.workerId],
      );
      ...
      const nativeBindings = await pool.query<{ binding_id: string }>(
        "SELECT binding_id FROM native_execution_bindings WHERE goal_id = $1 ORDER BY created_at, binding_id",
        [goalId],
      );
      ...
      for (const [index, binding] of nativeBindings.rows.entries()) {
        ...
        await pool.query(
          ...
          [
            ...
            routing.routeRef,
            routing.mode,
            routing.selectedModelRef,
            routing.accountBinding,
            JSON.stringify(routing.candidateRefs),
            JSON.stringify(routing.rejections),
            routing.taskDemandHash,
            routing.pressure,
            routing.pressureBand,
            routing.decisionLayer,
            routing.overlayVersion,
            routing.admissionBindingRef,
            routing.rationale,
            JSON.stringify(routing),
          ],
        );
      }
      const workerWorktree = await pool.query<{
        repository_path: string;
        worktree_path: string;
        branch_name: string;
        base_branch_name: string;
      }>("SELECT repository_path, worktree_path, branch_name, base_branch_name FROM worker_worktrees WHERE worker_id = $1", [
        worker.workerId,
      ]);
      expect(workerWorktree.rows).toHaveLength(1);
      expect(workerWorktree.rows[0]!.repository_path).toBe(repositoryPath);
      expect(
        execFileSync("git", ["-C", workerWorktree.rows[0]!.worktree_path, "rev-parse", "--is-inside-work-tree"]).toString().trim(),
      ).toBe("true");
      expect(execFileSync("git", ["-C", workerWorktree.rows[0]!.worktree_path, "rev-parse", "HEAD"]).toString().trim()).not.toBe(
      ...
      for (const ref of [workerWorktree.rows[0]!.branch_name, workerWorktree.rows[0]!.base_branch_name, "goal/integration"]) {
        await grantGitAction(pool, {
          ...
          goalId,
          actorId: operatorId,
          action: "git.local.revision.read",
          target: [repositoryPath, ref],
        });
      }
      await grantGitAction(pool, {
        projectId,
        goalId,
        actorId: operatorId,
        action: "git.local.branch.advance",
        target: [repositoryPath, "goal/integration", goalHead, workerHead],
      });
      const integrated = await send(`/v1/workers/${worker.workerId}/git/advance`, "POST", {
        projectId,
        message: "Integrate the certified disposable-target repair",
      ...
      const accepted = await send(`/v1/workers/${worker.workerId}/accept`, "POST", {
        projectId,
        reason: "Native repair and test evidence observed",
      });
      ...
      const certification = await send(`/v1/workers/${worker.workerId}/certifications/quality`, "POST", {
        projectId,
        certifyingDepartmentId: "quality",
        substance: {
          verdict: "passed",
          findings: [{ findingId: "release-scenario", severity: "noncritical", description: "Target test passes" }],
      ...
      const safetyCertification = await send(`/v1/workers/${worker.workerId}/certifications/safety_compliance`, "POST", {
        projectId,
        certifyingDepartmentId: "safety-compliance",
        substance: { verdict: "passed", findings: [], testEvidenceIds: [evidenceId] },
      });
      expect(safetyCertification.status).toBe(201);

      const retainMode = await send(`/v1/goals/${goalId}/capabilities/full-access-mode`, "POST", {
        projectId,
        capabilityKind: "ipython",
        sessionId: randomUUID(),
        fullAccessMode: "retain_intermediate_approvals",
      });
      const skipMode = await send(`/v1/goals/${goalId}/capabilities/full-access-mode`, "POST", {
        projectId,
        capabilityKind: "ipython",
        sessionId: randomUUID(),
      ...
      expect((await pool.query("SELECT count(*)::int AS count FROM capability_sessions WHERE goal_id = $1", [goalId])).rows[0]!.count).toBe(
        2,
      );

      const workerBeforeRestart = await pool.query<{ execution_ref: string; invocation_ref: string; status: string }>(
        "SELECT execution_ref, invocation_ref, status FROM workers WHERE worker_id = $1",
        [worker.workerId],
      );
      ...
      await pool.query(
        "UPDATE reconciler_leader_lease SET expires_at = clock_timestamp() - interval '1 millisecond' WHERE lease_key = 'singleton'",
      );
      ...
      const workerAfterRestart = await pool.query<{ execution_ref: string; invocation_ref: string; status: string }>(
        "SELECT execution_ref, invocation_ref, status FROM workers WHERE worker_id = $1",
        [worker.workerId],
      );
      ...
      const reportResponse = await fetch(`${restartedBaseUrl}/v1/goals/${goalId}/concertmaster-report`, {
        method: "POST",
        headers: { ...auth, "idempotency-key": reportCommandId },
      ...
      const reportRetry = await fetch(`${restartedBaseUrl}/v1/goals/${goalId}/concertmaster-report`, {
        method: "POST",
        headers: { ...auth, "idempotency-key": reportCommandId },
        body: JSON.stringify({ projectId }),
      });
      ...
      const reportRetryWithNewCommand = await fetch(`${restartedBaseUrl}/v1/goals/${goalId}/concertmaster-report`, {
        method: "POST",
        headers: { ...auth, "idempotency-key": randomUUID() },
        body: JSON.stringify({ projectId }),
      });
      ...
      if (reportReadResponse.status !== 200)
        throw new Error(`report read ${reportReadResponse.status}: ${await reportReadResponse.text()}`);
      if (certificationsResponse.status !== 200)
        throw new Error(`certifications read ${certificationsResponse.status}: ${await certificationsResponse.text()}`);
      expect(((await reportReadResponse.json()) as { evidenceBundleId: string }).evidenceBundleId).toBe(
        ((await bundleResponse.json()) as { bundleId: string }).bundleId,
      );
      ...
      const worktreeRows = await pool.query<{ repository_path: string; worktree_path: string }>(
        "SELECT repository_path, worktree_path FROM worker_worktrees",
      );
      for (const row of worktreeRows.rows)
        execFileSync("git", ["-C", row.repository_path, "worktree", "remove", "--force", "--", row.worktree_path]);
