...
function fakeProviderPlugin(): ProviderPlugin {
  ...
  const port: ModelProviderPort = {
    ...
    async turn(request) {
      return { requestId: request.requestId, model: identity, text: "acceptance run complete", toolCalls: [], stopReason: "end_turn", usage: { state: "unknown" } };
    },
...
describeDatabase("real Control Plane + PostgreSQL + Model Gateway Worker acceptance", () => {
  ...
  it("admits a real native Head and Worker through authenticated HTTP, a real Model Gateway process, and real PostgreSQL", async () => {
    ...
    const substance = {
      ...
      project: { projectId, repository: "/tmp/maestro-native-acceptance", immutableBaseRevision: "e".repeat(40), dataBoundary: "repository only" },
    ...
    try {
      ...
      const plan = {
        ...
        gitRepository: "/tmp/maestro-native-acceptance", gitBranch: "goal/product", integrationPath: "src", risks: [], safePausePoints: [],
