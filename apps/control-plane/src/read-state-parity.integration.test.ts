...
describeDatabase("App/API and CLI durable read-state parity (plan/phase3.md Tests item 18)", () => {
  ...
  it("shows the same Sentinel challenge, Overwatch Council round, certification, and Sane report state through the HTTP API and the CLI for the same real Goal", async () => {
    ...
    // challenge, an Overwatch Council round, a Quality certification, and a
    ...
    const round = await runOverwatchCouncilReview(pool, fakeReviewKernel(), {
    ...
    try {
      ...
      expect(await executeCli(["overwatch-council", "list", "--goal-id", goalId, "--json"], env, io)).toBe(0);
