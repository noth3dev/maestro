...
describe("plan review gate", () => {
  it("passes when the Plan Reviewer reviewed the latest plan with no blockers", () => {
    const events = [write("plan00.md", "conversation-lead"), write("task.md", "task-editor"), write("reviews/plan.md", "plan-reviewer")];
  ...
  it("requires a review written by the Plan Reviewer", () => {
    expect(evaluateReviewGate({ events: [write("task.md", "task-editor")], review: undefined }).state).toBe("missing");
  ...
  it("goes stale when the plan changes after the review", () => {
    const events = [write("reviews/plan.md", "plan-reviewer"), write("plan01.md", "task-editor")];
    expect(evaluateReviewGate({ events, review: clean })).toMatchObject({ state: "stale" });
  });

  it("refuses self-review and reports blockers", () => {
    expect(evaluateReviewGate({ events: [write("task.md", "plan-reviewer"), write("reviews/plan.md", "plan-reviewer")], review: clean }).state).toBe("self_review");
