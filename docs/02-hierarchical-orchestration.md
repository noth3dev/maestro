...
* Department Heads dispatch workers through the provider-neutral `ExecutionKernelPort`. The worker composition uses the native execution-kernel path; Ensemble Router selection is not implemented, and every worker admission is checked against its immutable Mission Bundle and gateway binding.
