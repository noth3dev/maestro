...
| Native `ToolRegistry` | Validates registered tool names, argument schemas, output schemas, grants, and data class | Production composition registers the IPython host tool (`apps/control-plane/src/main.ts`'s `tools.register(createIpPythonTool(...))`); no other host tool is registered. Unregistered tools are rejected. |
...
| Git adapter | Local branch/worktree/commit operations through `AuthorizedEffectExecutor` | No remote/push/merge/rewrite/release/deploy operation exists in `@maestro/git-adapter` **by construction** — this is a deliberate security boundary, not an unfinished feature. Remote push and other critical/deploy effects must be supplied by whoever deploys Maestro as an explicit, external concrete effect adapter; Maestro's own codebase never ships one. |
| Critical-action route | Records exact approval and invokes an injected effect seam | Production has no default no-op effect: an absent adapter throws instead of claiming success. This is intentional fail-closed behavior — see the Git adapter row above for why Maestro does not bundle a remote-push adapter itself. |
