...
1. **Model selection: prefer `openai-codex/gpt-5.6-luna` by default.** Resolve it with
   ...
   selector. Two other selectors exist on a separate quota pool (`openrouter/openai/gpt-5.6-luna`,
   `openrouter/openai/gpt-5.6-luna-pro`) — try the OpenRouter selector as a same-model fallback
   before dropping to the inherited default model, since it is the same model on different usage
   limits. If the user explicitly names a different model, use that instead.
