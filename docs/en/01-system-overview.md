...
5. **Native Runtime Ownership**: Maestro owns the provider-neutral agent runtime, model gateway boundary, conversation lifecycle, and authority checks. The worker path uses the native execution-kernel over the authenticated Model Gateway; Ensemble Router selection is implemented and is the default routing mode.
...
| **Worker execution** | `ExecutionKernelPort`; native Model Gateway transport (Ensemble Router selection is implemented and is the default routing mode) | Worker admission, authority checks, leases, audit pre-logging |
...
The native runtime owns conversation and worker execution. Provider credentials remain gateway-owned and never enter Control Plane state, prompts, evidence, or logs. The production `ToolRegistry` registers the IPython host tool (`apps/control-plane/src/main.ts`); other host tools are not registered yet, and unregistered tools fail closed.

---

### Ensemble Router boundary

The routing artifacts plus a production selector wired into worker admission are both present, and ensemble routing is the default mode. A/D/E domain contracts, B provider-facts domain and wire schema, C operational-overlay domain and wire schema with a pure Goal snapshot helper, four pressure-band domain and wire schemas, and the human-owned `config/model_map.json` baseline are present. [`routing-selector.ts`](../../packages/domain/src/routing-selector.ts) provides a pure selector with A↔D weakest-link checks and B/C hard filters. Persistence migration [`0072_ensemble_router_artifacts.sql`](../../packages/persistence/migrations/0072_ensemble_router_artifacts.sql) and [`ensemble-router-artifacts.ts`](../../packages/persistence/src/ensemble-router-artifacts.ts) provide durable C overlay/Goal snapshot and append-only routing-evidence storage. Production selector/native-admission wiring is implemented (`apps/control-plane/src/composition/execution-services.ts`'s `composeExecutionServices`, `apps/control-plane/src/worker-service.ts`'s `assertWorkerRoutingMode`/`resolveWorkerModelForRouting`); fixed-model pin migration and live acceptance are not present. `MAESTRO_MODEL_ROUTING_MODE=pin` with `MAESTRO_NATIVE_MODEL` remains an explicit fixed-model pin/routing-off input where required.
