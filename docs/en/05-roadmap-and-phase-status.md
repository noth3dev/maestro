...
| --- | --- | --- |
| **Act 1 — Foundation** | **In progress; not certified** | A/D/E contracts, B provider facts, C operational overlay with pure Goal snapshot, four pressure-band schemas, and the human-owned empty `model_map` baseline are present. Pure A↔D weakest-link selection with B/C hard filters is implemented, and migration `0072_ensemble_router_artifacts.sql` provides durable C/Goal snapshots and append-only routing evidence with a real PostgreSQL gate. Production selector/native-admission wiring, fixed-model evidence migration, host-tool writes/effects, live acceptance, and TUI SSE reconnect evidence remain open; Phase 1 is not closed. See [`roadmap/act-1-foundation/README.md`](../../roadmap/act-1-foundation/README.md). |
