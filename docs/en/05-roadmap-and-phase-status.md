...
| **Phase 2** | Concertmaster Office Core & Hierarchical Execution | **Plan 2 S6 verified; documentation sync complete; live acceptance pending** | Plan 2 S6 provides the persistent IPython local host-tool, four-level approval hierarchy, full-access modes, and recovery evidence. Live acceptance remains a user-run gate; historical code-level completion markers are not current product acceptance. |
...
| **Phase 5** | Concurrent Goals & Portfolio Control | **Implementation complete (S1–S5); user-owned live handoff pending** | [`plan-5.md`](../../execution/plan-5.md)'s first operable slice is merged: cross-Goal isolation proof, per-resource capacity inventory with durable queueing, safe pause/resume with execution fencing, Portfolio Council decisions (CEO pin and Discord safety preemption honored), and the three-Goal/two-project scenario harness with [`RUNBOOK.md`](../../test/phase5-scenario/RUNBOOK.md). The §5 HANDOFF live acceptance run remains open in `execution/PENDING_LIVE_CHECKS.md`; no live acceptance is claimed. Protected floors, demand reservations, recovery reserve, and broader portfolio scheduling policy are deliberately deferred follow-on slices. |
...
| **Phase 8** | Full-System Hardening & Release Certification | **S9 blocked; not accepted** | Eleven-scenario catalog/report contract is present, but live full-system evidence on one frozen candidate is unavailable. S10 refuses release; S11 artifact is prepared but clean global install is blocked by private workspace dependencies. |
...
The A/D/E domain contracts, B provider facts, C operational overlay and pure Goal snapshot, four pressure bands, and human-owned `model_map` baseline are implemented. [`routing-selector.ts`](../../packages/domain/src/routing-selector.ts) implements pure A↔D weakest-link selection with B/C hard filters. Migration [`0072_ensemble_router_artifacts.sql`](../../packages/persistence/migrations/0072_ensemble_router_artifacts.sql) and [`ensemble-router-artifacts.ts`](../../packages/persistence/src/ensemble-router-artifacts.ts) persist C overlays, immutable Goal snapshots, and append-only routing evidence; the real PostgreSQL gate is [`ensemble-router-artifacts.integration.test.ts`](../../packages/persistence/src/ensemble-router-artifacts.integration.test.ts). Production selector/native-admission wiring, fixed-model evidence migration, production host-tool writes/effects, and live host-tool acceptance remain open; native admission still requires one exact `modelPolicy` identity.
...
### Phase 8 §S9–§S12 current evidence

Phase 8 remains **not accepted**. The eleven-scenario catalog and bounded runner are at [`test/phase8-scenarios/`](../../test/phase8-scenarios/) and [`docs/operations/12-representative-scenarios.md`](../operations/12-representative-scenarios.md); they currently record `blocked` inventory/component evidence and do not claim live full-system proof. §S10 consequently returns `do_not_release` when given this report; its rollback automation is input-gated. §S11 has a prepared artifact, but clean global installation is blocked by private unpublished workspace dependencies; it is not published: [`docs/operations/13-cli-release.md`](../operations/13-cli-release.md) documents the manual handoff, and [`scripts/install.sh`](../../scripts/install.sh) assumes Node.js 24+. No npm publish, tag push, remote deployment, external send, or critical release effect was run.
