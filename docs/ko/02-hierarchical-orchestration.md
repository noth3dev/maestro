...
* 부서장은 provider-neutral `ExecutionKernelPort`를 통해 작업자를 디스패치합니다. 워커 구성은 네이티브 execution-kernel 경로를 사용하며 Ensemble Router selection은 구현되어 있고 기본 routing mode이며, 모든 worker admission은 immutable Mission Bundle과 gateway binding으로 검사됩니다.
