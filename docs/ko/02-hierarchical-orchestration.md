...
Carnegie는 전문화된 페르소나, 영구 부서, 접수 크루 및 독립 감시 의회에 책임을 분담하여 인간 조직 구조를 모델링합니다.
...
* 부서장은 provider-neutral `ExecutionKernelPort`를 통해 작업자를 디스패치합니다. 워커 구성은 네이티브 execution-kernel 경로를 사용하며 Ensemble Router selection은 아직 구현되지 않았고, 모든 worker admission은 immutable Mission Bundle과 gateway binding으로 검사됩니다.
