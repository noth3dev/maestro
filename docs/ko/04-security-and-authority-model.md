...
## 1. 액션 분류 체계 (Action Classification Matrix)
...
```mermaid
flowchart TD
    REQ[에이전트 Action Request] --> GATE[AuthorizedEffectExecutor]
    
    GATE --> CLASS[액션 분류 검사]
    CLASS -->|Forbidden / Ambiguous| DENY[실행 즉시 거부 & 보안 이벤트 기록]
    CLASS -->|Ordinary| CHECK_GRANT{현재 Goal/Mission 범위 내 유효한 Grant 존재?}
    CLASS -->|Critical| CHECK_APPROVAL{유효한 Conductor 사전 승인 존재?}
    
    CHECK_GRANT -->|No| DENY
    CHECK_GRANT -->|Yes| AUDIT[사전 감사 로그 DB 영속화]
    
    CHECK_APPROVAL -->|No| PAUSE[실행 일시 중단 및 Conductor 승인 요청]
    CHECK_APPROVAL -->|Yes| AUDIT
    
    AUDIT --> EXEC[실제 도구 / 부작용 실행]
...
```mermaid
flowchart LR
    INPUT[📄 Task Contract / Brief] --> CANON[정규화 Canonical JSON]
