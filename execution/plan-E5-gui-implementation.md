...
**Task 16 first orchestration-command handoff checkpoint (2026-09-24):** The atomic GoalCreated outbox payload now includes a scoped `start_goal` command identity (`commandId` equals the GoalCreated event ID, plus project/Goal/Task Contract IDs). RED failed on the missing metadata; GREEN passed the Task Contract API integration (**1/1**), persistence command integration (**16/16**), and Overture integration (**6/6**). `npm run build`, `npm run typecheck`, `npm run lint`, and `git diff --check` passed. This is only the durable handoff; no outbox consumer or automatic Head/Council/DepartmentPlan/MissionBundle/Worker progression exists yet.

## Task 16 durable outbox lease checkpoint (2026-09-24)

The next backend prerequisite is now implemented locally: `claimGoalOutbox` uses one PostgreSQL `FOR UPDATE SKIP LOCKED` claim with owner/expiry leases, `markGoalOutboxDelivered` requires the current unexpired owner lease, and `releaseGoalOutbox` returns a failed row to bounded retry without falsely delivering it. Claims are limited to the `goal-events` topic. Focused PostgreSQL verification passed **19 tests across 2 files** (18 command integration tests plus the persistence surface snapshot), including wrong-owner rejection, expiry rejection, reclaim with incremented attempts, release/retry, and delivered-row replay prevention. `npm run build`, `npm run typecheck`, `npm run lint`, and `git diff --check` passed. This is only restart-safe delivery infrastructure: no typed `start_goal` consumer exists, no Head/Council/DepartmentPlan/MissionBundle/Worker progression is executed, and E5/E6 plus live provider evidence remain open.
...
The post-binding unrestricted command `MAESTRO_TEST_DATABASE_URL=postgresql://maestro@127.0.0.1:55432/maestro_local npm test` exited 1 after **441 files / 2,930 passed tests out of 2,931**, duration **918.47s** (`/tmp/maestro-full-test-7.log`). The sole failure was `test/phase8-soak/soak-runner.test.ts`: concurrent report writers could observe the empty lock file between `open(..., "wx")` and its metadata write, producing the stale/unreadable error instead of the active-writer error. `scripts/run-soak.mjs` now writes metadata to a unique temporary file and atomically publishes it with `link`, preserving fail-closed stale-lock handling. The focused soak test passed 5/5, four parallel repeats passed 5/5 each, and build/typecheck/lint/diff checks passed. A fresh unrestricted regression is still required before claiming the repository-wide suite green.

## Task 16 post-soak-fix regression checkpoint (2026-09-24)

After commit `5634e9d1`, unrestricted `MAESTRO_TEST_DATABASE_URL=postgresql://maestro@127.0.0.1:55432/maestro_local npm test` passed **441 test files / 2,931 tests**, exit 0, duration **952.01s** (`/tmp/maestro-full-test-8.log`). This confirms the atomic soak-lock fix and durable `start_goal` binding against the full PostgreSQL suite. It remains automated evidence only; no live provider or E5 completion is claimed.

## Task 16 typed start_goal outbox consumer checkpoint (2026-09-24)

The first active consumer slice is now composed behind explicit `MAESTRO_START_GOAL_OUTBOX_INTERVAL_MS` configuration. PostgreSQL claims only rows whose payload explicitly declares `orchestration.type = "start_goal"`; generic Goal events remain unconsumed. Each claimed row is parsed and durably bound before delivery: malformed typed envelopes and durable binding mismatches are terminally reported and fenced-delivered, while transient validation failures release the owner lease for bounded retry. Startup starts the loop only after migrations/reconciliation and HTTP bind; shutdown stops it. The loop creates no Head, Council, Department Plan, Mission Bundle, Worker, provider, or external effect. RED/GREEN focused verification passed **46 tests across 4 files**, main integration wiring passed **10 tests**, and build/typecheck/lint/diff checks passed. Automatic downstream progression remains open.
