...
## Interaction model clarification — 2026-09-23

Overture is an interactive multi-role conversation, not a batch of hidden role reports. When Concertmaster invokes Overture, the selected Crew roles join the same channel-shaped conversation with explicit role identity. Concertmaster coordinates the turn order and asks the operator focused questions; roles may respond, challenge assumptions, request evidence, and react to the operator's answers. The conversation is resumable and its user, Concertmaster, and role messages are durable and cursor-addressable.

The Task Editor incrementally maintains the detailed execution plan during that conversation. The plan uses the repository's `execution/plan-*.md` style as its document grammar: root outcome and invariants, phase and slice documents, file/subsystem boundaries, dependencies, safe parallelism, tests, evidence, risks, and stop conditions. Each meaningful revision creates a durable plan/document version and hash. The operator sees the evolving summary, material changes, open questions, and plan/contract identities in the same conversation; panels are projections, not a second chat or authority path. No Task Contract is created from a single assistant response or placeholder template when the Crew still needs user clarification.
