...
export function createMaestroAgentRuntime(options: {
  ...
  type RuntimeToolEvent =
    | { readonly kind: "tool-validated"; readonly callId: string; readonly toolName: string }
    | { readonly kind: "tool-executing"; readonly callId: string; readonly toolName: string }
    | { readonly kind: "tool-completed"; readonly callId: string; readonly toolName: string; readonly status: ToolResultStatus }
    | { readonly kind: "tool-rejected"; readonly callId: string; readonly toolName: string; readonly reason: string };

  function emitRuntimeEvent(record: RuntimeRecord, event: RuntimeToolEvent, turnId: string): void {
    record.modelCursor += 1;
    options.onModelEvent?.({ ...event, cursor: record.modelCursor }, turnId);
  }

  function validateAdmission(request: SpawnRequest): {
    ...
    if (request.modelPolicy.length !== 1 || request.modelPolicy[0] !== formatModelRef(options.binding.provider))
      throw new Error("native invocation model is not bound to the gateway model");
    if (request.grant.modelPolicy.length !== 1 || request.grant.modelPolicy[0] !== request.modelPolicy[0])
      throw new Error("native invocation grant model policy mismatch");
    if (request.context.accountRef !== undefined && request.context.accountRef !== options.binding.account.accountRef)
  ...
  function assertChildAdmission(
    parent: RuntimeRecord,
    child: { context: InvocationContext; grant: CapabilityGrant; modelPolicy: readonly string[] },
  ): void {
    const parentContext = parent.context;
    const childContext = child.context;
    const stableContextKeys = [
      "operatorId",
      "projectId",
      "goalId",
      "missionBundleId",
    ...
    if (
      childContext.budgetEffectCents === undefined || parentContext.budgetEffectCents === undefined
        ? childContext.budgetEffectCents !== parentContext.budgetEffectCents
        : childContext.budgetEffectCents > parentContext.budgetEffectCents
    )
      throw new Error("child grant widens parent capability");
    if (
      child.modelPolicy.length !== parent.modelPolicy.length ||
      child.modelPolicy.some((model, index) => model !== parent.modelPolicy[index])
    )
      throw new Error("child grant widens parent capability");
    if (child.grant.allowedTools.some((tool) => !parent.grant.allowedTools.includes(tool)))
      throw new Error("child grant widens parent capability");
    if (child.grant.allowedSkills.some((skill) => !parent.grant.allowedSkills.includes(skill)))
      throw new Error("child grant widens parent capability");
    if (child.grant.pathScope.some((path) => !parent.grant.pathScope.includes(path)))
      throw new Error("child grant widens parent capability");
    if (child.grant.outboundDataClasses.some((dataClass) => !parent.grant.outboundDataClasses.includes(dataClass)))
      throw new Error("child grant widens parent capability");
    const parentRemaining = parent.grant.remaining;
    const childRemaining = child.grant.remaining;
    if (
      childRemaining.modelTurns > parentRemaining.modelTurns ||
      childRemaining.toolCalls > parentRemaining.toolCalls ||
      childRemaining.outputTokens > parentRemaining.outputTokens ||
      childRemaining.wallTimeMs > parentRemaining.wallTimeMs ||
      childRemaining.retryCount > parentRemaining.retryCount
    )
      throw new Error("child grant widens parent capability");
  }
  ...
  async function executeTurn(record: RuntimeRecord, text?: string): Promise<void> {
    ...
    for (;;) {
      ...
      if (record.turnCount >= record.grant.remaining.modelTurns) {
        record.status = "failed";
        record.error = "model turn limit exceeded";
        record.phase = "terminal";
        return;
      }
      record.turnCount += 1;
      ...
      try {
        result = await options.gateway.turn({
          binding: options.binding,
          requestId,
          sessionId: record.sessionId,
          turnId: `${record.invocation}-turn-${record.turnCount}`,
          messages: messagesForGateway(record, turnLimits.maxInputBytes),
          tools: options.tools.definitions(record.grant.allowedTools).slice(0, MAX_WIRE_TOOL_DEFINITIONS),
          limits: turnLimits,
          signal: record.abort.signal,
          emit: (event: ModelStreamEvent) => {
            record.modelCursor = Math.max(record.modelCursor, event.cursor);
            if (event.kind === "tool-proposed") streamedToolCallIds.add(event.call.id);
            if (event.kind !== "text-delta" || event.text === "") {
              options.onModelEvent?.(event, `${record.invocation}-turn-${record.turnCount}`);
              return;
            }
            ...
            if (bounded !== event.text) {
              streamExceeded = true;
              record.abort.abort();
            }
            if (bounded !== "")
              options.onModelEvent?.(
                bounded === event.text ? event : { ...event, text: bounded },
                `${record.invocation}-turn-${record.turnCount}`,
              );
          },
        });
      } catch (error) {
        ...
        if (streamExceeded) {
          record.status = "failed";
          record.error = "model output limit exceeded";
          return;
        }
        if (record.abort.signal.aborted) {
          if ((record.status as InvocationStatus) !== "cancelled" && (record.status as InvocationStatus) !== "unknown") {
            record.status = "unknown";
            record.error = "provider cancellation outcome is unknown";
          }
        } else {
          record.status = "unknown";
          record.error = error instanceof Error ? error.message : "provider outcome is unknown";
        }
        return;
      }
      record.activeRequestId = undefined;
      ...
      if (record.abort.signal.aborted) {
        if ((record.status as InvocationStatus) !== "cancelled") {
          record.status = "unknown";
          record.error = "provider cancellation outcome is unknown";
        }
        record.phase = "terminal";
      ...
      if (Buffer.byteLength(result.text, "utf8") > turnLimits.maxResultBytes) {
        record.status = "failed";
        record.error = "model output limit exceeded";
        record.phase = "terminal";
        return;
      }
      if (result.model.provider !== options.binding.provider.provider || result.model.id !== options.binding.provider.id) {
        record.status = "failed";
        record.error = "provider model identity mismatch";
        record.phase = "terminal";
        return;
      }
      ...
      for (const call of result.toolCalls) {
        const turnId = `${record.invocation}-turn-${record.turnCount}`;
        if (!streamedToolCallIds.has(call.id))
          options.onModelEvent?.({ kind: "tool-proposed", cursor: ++record.modelCursor, call }, turnId);
        const argsKey = safeJson(call.arguments);
        const prior = seen.get(call.id);
        if (prior !== undefined && prior !== argsKey) {
          emitRuntimeEvent(
            record,
            { kind: "tool-rejected", callId: call.id, toolName: call.name, reason: "duplicate tool call identity" },
            turnId,
          );
          record.status = "failed";
          record.error = "tool call ID was reused with different arguments";
          record.phase = "terminal";
          return;
        }
        if (prior !== undefined) {
          ...
          record.status = "failed";
          record.error = "duplicate tool call ID";
          record.phase = "terminal";
          return;
        }
        seen.set(call.id, argsKey);
        if (record.toolCount >= record.grant.remaining.toolCalls) {
          emitRuntimeEvent(
            record,
            { kind: "tool-rejected", callId: call.id, toolName: call.name, reason: "tool call limit exceeded" },
            turnId,
          );
          record.status = "failed";
          record.error = "tool call limit exceeded";
          record.phase = "terminal";
          return;
        }
        record.toolCount += 1;
        const tool = options.tools.get(call.name);
        if (tool === undefined || !record.grant.allowedTools.includes(call.name)) {
          appendToolEvent(record, { state: "waiting", toolName: call.name });
          emitRuntimeEvent(
            record,
            { kind: "tool-rejected", callId: call.id, toolName: call.name, reason: "tool is outside the invocation grant" },
            turnId,
          );
          record.status = "failed";
          record.error = "tool is outside the invocation grant";
          record.phase = "terminal";
          return;
        }
        try {
          if (call.arguments.state !== "valid") throw new Error("invalid tool arguments");
          tool.inputSchema.parse(call.arguments.value);
        } catch {
          appendToolEvent(record, { state: "waiting", toolName: call.name });
          emitRuntimeEvent(
            record,
            { kind: "tool-rejected", callId: call.id, toolName: call.name, reason: "tool arguments failed validation" },
            turnId,
          );
          record.status = "failed";
          record.error = "tool arguments failed validation";
          record.phase = "terminal";
          return;
        }
        emitRuntimeEvent(record, { kind: "tool-validated", callId: call.id, toolName: call.name }, turnId);
        record.phase = "tool_executing";
        appendToolEvent(record, { state: "executing", toolName: call.name });
        emitRuntimeEvent(record, { kind: "tool-executing", callId: call.id, toolName: call.name }, turnId);
        const context: ToolContext = {
          ...
          controllerPolicyHash: record.context.policyVersion,
          capabilityGrant: record.grant,
          outboundDataPolicyHash: options.binding.dataPolicyHash,
        };
        let toolResult: ToolExecutionResult;
        try {
          toolResult = await options.tools.execute(call, context);
        } catch (error) {
          emitRuntimeEvent(record, { kind: "tool-completed", callId: call.id, toolName: call.name, status: "error" }, turnId);
          record.status = "failed";
          record.error = error instanceof Error ? error.message : "tool execution failed";
          record.phase = "terminal";
          return;
        }
        emitRuntimeEvent(record, { kind: "tool-completed", callId: call.id, toolName: call.name, status: toolResult.status }, turnId);
        appendToolEvent(record, { state: "waiting", toolName: call.name });
        record.messages.push({ role: "assistant", content: [{ kind: "tool-call", call }] });
        record.messages.push(
          toolMessage(call.id, { ...toolResult, content: toolResult.content.slice(0, limitsFor(record.grant).maxResultBytes) }),
        );
      }
    }
  }
  ...
  const runtime: MaestroAgentRuntime = {
    async spawn(request): Promise<SpawnedInvocation> {
      ...
      if (request.parent !== undefined) {
        ...
        if (childGrant.parentGrantId !== parent.grant.grantId || childGrant.remaining.childCalls > parent.grant.remaining.childCalls - 1)
          throw new Error("child grant widens parent capability");
      ...
      records.set(invocation, record);
      byExecution.set(execution, invocation);
      return { execution, invocation };
    },
    async prompt(execution, text) {
      const record = rootForExecution(execution);
    ...
    async observe(execution): Promise<readonly InvocationObservation[]> {
      ...
      return [...records.values()]
        ...
        .map((record) => ({
          ...
          toolEvents:
            record.toolEvents.length === 0 ? { state: "empty", events: [] as const } : { state: "available", events: record.toolEvents },
          usage: record.usage,
    ...
    async cancel(invocation) {
      ...
      if (record.activeRequestId !== undefined) {
        ...
        record.status = "unknown";
        record.error = "provider cancellation is not proven";
        record.phase = "terminal";
    ...
    async getModelIdentity(execution) {
      const record = rootForExecution(execution);
      if (record?.model === undefined) throw new ExecutionKernelUnavailableError("getModelIdentity");
      return record.model;
    },
    ...
    async close() {
      ...
      if (options.closeGateway !== false) await options.gateway.close();
    },
    inspectGrantForTest(invocation) {
      return requireRecord(invocation).grant;
    },
  };
  return runtime;
}
