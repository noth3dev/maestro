...
describe("persistent ipython tool boundary", () => {
  ...
  it("binds the model-facing tool to project, Goal, and conversation identity", async () => {
    ...
    expect(requests).toEqual([{ sessionId: '["project-1","goal-1","conversation-1"]', code: "show_lines('README.md')", binding: { sessionId: '["project-1","goal-1","conversation-1"]', operatorId: "operator-1", projectId: "project-1", goalId: "goal-1", pathScope: ["/workspace/project-1"], outboundDataClasses: ["public", "workspace"] } }]);
  });
