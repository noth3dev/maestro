...
const context = {
  ...
  capabilityGrant: {
    grantId: "grant",
    allowedTools: ["task-contract:create"],
    allowedSkills: [],
    modelPolicy: ["fake/model"],
    pathScope: [],
    outboundDataClasses: ["public", "workspace"],
    remaining: { modelTurns: 1, toolCalls: 1, childCalls: 0, outputTokens: 1000, wallTimeMs: 1000, retryCount: 0 },
  },
...
describe("Overture Task Contract drafting tool", () => {
  it("creates the full substance through the injected durable Task Contract creator", async () => {
    const create = vi.fn(async (contractId: string, input: unknown, _operator: { operatorId: string }) => ({
      contractId,
      ...(input as { substance: object }).substance,
      schemaVersion: 1,
      version: 1,
      decisionHistory: [],
    ...
    const tool = createTaskContractDraftingTool({
      createTaskContract: create,
      onCreated: (contract) => {
        observed = contract;
      },
    });
    ...
    expect(create).toHaveBeenCalledWith(
      deriveTaskContractId(context.commandId, context.turnId, context.toolCallId),
      { projectId, substance },
      { operatorId: context.operatorId },
    );
    expect(JSON.parse(result.content)).toMatchObject({
      desiredOutcome: substance.desiredOutcome,
      project: substance.project,
      launchState: "awaiting_confirmation",
    });
  ...
  it("rejects a goal-bound context and never widens the authority boundary", async () => {
    ...
    await expect(tool.execute({ projectId, substance }, { ...context, goalId: "44444444-4444-4444-8444-444444444444" })).rejects.toThrow(
      "goal-less",
    );
