...
it("reads all four goal state resources with typed methods", async () => {
 ...
 await expect(c.listSentinelChallenges(goalId)).resolves.toEqual({challenges:[]}); await expect(c.listOverwatchCouncilRounds(goalId)).resolves.toEqual({rounds:[]}); await expect(c.listCertifications(goalId)).resolves.toEqual({certifications:[]}); await expect(c.getSaneReport(goalId)).resolves.toMatchObject({success:true});
});
