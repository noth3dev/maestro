...
export const BillingDailySpendSchema = z.object({
  date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
  costCents: z.number().int().nonnegative(),
}).strict();
export type BillingDailySpend = z.infer<typeof BillingDailySpendSchema>;

export const BillingGoalSummarySchema = z.object({
  goalId: UuidSchema,
  budgetCents: z.number().int().nonnegative(),
  reservedCents: z.number().int().nonnegative(),
  costCents: z.number().int().nonnegative(),
...
const RoutingShortfallApiSchema = z.object({
...
export const StableApiErrorSchema = z.object({
  error: z.object({ code: StableApiErrorCodeSchema, message: z.string().min(1), detail: z.string().min(1).max(512).optional(), routing: RoutingShortfallApiSchema.optional() }).strict(),
}).strict();
...
export const ProjectionNodeKindSchema = z.enum([
  "goal", "council", "department_plan", "mission_bundle", "worker",
  "git_goal_branch", "git_department_branch", "git_worker_worktree", "git_commit",
  "capability_approval", "improvement_digest",
]);
...
export const ProjectionReadModelSchema = z.object({
...
}).strict().refine((value) => value.projectId !== undefined || value.goalId !== undefined, "projection scope requires projectId or goalId");
...
export const InboxApprovalSchema = z.object({
  decisionId: UuidSchema, commandId: UuidSchema, projectId: UuidSchema, goalId: UuidSchema,
  actorId: z.string().min(1), action: z.string().min(1), target: z.string().min(1),
  policyVersion: z.number().int().nonnegative(), budgetEffectCents: z.number().int().nonnegative(),
  classification: ActionClassificationSchema, reason: z.string().min(1), decidedAt: z.string().datetime(),
}).strict();
...
const ArrangementCandidateTargetSchema = z.object({
  roleId: z.string().min(1).optional(), taskClass: z.string().min(1).optional(), routingTarget: z.string().min(1).optional(),
}).strict();
...
const ArrangementRolloutSchema = z.object({
  ...
  activeVersion: z.number().int().positive(), contentHash: z.string().regex(/^[a-f0-9]{64}$/),
}).strict();
...
const ArrangementCandidateSchema = z.object({
  ...
  kind: z.enum(["persona_axis", "routing_capability_axis"]), state: z.enum(["candidate", "evaluated", "judged", "applied", "rejected", "retained", "rolled_back"]),
  target: ArrangementCandidateTargetSchema, changes: z.array(ArrangementCandidateChangeSchema), sourceEvidenceIds: z.array(UuidSchema), predictedEffect: z.string().min(1),
  contentHash: z.string().regex(/^[a-f0-9]{64}$/), evaluation: ArrangementEvaluationSchema.nullable(), rollout: ArrangementRolloutSchema.nullable(), rejectionReason: z.string().nullable(),
}).strict();
const ArrangementCouncilSchema = z.object({
  candidateId: UuidSchema, roundId: UuidSchema, question: z.string().min(1), finalVerdict: z.enum(["proceed", "do_not_proceed", "escalate"]),
...
export const ChannelMemberSchema = z.object({ identityId: z.string().min(1), identityKind: z.enum(["head", "worker", "role"]), displayName: z.string().min(1), departmentId: z.string().min(1).nullable(), status: z.string().min(1) }).strict();
export type ChannelMember = z.infer<typeof ChannelMemberSchema>;
export const ChannelSchema = z.object({ channelId: UuidSchema, projectId: UuidSchema, goalId: UuidSchema, state: GoalStateSchema, kind: z.enum(["department", "organization", "encore"]), scopeId: z.string().min(1), displayName: z.string().min(1) }).strict();
