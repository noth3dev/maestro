...
export const ProviderCredentialLoginInputSchema = z
  .object({
...
export const ProviderCredentialBindingSchema = z.union([
  z
    .object({ ...ProviderCredentialBindingCommonSchema, providerId: z.enum(["openai", "anthropic"]), authMode: z.literal("api-key") })
    .strict(),
  z
    .object({
      ...ProviderCredentialBindingCommonSchema,
      providerId: z.literal("openai-codex"),
      authMode: z.literal("managed-subscription"),
    })
    .strict(),
]);
...
export const ProviderAccountLoginStartResultSchema = z
  .object({
    providerId: z.literal("openai-codex"),
...
export const ProviderAccountLoginStatusSchema = z
  .object({
    providerId: z.literal("openai-codex"),
    loginId: z.string().min(1).max(256),
...
export const WorkerSchema = z
  .object({
    ...
    status: z.enum(["spawned", "running", "succeeded", "failed", "cancelled", "unknown"]),
...
export const SpawnWorkerInputSchema = z
  .object({ projectId: UuidSchema, planVersion: z.number().int().positive(), itemId: z.string().min(1), model: ModelRefSchema.optional() })
  .strict();
