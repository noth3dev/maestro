...
const RolloutSchema = z.object({
  ...
  rollbackTarget: z.object({ candidate: z.unknown().optional(), candidateId: z.string().min(1), version: z.number().int().positive(), contentHash: z.string().min(1) }).strict(),
...
export const PersonaInspectionSchema = z.object({
  ...
  missionOverlay: DeltaSchema, candidates: z.array(CandidateSchema).readonly(), rollouts: z.array(RolloutSchema).readonly(),
}).strict();
