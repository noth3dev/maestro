...
const proposal = createWorkerProposedKnowledge({ schemaVersion: 1, projectId, sourceGoalId: goalId, departmentId: "engineering", statement: "Keep validation bounded.", rationale: "It prevents recurrence.", sourceEvidenceIds: [evidenceA, evidenceB], sourceDigestIds: [], episodeIds: ["episode-a", "episode-b"], confidence: 0.8, freshness: 1, generalized: false });
const project = promoteKnowledgeToProject(proposal, { promoterRoleKind: "department_head", promoterDepartmentId: "engineering" });
const approved = { encoreCouncilApproved: true, corroboratingSourceIds: [evidenceA, evidenceB], corroboratingEpisodeIds: ["episode-a", "episode-b"], generalizedStatement: "Use bounded validation gates for changes.", curatorRoleId: "head-security", noRawProjectContent: true, noPersonalInformation: true };
