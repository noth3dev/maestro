-- Phase 3 work-sequence step 1: Sentinel event consumer and deterministic
...
CREATE INDEX IF NOT EXISTS sentinel_findings_goal_idx ON sentinel_findings (goal_id, detected_at);
CREATE INDEX IF NOT EXISTS sentinel_findings_unresolved_idx ON sentinel_findings (goal_id) WHERE resolved_at IS NULL;

CREATE OR REPLACE FUNCTION reject_sentinel_finding_mutation()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF OLD.resolved_at IS NOT NULL THEN
    RAISE EXCEPTION 'Sentinel finding is already resolved and immutable';
  END IF;
  IF NEW.goal_id IS DISTINCT FROM OLD.goal_id OR NEW.rule_id IS DISTINCT FROM OLD.rule_id
    ...
    RAISE EXCEPTION 'Sentinel finding identity/detection facts are immutable; only resolution is allowed';
  END IF;
  RETURN NEW;
END;
$$;
DROP TRIGGER IF EXISTS sentinel_findings_immutable ON sentinel_findings;
CREATE TRIGGER sentinel_findings_immutable
  BEFORE UPDATE ON sentinel_findings
  FOR EACH ROW EXECUTE FUNCTION reject_sentinel_finding_mutation();
DROP TRIGGER IF EXISTS sentinel_findings_no_delete ON sentinel_findings;
CREATE OR REPLACE FUNCTION reject_sentinel_finding_delete()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  RAISE EXCEPTION 'Sentinel findings are never deleted, only resolved';
END;
$$;
CREATE TRIGGER sentinel_findings_no_delete
  BEFORE DELETE ON sentinel_findings
  FOR EACH ROW EXECUTE FUNCTION reject_sentinel_finding_delete();
