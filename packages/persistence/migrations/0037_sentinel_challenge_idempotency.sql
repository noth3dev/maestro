-- Phase 3 Sentinel challenge identity/idempotency and canonical actor hardening.
...
ALTER TABLE sentinel_challenges
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
      FROM pg_constraint
     WHERE conrelid = 'sentinel_challenges'::regclass
       AND conname = 'sentinel_challenges_idempotency_key_format'
  ) THEN
    ALTER TABLE sentinel_challenges
      ADD CONSTRAINT sentinel_challenges_idempotency_key_format
  ...
  IF NOT EXISTS (
    SELECT 1
      FROM pg_constraint
     WHERE conrelid = 'sentinel_challenges'::regclass
       AND conname = 'sentinel_challenges_request_hash_format'
  ) THEN
    ALTER TABLE sentinel_challenges
      ADD CONSTRAINT sentinel_challenges_request_hash_format
  ...
  IF NOT EXISTS (
    SELECT 1
      FROM pg_constraint
     WHERE conrelid = 'sentinel_challenges'::regclass
       AND conname = 'sentinel_challenges_identity_hash_pair'
  ) THEN
    ALTER TABLE sentinel_challenges
      ADD CONSTRAINT sentinel_challenges_identity_hash_pair
  ...
  IF NOT EXISTS (
    SELECT 1
      FROM pg_constraint
     WHERE conrelid = 'sentinel_challenges'::regclass
       AND conname = 'sentinel_challenges_goal_idempotency_key_unique'
  ) THEN
    ALTER TABLE sentinel_challenges
      ADD CONSTRAINT sentinel_challenges_goal_idempotency_key_unique
