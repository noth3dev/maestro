...
-- Legacy schemas without payload binding must have no pending proposal authorization rows.
-- Abort before any destructive DDL; the old payload cannot be reconstructed safely.
DO $$
DECLARE has_rows boolean; has_column boolean;
BEGIN
  IF to_regclass('knowledge_proposal_authorizations') IS NOT NULL THEN
    SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = 'knowledge_proposal_authorizations' AND column_name = 'payload_hash') INTO has_column;
    IF has_column THEN
      EXECUTE 'SELECT EXISTS (SELECT 1 FROM knowledge_proposal_authorizations WHERE payload_hash IS NULL)' INTO has_rows;
    ELSE
      EXECUTE 'SELECT EXISTS (SELECT 1 FROM knowledge_proposal_authorizations)' INTO has_rows;
    END IF;
    IF has_rows THEN RAISE EXCEPTION 'migration 0087 requires pending knowledge proposal authorizations to be drained before payload binding'; END IF;
  END IF;
END $$;
...
DO $$
DECLARE has_author_operator boolean; has_author_role boolean; bad boolean;
BEGIN
  IF to_regclass('organizational_knowledge') IS NOT NULL THEN
    SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = 'organizational_knowledge' AND column_name = 'author_operator_id') INTO has_author_operator;
    SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = 'organizational_knowledge' AND column_name = 'author_role_id') INTO has_author_role;
    IF has_author_operator AND has_author_role THEN EXECUTE 'SELECT EXISTS (SELECT 1 FROM organizational_knowledge WHERE scope = ''worker_proposed'' AND (author_operator_id IS NULL OR author_role_id IS NULL))' INTO bad; ELSE EXECUTE 'SELECT EXISTS (SELECT 1 FROM organizational_knowledge WHERE scope = ''worker_proposed'')' INTO bad; END IF;
    IF bad THEN RAISE EXCEPTION 'migration 0087 requires existing worker proposals to be reissued with durable author identity'; END IF;
  END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS organizational_knowledge (
  ...
  generalized_statement text CHECK (generalized_statement IS NULL OR (btrim(generalized_statement) <> '' AND length(generalized_statement) <= 4096)),
  ...
  promotion_operator_id uuid,
  promotion_role_id text CHECK (promotion_role_id IS NULL OR (btrim(promotion_role_id) <> '' AND length(promotion_role_id) <= 256)),
...
ALTER TABLE organizational_knowledge ADD COLUMN IF NOT EXISTS curator_operator_id uuid;
...
ALTER TABLE organizational_knowledge ADD COLUMN IF NOT EXISTS promotion_operator_id uuid;
ALTER TABLE organizational_knowledge ADD COLUMN IF NOT EXISTS promotion_role_id text;
...
CREATE TABLE IF NOT EXISTS organizational_knowledge_digest_loss_authorizations (
...
ALTER TABLE organizational_knowledge_digest_loss_authorizations DROP CONSTRAINT IF EXISTS organizational_knowledge_digest_loss_authorizations_text_safety;
ALTER TABLE organizational_knowledge_digest_loss_authorizations ADD CONSTRAINT organizational_knowledge_digest_loss_authorizations_text_safety CHECK (reason !~* '(authorization[[:space:]]*:[[:space:]]*bearer|password[[:space:]]*[:=]|secret[[:space:]]*[:=]|api[_-]?key[[:space:]]*[:=]|private[_-]?key|-----BEGIN.*PRIVATE KEY-----|(^|[^a-z])(email|phone|ssn|social security|home address|personal information)([^a-z]|$))');
...
DROP FUNCTION IF EXISTS validate_organizational_knowledge_digest_loss_authorization_provenance();
...
BEGIN
  RAISE EXCEPTION 'organizational knowledge digest losses are append-only';
END;
$$;
DROP TRIGGER IF EXISTS organizational_knowledge_digest_losses_immutable ON organizational_knowledge_digest_losses;
CREATE TRIGGER organizational_knowledge_digest_losses_immutable BEFORE UPDATE OR DELETE ON organizational_knowledge_digest_losses FOR EACH ROW EXECUTE FUNCTION reject_organizational_knowledge_digest_loss_mutation();
CREATE OR REPLACE FUNCTION authorize_knowledge_digest_loss(p_token text, p_digest_id uuid, p_goal_id uuid, p_project_id uuid, p_owner_id text, p_fencing_token bigint, p_reason text, p_recorded_by uuid, p_role_id text) RETURNS void LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
  IF btrim(p_token) = '' OR btrim(p_reason) = '' OR length(p_reason) > 1024 OR p_reason ~* '(authorization[[:space:]]*:[[:space:]]*bearer|password[[:space:]]*[:=]|secret[[:space:]]*[:=]|api[_-]?key[[:space:]]*[:=]|private[_-]?key|-----BEGIN.*PRIVATE KEY-----|(^|[^a-z])(email|phone|ssn|social security|home address|personal information)([^a-z]|$))'
     OR NOT EXISTS (SELECT 1 FROM improvement_digests d WHERE d.digest_id = p_digest_id AND d.goal_id = p_goal_id AND d.project_id = p_project_id)
     OR NOT EXISTS (SELECT 1 FROM local_operators WHERE operator_id = p_recorded_by AND active = true)
     OR NOT EXISTS (SELECT 1 FROM permanent_roles WHERE status = 'standing' AND ((role_id = p_role_id AND role_kind = 'department_head') OR (p_role_id = 'ceo' AND role_id = 'concertmaster' AND role_kind = 'concertmaster')))
     OR NOT EXISTS (SELECT 1 FROM operator_project_roles r JOIN operator_project_memberships m ON m.operator_id = r.operator_id AND m.project_id = r.project_id AND m.active = true WHERE r.operator_id = p_recorded_by AND r.project_id = p_project_id AND r.role_id = p_role_id AND r.active = true)
  ...
  DELETE FROM organizational_knowledge_digest_loss_authorizations WHERE token_hash = encode(public.digest(p_token, 'sha256'), 'hex');
  IF EXISTS (SELECT 1 FROM organizational_knowledge_digest_losses l WHERE l.digest_id = p_digest_id AND (l.goal_id IS DISTINCT FROM p_goal_id OR l.project_id IS DISTINCT FROM p_project_id OR l.owner_id IS DISTINCT FROM p_owner_id OR l.fencing_token IS DISTINCT FROM p_fencing_token OR l.reason IS DISTINCT FROM p_reason OR l.recorded_by IS DISTINCT FROM p_recorded_by OR l.role_id IS DISTINCT FROM p_role_id)) THEN
    RAISE EXCEPTION 'conflicting knowledge digest loss retry';
  END IF;
END;
$$;

CREATE OR REPLACE FUNCTION authorize_organizational_knowledge_digest_loss_insert() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM knowledge_issuer_transaction_markers m WHERE m.transaction_id = txid_current() AND m.issuer_kind = 'digest_loss' AND m.token_hash = NEW.token_hash) THEN RAISE EXCEPTION 'digest-loss authorization must be issued by secured function'; END IF;
  DELETE FROM knowledge_issuer_transaction_markers WHERE transaction_id = txid_current() AND issuer_kind = 'digest_loss' AND token_hash = NEW.token_hash;
  RETURN NEW;
END;
$$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM organizational_knowledge_digest_loss_authorizations a
     WHERE a.token_hash = NEW.token_hash AND a.digest_id = NEW.digest_id AND a.goal_id = NEW.goal_id AND a.project_id = NEW.project_id
       AND a.owner_id = NEW.owner_id AND a.fencing_token = NEW.fencing_token AND a.reason = NEW.reason
       AND a.recorded_by = NEW.recorded_by AND a.role_id = NEW.role_id AND a.retention = NEW.retention
...
ALTER TABLE organizational_knowledge_digest_losses DROP CONSTRAINT IF EXISTS organizational_knowledge_digest_losses_text_safety;
ALTER TABLE organizational_knowledge_digest_losses ADD CONSTRAINT organizational_knowledge_digest_losses_text_safety CHECK (reason !~* '(authorization[[:space:]]*:[[:space:]]*bearer|password[[:space:]]*[:=]|secret[[:space:]]*[:=]|api[_-]?key[[:space:]]*[:=]|private[_-]?key|-----BEGIN.*PRIVATE KEY-----|(^|[^a-z])(email|phone|ssn|social security|home address|personal information)([^a-z]|$))');
...
CREATE TABLE IF NOT EXISTS knowledge_issuer_transaction_markers (
  ...
  token_hash char(64),
  nonce uuid,
  issuer_kind text NOT NULL CHECK (issuer_kind IN ('proposal', 'promotion', 'source_loss', 'maintenance', 'cleanup', 'digest_loss')),
  created_at timestamptz NOT NULL DEFAULT transaction_timestamp(),
  PRIMARY KEY (transaction_id, issuer_kind)
);
REVOKE ALL ON knowledge_issuer_transaction_markers FROM PUBLIC;
ALTER TABLE knowledge_issuer_transaction_markers ADD COLUMN IF NOT EXISTS token_hash char(64);
ALTER TABLE knowledge_issuer_transaction_markers ADD COLUMN IF NOT EXISTS nonce uuid;
ALTER TABLE knowledge_issuer_transaction_markers DROP CONSTRAINT IF EXISTS knowledge_issuer_transaction_markers_issuer_kind_check;
ALTER TABLE knowledge_issuer_transaction_markers ADD CONSTRAINT knowledge_issuer_transaction_markers_issuer_kind_check CHECK (issuer_kind IN ('proposal', 'promotion', 'source_loss', 'maintenance', 'cleanup', 'digest_loss'));
DELETE FROM knowledge_issuer_transaction_markers
 WHERE issuer_kind NOT IN ('proposal', 'promotion', 'source_loss', 'maintenance', 'cleanup', 'digest_loss')
    OR (issuer_kind = 'cleanup' AND (nonce IS NULL OR token_hash IS NOT NULL))
    OR (issuer_kind <> 'cleanup' AND (token_hash IS NULL OR nonce IS NOT NULL));
ALTER TABLE knowledge_issuer_transaction_markers DROP CONSTRAINT IF EXISTS knowledge_issuer_transaction_markers_binding_check;
ALTER TABLE knowledge_issuer_transaction_markers ADD CONSTRAINT knowledge_issuer_transaction_markers_binding_check CHECK ((issuer_kind = 'cleanup' AND nonce IS NOT NULL AND token_hash IS NULL) OR (issuer_kind <> 'cleanup' AND token_hash IS NOT NULL AND nonce IS NULL));
...
CREATE TABLE IF NOT EXISTS knowledge_promotion_authorizations (
  ...
  role_id text NOT NULL CHECK (btrim(role_id) <> '' AND length(role_id) <= 256),
  department_id text NOT NULL REFERENCES departments(department_id) CHECK (btrim(department_id) <> '' AND length(department_id) <= 256),
  ...
  payload_json jsonb,
  authorization_transaction_id bigint NOT NULL DEFAULT txid_current(),
  created_at timestamptz NOT NULL DEFAULT transaction_timestamp(),
  retention retention_class NOT NULL DEFAULT 'project_lifetime'
);
...
ALTER TABLE knowledge_promotion_authorizations ADD COLUMN IF NOT EXISTS payload_json jsonb;
ALTER TABLE knowledge_promotion_authorizations ADD COLUMN IF NOT EXISTS authorization_transaction_id bigint NOT NULL DEFAULT txid_current();
...
ALTER TABLE knowledge_promotion_authorizations ADD CONSTRAINT knowledge_promotion_authorizations_curator_role_check CHECK (curator_role_id IS NULL OR (btrim(curator_role_id) <> '' AND length(curator_role_id) <= 256));
ALTER TABLE knowledge_promotion_authorizations DROP CONSTRAINT IF EXISTS knowledge_promotion_authorizations_role_check;
ALTER TABLE knowledge_promotion_authorizations ADD CONSTRAINT knowledge_promotion_authorizations_role_check CHECK (btrim(role_id) <> '' AND length(role_id) <= 256);
ALTER TABLE knowledge_promotion_authorizations DROP CONSTRAINT IF EXISTS knowledge_promotion_authorizations_scope_check;
ALTER TABLE knowledge_promotion_authorizations ADD CONSTRAINT knowledge_promotion_authorizations_scope_check CHECK (scope IN ('project_department', 'global'));
ALTER TABLE knowledge_promotion_authorizations DROP CONSTRAINT IF EXISTS knowledge_promotion_authorizations_text_safety;
...
ALTER TABLE knowledge_refresh_operations ADD CONSTRAINT knowledge_refresh_operations_operation_ref_safety CHECK (operation_ref ~ '^[A-Za-z0-9:_./-]+$' AND operation_ref !~* '(authorization[[:space:]]*:[[:space:]]*bearer|password[[:space:]]*[:=]|secret[[:space:]]*[:=]|api[_-]?key[[:space:]]*[:=]|private[_-]?key|-----BEGIN.*PRIVATE KEY-----|(^|[^a-z])(email|phone|ssn|social security|home address|personal information)([^a-z]|$))');

REVOKE INSERT, UPDATE, DELETE, TRUNCATE ON knowledge_refresh_operations FROM PUBLIC;

CREATE TABLE IF NOT EXISTS knowledge_maintenance_authorizations (
  token_hash char(64) PRIMARY KEY CHECK (token_hash ~ '^[0-9a-f]{64}$'), knowledge_id uuid NOT NULL, revision integer NOT NULL,
...
ALTER TABLE knowledge_maintenance_authorizations DROP CONSTRAINT IF EXISTS knowledge_maintenance_authorizations_text_safety;
ALTER TABLE knowledge_maintenance_authorizations ADD CONSTRAINT knowledge_maintenance_authorizations_text_safety CHECK (concat_ws('|', reason, source_session_ref) !~* '(authorization[[:space:]]*:[[:space:]]*bearer|password[[:space:]]*[:=]|secret[[:space:]]*[:=]|api[_-]?key[[:space:]]*[:=]|private[_-]?key|-----BEGIN.*PRIVATE KEY-----|(^|[^a-z])(email|phone|ssn|social security|home address|personal information)([^a-z]|$))');
...
BEGIN
  IF EXISTS (SELECT 1 FROM jsonb_array_elements_text(NEW.source_evidence_ids) value WHERE value <> lower(value)) OR EXISTS (SELECT 1 FROM jsonb_array_elements_text(NEW.source_digest_ids) value WHERE value <> lower(value)) THEN
    RAISE EXCEPTION 'organizational knowledge UUID references must be lowercase canonical values';
  END IF;
  ...
  IF concat_ws('|', NEW.reason, NEW.created_by, NEW.source_session_ref, NEW.curator_role_id, NEW.curator_department_id, NEW.author_role_id, NEW.promotion_role_id, NEW.department_id, NEW.promotion_marker) ~* '(^|[^a-z])(email|phone|telephone|ssn|social security|home address|personal information)([^a-z]|$)' THEN
    RAISE EXCEPTION 'organizational knowledge metadata contains personal information';
  END IF;
  ...
  IF NEW.promotion_marker = 'source-loss' AND (NEW.status <> 'unsupported' OR NOT EXISTS (SELECT 1 FROM organizational_knowledge prior WHERE prior.knowledge_id = NEW.knowledge_id AND prior.revision = NEW.revision - 1 AND prior.status NOT IN ('unsupported', 'retired'))) THEN RAISE EXCEPTION 'source-loss revisions must be unsupported transitions from live knowledge'; END IF;
  IF NEW.status IN ('unsupported', 'contradicted', 'retired') AND NEW.promotion_marker NOT IN ('knowledge-decay', 'adjudication', 'source-loss') THEN RAISE EXCEPTION 'maintenance revision marker is invalid'; END IF;
  IF NEW.status IN ('unsupported', 'contradicted', 'retired') AND NEW.promotion_marker <> 'source-loss' AND EXISTS (SELECT 1 FROM organizational_knowledge prior WHERE prior.knowledge_id = NEW.knowledge_id AND prior.revision = NEW.revision - 1 AND (to_jsonb(prior) - ARRAY['revision','status','reason','source_session_ref','created_at','confidence','freshness','operation_payload_hash','promotion_operator_id','promotion_role_id','promotion_marker']) IS DISTINCT FROM (to_jsonb(NEW) - ARRAY['revision','status','reason','source_session_ref','created_at','confidence','freshness','operation_payload_hash','promotion_operator_id','promotion_role_id','promotion_marker'])) THEN RAISE EXCEPTION 'maintenance revision may only change lifecycle or decay metadata'; END IF;
  IF NEW.promotion_marker = 'source-loss' AND NEW.status = 'unsupported' AND EXISTS (SELECT 1 FROM organizational_knowledge prior WHERE prior.knowledge_id = NEW.knowledge_id AND prior.revision = NEW.revision - 1 AND (prior.schema_version IS DISTINCT FROM NEW.schema_version OR prior.source_project_id IS DISTINCT FROM NEW.source_project_id OR prior.project_id IS DISTINCT FROM NEW.project_id OR prior.source_goal_id IS DISTINCT FROM NEW.source_goal_id OR prior.department_id IS DISTINCT FROM NEW.department_id OR prior.scope IS DISTINCT FROM NEW.scope OR prior.statement IS DISTINCT FROM NEW.statement OR prior.rationale IS DISTINCT FROM NEW.rationale OR prior.source_evidence_ids IS DISTINCT FROM NEW.source_evidence_ids OR prior.source_digest_ids IS DISTINCT FROM NEW.source_digest_ids OR prior.episode_ids IS DISTINCT FROM NEW.episode_ids OR prior.confidence IS DISTINCT FROM NEW.confidence OR prior.freshness IS DISTINCT FROM NEW.freshness OR prior.generalized IS DISTINCT FROM NEW.generalized OR prior.council_round_id IS DISTINCT FROM NEW.council_round_id OR prior.generalized_statement IS DISTINCT FROM NEW.generalized_statement OR prior.curator_role_id IS DISTINCT FROM NEW.curator_role_id OR prior.curator_operator_id IS DISTINCT FROM NEW.curator_operator_id OR prior.curator_department_id IS DISTINCT FROM NEW.curator_department_id OR prior.created_by IS DISTINCT FROM NEW.created_by OR prior.author_operator_id IS DISTINCT FROM NEW.author_operator_id OR prior.author_role_id IS DISTINCT FROM NEW.author_role_id OR prior.retention IS DISTINCT FROM NEW.retention)) THEN RAISE EXCEPTION 'source-loss revision may only change lifecycle metadata'; END IF;
  IF NEW.revision > 1 AND EXISTS (SELECT 1 FROM organizational_knowledge prior WHERE prior.knowledge_id = NEW.knowledge_id AND prior.revision = NEW.revision - 1 AND (prior.schema_version IS DISTINCT FROM NEW.schema_version OR prior.source_project_id IS DISTINCT FROM NEW.source_project_id OR prior.source_goal_id IS DISTINCT FROM NEW.source_goal_id OR prior.department_id IS DISTINCT FROM NEW.department_id OR prior.retention IS DISTINCT FROM NEW.retention OR prior.author_operator_id IS DISTINCT FROM NEW.author_operator_id OR prior.author_role_id IS DISTINCT FROM NEW.author_role_id)) THEN RAISE EXCEPTION 'organizational knowledge core provenance is immutable'; END IF;
  IF NEW.scope = 'project_department' AND NEW.status = 'active' AND EXISTS (
    SELECT 1 FROM organizational_knowledge prior
     WHERE prior.knowledge_id = NEW.knowledge_id AND prior.revision = NEW.revision - 1
       AND prior.scope = 'worker_proposed' AND prior.status = 'proposed'
       AND (to_jsonb(prior) - ARRAY['revision','status','scope','reason','source_session_ref','created_at','operation_payload_hash','promotion_operator_id','promotion_role_id','promotion_marker']) IS DISTINCT FROM
           (to_jsonb(NEW) - ARRAY['revision','status','scope','reason','source_session_ref','created_at','operation_payload_hash','promotion_operator_id','promotion_role_id','promotion_marker'])
  ) THEN RAISE EXCEPTION 'project promotion must preserve the proposed knowledge content'; END IF;
  IF NEW.scope = 'project_department' AND NEW.status = 'active' AND EXISTS (
    SELECT 1 FROM organizational_knowledge prior
     WHERE prior.knowledge_id = NEW.knowledge_id AND prior.revision = NEW.revision - 1
  ...
  IF NEW.scope = 'global' AND NEW.status = 'active' AND EXISTS (
    SELECT 1 FROM organizational_knowledge prior
     WHERE prior.knowledge_id = NEW.knowledge_id AND prior.revision = NEW.revision - 1
       ...
       AND (to_jsonb(prior) - ARRAY['revision','status','reason','source_session_ref','created_at','confidence','freshness','operation_payload_hash']) IS DISTINCT FROM
           (to_jsonb(NEW) - ARRAY['revision','status','reason','source_session_ref','created_at','confidence','freshness','operation_payload_hash'])
  ) THEN RAISE EXCEPTION 'active global maintenance may only change decay metadata'; END IF;
  IF NEW.status = 'active' AND NEW.scope IN ('project_department', 'global') AND NEW.author_operator_id = NEW.promotion_operator_id THEN RAISE EXCEPTION 'organizational knowledge author and promotion operators must be distinct'; END IF;
  ...
  END IF;
  IF NEW.scope = 'project_department' AND NEW.status = 'active' AND NEW.promotion_marker = 'department-promotion' AND NOT EXISTS (SELECT 1 FROM organizational_knowledge prior WHERE prior.knowledge_id = NEW.knowledge_id AND prior.revision = NEW.revision - 1 AND ((prior.scope = 'worker_proposed' AND prior.status = 'proposed') OR (prior.scope = 'project_department' AND prior.status = 'active'))) THEN
    RAISE EXCEPTION 'project organizational knowledge requires proposal-first lineage';
  END IF;
  IF NEW.scope = 'global' AND NEW.status = 'active' AND NEW.promotion_marker = 'global-promotion' AND NOT EXISTS (SELECT 1 FROM organizational_knowledge prior WHERE prior.knowledge_id = NEW.knowledge_id AND prior.revision = NEW.revision - 1 AND ((prior.scope = 'project_department' AND prior.status = 'active') OR (prior.scope = 'global' AND prior.status = 'active'))) THEN
  ...
  IF NEW.scope = 'global' AND NEW.status = 'active' AND NOT EXISTS (
    SELECT 1 FROM encore_council_rounds r JOIN encore_council_syntheses s ON s.round_id = r.round_id
     WHERE r.round_id = NEW.council_round_id AND r.goal_id = NEW.source_goal_id AND s.final_verdict = 'proceed' AND s.same_model_only = false
       ...
       AND (SELECT count(DISTINCT j.judgment_id) FROM encore_council_judgments j JOIN native_execution_bindings b ON b.execution_ref = j.execution_ref AND b.invocation_ref = j.invocation_ref AND b.goal_id = r.goal_id AND b.project_id = NEW.source_project_id AND b.admission_kind = 'encore_reviewer' AND b.actual_model_provider = j.model_provider AND b.actual_model_id = j.model_id AND b.operator_id IS DISTINCT FROM NEW.author_operator_id::text WHERE j.round_id = r.round_id) = r.reviewer_count
  ) THEN RAISE EXCEPTION 'global organizational knowledge requires exact durable Council review'; END IF;
  IF NEW.scope = 'worker_proposed' AND NOT EXISTS (SELECT 1 FROM knowledge_proposal_authorizations a WHERE a.knowledge_id = NEW.knowledge_id AND a.revision = NEW.revision AND a.project_id = NEW.source_project_id AND a.goal_id = NEW.source_goal_id AND a.actor_id = NEW.created_by AND a.session_ref = NEW.source_session_ref AND a.operator_id = NULLIF(current_setting('maestro.knowledge_proposal_operator', true), '')::uuid AND a.owner_id = current_setting('maestro.knowledge_proposal_owner', true) AND a.operator_id = NEW.author_operator_id AND a.role_id = NEW.author_role_id AND a.fencing_token = NULLIF(current_setting('maestro.knowledge_proposal_fence', true), '')::bigint AND a.token_hash = encode(public.digest(current_setting('maestro.knowledge_proposal_token', true), 'sha256'), 'hex')
  ...
  IF NEW.status = 'active' AND NEW.scope IN ('project_department', 'global') AND NOT EXISTS (
    SELECT 1 FROM knowledge_promotion_authorizations a
     WHERE a.knowledge_id = NEW.knowledge_id AND a.revision = NEW.revision AND a.scope = NEW.scope
       AND a.role_id = NEW.promotion_role_id AND a.department_id = NEW.department_id AND a.operator_id = NEW.promotion_operator_id
       AND a.goal_id = NEW.source_goal_id AND a.authorization_transaction_id = txid_current()
       AND a.payload_hash = NEW.operation_payload_hash AND a.payload_hash IS NOT NULL
       AND (NEW.scope <> 'global' OR (a.curator_role_id = NEW.curator_role_id AND a.curator_operator_id = NEW.curator_operator_id AND a.curator_department_id = NEW.curator_department_id AND a.curator_operator_id IS NOT NULL
         AND a.curator_operator_id <> a.operator_id AND a.curator_role_id <> a.role_id AND a.curator_department_id <> NEW.department_id
         AND EXISTS (SELECT 1 FROM local_operators co WHERE co.operator_id = a.curator_operator_id AND co.active = true)
         AND EXISTS (SELECT 1 FROM permanent_roles cr WHERE cr.role_id = a.curator_role_id AND cr.role_kind = 'department_head' AND cr.department_id = a.curator_department_id AND cr.status = 'standing')
         AND EXISTS (SELECT 1 FROM operator_project_memberships cm WHERE cm.operator_id = a.curator_operator_id AND cm.project_id = NEW.source_project_id AND cm.active = true)
         AND EXISTS (SELECT 1 FROM operator_project_roles crole JOIN operator_project_memberships cm ON cm.operator_id = crole.operator_id AND cm.project_id = crole.project_id AND cm.active = true WHERE crole.operator_id = a.curator_operator_id AND crole.project_id = NEW.source_project_id AND crole.role_id = a.curator_role_id AND crole.active = true)))
  ...
  ) THEN RAISE EXCEPTION 'organizational knowledge promotion payload is not canonical for the inserted revision'; END IF;
  ...
  FOR item IN SELECT value FROM jsonb_array_elements(NEW.source_evidence_ids) LOOP
    ...
    IF NOT EXISTS (SELECT 1 FROM evidence_records WHERE evidence_id = ref_id AND project_id = NEW.source_project_id AND goal_id = NEW.source_goal_id)
       AND NOT (
         (NEW.status = 'unsupported' AND NEW.promotion_marker = 'source-loss' AND NEW.source_session_ref = 'evidence:' || ref_id::text
          AND EXISTS (SELECT 1 FROM organizational_knowledge prior WHERE prior.knowledge_id = NEW.knowledge_id AND prior.revision = NEW.revision - 1 AND prior.status NOT IN ('unsupported', 'retired') AND prior.source_evidence_ids @> jsonb_build_array(ref_id::text))
          AND EXISTS (SELECT 1 FROM source_evidence_loss_authorizations a JOIN source_evidence_loss_events e ON e.evidence_id = a.evidence_id AND e.goal_id = a.goal_id AND e.project_id = a.project_id AND e.reason = a.reason AND e.recorded_by = a.recorded_by WHERE a.evidence_id = ref_id AND a.goal_id = NEW.source_goal_id AND a.project_id = NEW.source_project_id AND a.token_hash = encode(public.digest(current_setting('maestro.source_evidence_loss_token', true), 'sha256'), 'hex')))
         OR (NEW.status = 'retired' AND EXISTS (SELECT 1 FROM organizational_knowledge prior WHERE prior.knowledge_id = NEW.knowledge_id AND prior.revision = NEW.revision - 1 AND prior.status = 'unsupported' AND prior.promotion_marker = 'source-loss' AND prior.source_project_id = NEW.source_project_id AND prior.source_goal_id = NEW.source_goal_id AND prior.source_evidence_ids = NEW.source_evidence_ids))
  ...
  FOR item IN SELECT value FROM jsonb_array_elements(NEW.source_digest_ids) LOOP
    ...
    ref_id := (item #>> '{}')::uuid;
    IF (NOT EXISTS (SELECT 1 FROM improvement_digests WHERE digest_id = ref_id AND project_id = NEW.source_project_id AND goal_id = NEW.source_goal_id)
        OR EXISTS (SELECT 1 FROM organizational_knowledge_digest_losses l WHERE l.digest_id = ref_id AND l.goal_id = NEW.source_goal_id AND l.project_id = NEW.source_project_id))
       AND NOT (
         (NEW.status = 'unsupported' AND NEW.promotion_marker = 'source-loss' AND NEW.source_session_ref = 'digest:' || ref_id::text
          AND EXISTS (SELECT 1 FROM organizational_knowledge prior WHERE prior.knowledge_id = NEW.knowledge_id AND prior.revision = NEW.revision - 1 AND prior.status NOT IN ('unsupported', 'retired') AND prior.source_digest_ids @> jsonb_build_array(ref_id::text))
          AND EXISTS (SELECT 1 FROM organizational_knowledge_digest_losses l WHERE l.digest_id = ref_id AND l.goal_id = NEW.source_goal_id AND l.project_id = NEW.source_project_id))
         OR (NEW.status = 'retired' AND EXISTS (SELECT 1 FROM organizational_knowledge prior WHERE prior.knowledge_id = NEW.knowledge_id AND prior.revision = NEW.revision - 1 AND prior.status = 'unsupported' AND prior.promotion_marker = 'source-loss' AND prior.source_project_id = NEW.source_project_id AND prior.source_goal_id = NEW.source_goal_id AND prior.source_digest_ids = NEW.source_digest_ids))
       ) THEN RAISE EXCEPTION 'organizational knowledge digest reference is missing or outside source project'; END IF;
  END LOOP;
  ...
  IF NEW.revision > 1 AND NEW.status IN ('unsupported', 'contradicted', 'retired') AND NOT (NEW.promotion_marker = 'source-loss' AND NEW.source_session_ref LIKE 'evidence:%' AND EXISTS (SELECT 1 FROM source_evidence_loss_events e JOIN source_evidence_loss_authorizations a ON a.evidence_id = e.evidence_id AND a.goal_id = e.goal_id AND a.project_id = e.project_id AND a.reason = e.reason AND a.recorded_by = e.recorded_by AND e.reason = NEW.reason AND a.role_id = NEW.promotion_role_id AND NEW.promotion_operator_id::text = e.recorded_by WHERE e.evidence_id = substring(NEW.source_session_ref FROM 10)::uuid AND e.goal_id = NEW.source_goal_id AND e.project_id = NEW.source_project_id AND a.token_hash = encode(public.digest(current_setting('maestro.source_evidence_loss_token', true), 'sha256'), 'hex'))) AND NOT EXISTS (SELECT 1 FROM knowledge_maintenance_authorizations a WHERE a.knowledge_id = NEW.knowledge_id AND a.revision = NEW.revision AND a.status = NEW.status AND a.operator_id = NULLIF(current_setting('maestro.knowledge_maintenance_operator', true), '')::uuid AND a.role_id = current_setting('maestro.knowledge_maintenance_role', true) AND a.owner_id = current_setting('maestro.knowledge_maintenance_owner', true) AND a.fencing_token = NULLIF(current_setting('maestro.knowledge_maintenance_fence', true), '')::bigint AND a.reason = NEW.reason AND a.source_session_ref = NEW.source_session_ref AND a.token_hash = encode(public.digest(current_setting('maestro.knowledge_maintenance_token', true), 'sha256'), 'hex')) THEN RAISE EXCEPTION 'organizational knowledge maintenance authorization is missing or mismatched'; END IF;
  IF NEW.revision > 1 AND NEW.status IN ('unsupported', 'contradicted', 'retired') AND EXISTS (SELECT 1 FROM knowledge_maintenance_authorizations a WHERE a.knowledge_id = NEW.knowledge_id AND a.revision = NEW.revision AND a.token_hash = encode(public.digest(current_setting('maestro.knowledge_maintenance_token', true), 'sha256'), 'hex')) THEN DELETE FROM knowledge_maintenance_authorizations WHERE knowledge_id = NEW.knowledge_id AND revision = NEW.revision; END IF;
  RETURN NEW;
END;
$$;
...
CREATE TABLE IF NOT EXISTS evidence_source_tombstones (
  ...
  owner_id text NOT NULL CHECK (btrim(owner_id) <> '' AND length(owner_id) <= 256), fencing_token bigint NOT NULL CHECK (fencing_token > 0),
  reason text NOT NULL CHECK (btrim(reason) <> '' AND length(reason) <= 1024), recorded_by text NOT NULL CHECK (btrim(recorded_by) <> '' AND length(recorded_by) <= 256),
...
CREATE UNIQUE INDEX IF NOT EXISTS source_evidence_loss_events_evidence_unique ON source_evidence_loss_events (evidence_id);
...
ALTER TABLE source_evidence_loss_events DROP CONSTRAINT IF EXISTS source_evidence_loss_events_text_safety;
ALTER TABLE source_evidence_loss_events ADD CONSTRAINT source_evidence_loss_events_text_safety CHECK (concat_ws('|', owner_id, reason, recorded_by) !~* '(authorization[[:space:]]*:[[:space:]]*bearer|password[[:space:]]*[:=]|secret[[:space:]]*[:=]|api[_-]?key[[:space:]]*[:=]|private[_-]?key|-----BEGIN.*PRIVATE KEY-----|(^|[^a-z])(email|phone|ssn|social security|home address|personal information)([^a-z]|$))');
ALTER TABLE source_evidence_loss_authorizations DROP CONSTRAINT IF EXISTS source_evidence_loss_authorizations_text_safety;
ALTER TABLE source_evidence_loss_authorizations ADD CONSTRAINT source_evidence_loss_authorizations_text_safety CHECK (concat_ws('|', owner_id, reason, recorded_by, role_id) !~* '(authorization[[:space:]]*:[[:space:]]*bearer|password[[:space:]]*[:=]|secret[[:space:]]*[:=]|api[_-]?key[[:space:]]*[:=]|private[_-]?key|-----BEGIN.*PRIVATE KEY-----|(^|[^a-z])(email|phone|ssn|social security|home address|personal information)([^a-z]|$))');
...
CREATE OR REPLACE FUNCTION validate_evidence_source_tombstone_binding() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM source_evidence_loss_authorizations a JOIN source_evidence_loss_events e ON e.evidence_id = a.evidence_id AND e.goal_id = a.goal_id AND e.project_id = a.project_id AND e.owner_id = a.owner_id AND e.fencing_token = a.fencing_token AND e.reason = a.reason AND e.recorded_by = a.recorded_by WHERE a.evidence_id = NEW.evidence_id AND a.goal_id = NEW.goal_id AND a.project_id = NEW.project_id AND a.owner_id = NEW.owner_id AND a.fencing_token = NEW.fencing_token AND a.reason = NEW.reason AND a.recorded_by = NEW.recorded_by AND a.token_hash = encode(public.digest(current_setting('maestro.source_evidence_loss_token', true), 'sha256'), 'hex')) THEN
    RAISE EXCEPTION 'evidence source tombstone authorization or loss event is missing';
  END IF;
  RETURN NEW;
END;
$$;
DROP TRIGGER IF EXISTS evidence_source_tombstones_binding ON evidence_source_tombstones;
CREATE TRIGGER evidence_source_tombstones_binding BEFORE INSERT ON evidence_source_tombstones FOR EACH ROW EXECUTE FUNCTION validate_evidence_source_tombstone_binding();
CREATE OR REPLACE FUNCTION reject_evidence_source_tombstone_mutation() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN RAISE EXCEPTION 'evidence source tombstones are immutable'; END;
$$;
DROP TRIGGER IF EXISTS evidence_source_tombstones_immutable ON evidence_source_tombstones;
...
BEGIN
    IF NEW.token_hash IS DISTINCT FROM encode(public.digest(NULLIF(current_setting('maestro.source_evidence_loss_token', true), ''), 'sha256'), 'hex') OR NEW.owner_id IS DISTINCT FROM NULLIF(current_setting('maestro.source_loss_owner', true), '') OR NEW.fencing_token IS DISTINCT FROM NULLIF(current_setting('maestro.source_loss_fence', true), '')::bigint OR NOT EXISTS (SELECT 1 FROM knowledge_issuer_transaction_markers m WHERE m.transaction_id = txid_current() AND m.issuer_kind = 'source_loss' AND m.token_hash = NEW.token_hash) OR NOT EXISTS (SELECT 1 FROM permanent_roles WHERE status = 'standing' AND ((role_id = NEW.role_id AND role_kind = 'department_head') OR (NEW.role_id = 'ceo' AND role_id = 'concertmaster' AND role_kind = 'concertmaster'))) THEN RAISE EXCEPTION 'source evidence loss authorization must be issued by secured function'; END IF;
  DELETE FROM knowledge_issuer_transaction_markers WHERE transaction_id = txid_current() AND issuer_kind = 'source_loss' AND token_hash = NEW.token_hash;
  RETURN NEW;
END;
$$;
DROP TRIGGER IF EXISTS source_evidence_loss_authorizations_issuer ON source_evidence_loss_authorizations;
CREATE TRIGGER source_evidence_loss_authorizations_issuer BEFORE INSERT ON source_evidence_loss_authorizations FOR EACH ROW EXECUTE FUNCTION authorize_source_evidence_loss_authorization_insert();
...
CREATE TABLE IF NOT EXISTS knowledge_proposal_authorizations (
  ...
  operator_id uuid NOT NULL, role_id text NOT NULL CHECK (btrim(role_id) <> '' AND length(role_id) <= 256), owner_id text NOT NULL CHECK (btrim(owner_id) <> '' AND length(owner_id) <= 256),
...
ALTER TABLE knowledge_proposal_authorizations DROP CONSTRAINT IF EXISTS knowledge_proposal_authorizations_text_safety;
ALTER TABLE knowledge_proposal_authorizations ADD CONSTRAINT knowledge_proposal_authorizations_text_safety CHECK (concat_ws('|', owner_id, role_id, actor_id, session_ref) !~* '(authorization[[:space:]]*:[[:space:]]*bearer|password[[:space:]]*[:=]|secret[[:space:]]*[:=]|api[_-]?key[[:space:]]*[:=]|private[_-]?key|-----BEGIN.*PRIVATE KEY-----|(^|[^a-z])(email|phone|ssn|social security|home address|personal information)([^a-z]|$))');

DROP FUNCTION IF EXISTS authorize_knowledge_proposal(text, uuid, uuid, uuid, uuid, text, text, bigint, text, text);
CREATE OR REPLACE FUNCTION authorize_knowledge_proposal(p_token text, p_knowledge_id uuid, p_project_id uuid, p_goal_id uuid, p_operator_id uuid, p_role_id text, p_owner_id text, p_fencing_token bigint, p_actor_id text, p_session_ref text, p_payload jsonb) RETURNS void LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
  IF concat_ws('|', p_owner_id, p_role_id, p_actor_id, p_session_ref) ~* '(authorization[[:space:]]*:[[:space:]]*bearer|password[[:space:]]*[:=]|secret[[:space:]]*[:=]|api[_-]?key[[:space:]]*[:=]|private[_-]?key|-----BEGIN.*PRIVATE KEY-----|(^|[^a-z])(email|phone|ssn|social security|home address|personal information)([^a-z]|$))' THEN RAISE EXCEPTION 'knowledge proposal authorization contains unsafe text'; END IF;
  ...
  INSERT INTO knowledge_issuer_transaction_markers (transaction_id, issuer_kind, token_hash) VALUES (txid_current(), 'proposal', encode(public.digest(p_token, 'sha256'), 'hex')) ON CONFLICT DO NOTHING;
  INSERT INTO knowledge_proposal_authorizations (revision, token_hash, payload_hash, knowledge_id, project_id, goal_id, operator_id, role_id, owner_id, fencing_token, actor_id, session_ref) VALUES (1, encode(public.digest(p_token, 'sha256'), 'hex'), encode(public.digest(p_payload::text, 'sha256'), 'hex'), p_knowledge_id, p_project_id, p_goal_id, p_operator_id, p_role_id, p_owner_id, p_fencing_token, p_actor_id, p_session_ref);
END;
$$;
CREATE OR REPLACE FUNCTION authorize_knowledge_proposal_authorization_insert() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
  IF NEW.token_hash IS DISTINCT FROM encode(public.digest(NULLIF(current_setting('maestro.knowledge_proposal_token', true), ''), 'sha256'), 'hex')
     OR NEW.operator_id::text IS DISTINCT FROM NULLIF(current_setting('maestro.knowledge_proposal_operator', true), '')
     OR NEW.owner_id IS DISTINCT FROM NULLIF(current_setting('maestro.knowledge_proposal_owner', true), '')
     OR NEW.fencing_token IS DISTINCT FROM NULLIF(current_setting('maestro.knowledge_proposal_fence', true), '')::bigint
     OR NOT EXISTS (SELECT 1 FROM knowledge_issuer_transaction_markers m WHERE m.transaction_id = txid_current() AND m.issuer_kind = 'proposal' AND m.token_hash = NEW.token_hash) THEN
    RAISE EXCEPTION 'knowledge proposal authorization must be issued by secured function';
  END IF;
  DELETE FROM knowledge_issuer_transaction_markers WHERE transaction_id = txid_current() AND issuer_kind = 'proposal' AND token_hash = NEW.token_hash;
  RETURN NEW;
END;
$$;
DROP TRIGGER IF EXISTS knowledge_proposal_authorizations_issuer ON knowledge_proposal_authorizations;
CREATE TRIGGER knowledge_proposal_authorizations_issuer BEFORE INSERT ON knowledge_proposal_authorizations FOR EACH ROW EXECUTE FUNCTION authorize_knowledge_proposal_authorization_insert();
...
REVOKE EXECUTE ON FUNCTION authorize_knowledge_promotion(text, uuid, integer, text, text, text, uuid, uuid, uuid, text, bigint, uuid, text) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION authorize_knowledge_promotion(text, uuid, integer, text, text, text, uuid, uuid, uuid, text, bigint, uuid, text, text) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION authorize_knowledge_promotion(text, uuid, integer, text, text, text, uuid, uuid, uuid, text, bigint, uuid, text, text, text) FROM PUBLIC;
...
BEGIN
  ...
  INSERT INTO knowledge_issuer_transaction_markers (transaction_id, issuer_kind, token_hash) VALUES (txid_current(), 'source_loss', encode(public.digest(p_token, 'sha256'), 'hex')) ON CONFLICT DO NOTHING;
  ...
  IF TG_OP = 'DELETE' AND EXISTS (
    SELECT 1 FROM source_evidence_loss_authorizations a
     WHERE a.evidence_id = OLD.evidence_id AND a.goal_id = OLD.goal_id AND a.project_id = OLD.project_id
       ...
       AND a.fencing_token = NULLIF(current_setting('maestro.source_loss_fence', true), '')::bigint
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    IF NEW.transaction_id <> txid_current() OR NEW.nonce IS DISTINCT FROM NULLIF(current_setting('maestro.schema_cleanup_nonce', true), '')::uuid OR NEW.authorized_by IS DISTINCT FROM session_user OR NOT EXISTS (SELECT 1 FROM knowledge_issuer_transaction_markers m WHERE m.transaction_id = txid_current() AND m.issuer_kind = 'cleanup' AND m.nonce IS NOT DISTINCT FROM NEW.nonce) THEN RAISE EXCEPTION 'schema cleanup authorization must be issued by secured goal reset'; END IF;
    DELETE FROM knowledge_issuer_transaction_markers WHERE transaction_id = txid_current() AND issuer_kind = 'cleanup' AND nonce IS NOT DISTINCT FROM NEW.nonce;
    RETURN NEW;
  END IF;
...
CREATE TRIGGER organizational_knowledge_schema_cleanup_immutable BEFORE INSERT OR UPDATE OR DELETE ON organizational_knowledge_schema_cleanup_authorizations FOR EACH ROW EXECUTE FUNCTION reject_organizational_knowledge_schema_cleanup_mutation();
...
BEGIN
  ...
  INSERT INTO knowledge_issuer_transaction_markers (transaction_id, issuer_kind, nonce) VALUES (txid_current(), 'cleanup', NULLIF(current_setting('maestro.schema_cleanup_nonce', true), '')::uuid) ON CONFLICT DO NOTHING;
...
DROP TRIGGER IF EXISTS organizational_knowledge_digest_loss_authorizations_no_truncate ON organizational_knowledge_digest_loss_authorizations;
CREATE TRIGGER organizational_knowledge_digest_loss_authorizations_no_truncate BEFORE TRUNCATE ON organizational_knowledge_digest_loss_authorizations FOR EACH STATEMENT EXECUTE FUNCTION reject_unscoped_truncate();
...
BEGIN
  ...
  EXECUTE pg_catalog.format('ALTER FUNCTION %I.validate_evidence_source_tombstone_binding() SET search_path = pg_catalog, %I', knowledge_schema, knowledge_schema);
  ...
  EXECUTE pg_catalog.format('ALTER FUNCTION %I.authorize_knowledge_maintenance_authorization_insert() SET search_path = pg_catalog, %I', knowledge_schema, knowledge_schema);
  ...
  EXECUTE pg_catalog.format('ALTER FUNCTION %I.authorize_knowledge_proposal_authorization_insert() SET search_path = pg_catalog, %I', knowledge_schema, knowledge_schema);
