...
import { taskContractContentHash, type DecisionPacket, type IndependentBrief, type TaskContractSubstance } from "@carnegie/domain";
...
describeDatabase("Budget reservations with PostgreSQL", () => {
  ...
  beforeEach(async () => { await pool.query("TRUNCATE goals, budget_forecasts, budget_reservations, council_protocol_events, head_councils, goal_head_participations, task_contracts, evidence_records, goal_leases, outbox, goal_events, command_receipts, goal_controls RESTART IDENTITY CASCADE"); await bootstrapPermanentOrganization(pool); });
