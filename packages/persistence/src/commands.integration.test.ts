...
import { createHeadActivationPlan, type TaskContractSubstance } from "@maestro/domain";
...
describeDatabase("Goal lease fencing with PostgreSQL", () => {
  ...
  beforeEach(async () => {
    await pool.query(
      "TRUNCATE goal_leases, outbox, goal_events, goals, command_receipts, task_contract_confirmations, task_contract_decisions, task_contracts RESTART IDENTITY CASCADE",
    );
  });
  ...
  it("replays project-scoped events strictly after an exact global cursor", async () => {
    ...
    for (const [goalId, id, project] of [
      [firstGoal, randomUUID(), projectId],
      [secondGoal, randomUUID(), projectId],
      [randomUUID(), randomUUID(), otherProjectId],
    ] as const) {
      const proof = await lease(goalId, "reader");
      await executeGoalCommand(
        pool,
        { commandId: id, projectId: project, goalId, actorId: "reader", type: "CreateGoal", expectedVersion: 0 },
        proof,
      );
    }
    ...
    await executeGoalCommand(
      pool,
      { commandId: randomUUID(), projectId, goalId: largeGoal, actorId: "reader", type: "CreateGoal", expectedVersion: 0 },
      proof,
    );
    const all = await listGoalEvents(pool, { projectId, after: "0" });
    expect(all).toHaveLength(3);
    expect(all.map((event) => event.cursor)).toEqual(
      [...all.map((event) => event.cursor)].sort((a, b) => a.length - b.length || a.localeCompare(b)),
    );
    expect(all.at(-1)?.cursor).toBe("9007199254740993");
    expect((await listGoalEvents(pool, { projectId, after: all[0]!.cursor })).map((event) => event.cursor)).toEqual(
      all.slice(1).map((event) => event.cursor),
    );
  ...
  it("gives an expired lease successor a higher exact bigint token", async () => {
    ...
    await pool.query(
      "UPDATE goal_leases SET fencing_token = 9007199254740992, expires_at = transaction_timestamp() - interval '1 millisecond' WHERE goal_id = $1",
      [goalId],
    );
  ...
  it("rejects expired or forged renewal proofs", async () => {
    ...
    await expect(renewGoalLease(pool, { ...proof, fencingToken: "999" }, 60_000)).rejects.toMatchObject({ code: "stale_lease" });
    await pool.query("UPDATE goal_leases SET expires_at = transaction_timestamp() - interval '1 millisecond' WHERE goal_id = $1", [
      proof.goalId,
    ]);
  ...
  it("rolls back a pre-commit injected failure and retries CreateGoal exactly once", async () => {
    ...
    await expect(
      executeGoalCommand(pool, command, proof, {
        beforeCommit: () => {
  ...
  it("claims and acknowledges a Goal outbox command with an owner lease", async () => {
    ...
    const outboxId = (await pool.query<{ outbox_id: string }>("SELECT outbox_id::text FROM outbox ORDER BY outbox_id DESC LIMIT 1"))
      .rows[0]!.outbox_id;
    ...
    await pool.query("UPDATE outbox SET locked_until = transaction_timestamp() - interval '1 millisecond' WHERE outbox_id = $1", [
      claimed[0]!.outboxId,
    ]);
  ...
  it("rejects a wrong Goal, token, or expired proof before command mutation", async () => {
    ...
    await expect(executeGoalCommand(pool, command, { ...proof, goalId: randomUUID() })).rejects.toMatchObject({ code: "stale_lease" });
    await expect(executeGoalCommand(pool, command, { ...proof, fencingToken: "999" })).rejects.toMatchObject({ code: "stale_lease" });
    await pool.query("UPDATE goal_leases SET expires_at = transaction_timestamp() - interval '1 millisecond' WHERE goal_id = $1", [
      proof.goalId,
    ]);
    await expect(executeGoalCommand(pool, command, proof)).rejects.toMatchObject({ code: "stale_lease" });
  ...
  it("creates a Goal only from a launched project-matching Task Contract", async () => {
    ...
    const contractSubstance: TaskContractSubstance = {
      ...
      headActivationPlan: createHeadActivationPlan({
        version: 1,
        departments: [
          {
            departmentId: "product",
            requestedContribution: "define boundary",
            urgency: "normal",
            contextScope: ["contract"],
            budgetEffect: "none",
            reason: "explicit launch brief",
          },
        ],
      }),
    };
    ...
    await expect(executeGoalCommand(pool, command, proof)).resolves.toMatchObject({
      outcome: "rejected",
      code: "task_contract_not_launched",
    });
    ...
    expect((await pool.query("SELECT task_contract_id FROM goals WHERE goal_id = $1", [goalId])).rows[0]!.task_contract_id).toBe(
      contractId,
    );
    await pool.query("UPDATE outbox SET payload = $2::jsonb WHERE event_id = $1", [
      created.eventId,
      JSON.stringify({
    ...
    await expect(validateStartGoalOrchestrationCommand(pool, claimed[0]!)).resolves.toMatchObject({
      ...
      headActivationPlan: contract.headActivationPlan,
    });
    ...
    await expect(validateStartGoalOrchestrationCommand(pool, { ...claimed[0]!, payload: tamperedPayload })).rejects.toBeInstanceOf(
      GoalOrchestrationBindingError,
    );
    ...
    await expect(
      executeGoalCommand(pool, { ...command, commandId: randomUUID(), goalId: mismatchGoalId, projectId: randomUUID() }, mismatchProof),
    ).resolves.toMatchObject({ outcome: "rejected", code: "task_contract_project_mismatch" });
  });
  ...
  it("couples generic lifecycle transitions to durable Goal control latches and makes emergency stop terminal", async () => {
    ...
    const command = async (to: import("@maestro/domain").GoalState, expectedVersion: number) =>
      executeGoalCommand(
        pool,
        { commandId: randomUUID(), projectId, goalId, actorId: "operator", type: "TransitionGoal", expectedVersion, to },
        proof,
      );
    ...
    await expect(
      pool.query<{ control_epoch: string; pause_requested_at: Date | null; paused_at: Date | null }>(
        "SELECT control_epoch, pause_requested_at, paused_at FROM goal_controls WHERE project_id = $1 AND goal_id = $2",
        [projectId, goalId],
      ),
    ...
    await expect(
      pool.query<{ stopping_at: Date | null; stopped_at: Date | null }>(
        "SELECT stopping_at, stopped_at FROM goal_controls WHERE project_id = $1 AND goal_id = $2",
        [projectId, goalId],
      ),
    ...
    const emergencyCommand = {
      ...
      actorId: "operator",
      type: "EmergencyStopGoal" as const,
      expectedVersion: 1,
    };
    await executeGoalCommand(
      pool,
      { commandId: randomUUID(), projectId, goalId: emergencyGoalId, actorId: "operator", type: "CreateGoal", expectedVersion: 0 },
      emergencyProof,
    );
    ...
    await expect(
      pool.query<{ state: string; control_epoch: string; emergency_stopped_at: Date | null; stopped_at: Date | null }>(
        "SELECT g.state, c.control_epoch, c.emergency_stopped_at, c.stopped_at FROM goals g JOIN goal_controls c USING (project_id, goal_id) WHERE g.project_id = $1 AND g.goal_id = $2",
        [projectId, emergencyGoalId],
      ),
    ).resolves.toMatchObject({
      rows: [{ state: "stopped", control_epoch: "2", emergency_stopped_at: expect.any(Date), stopped_at: expect.any(Date) }],
    });
  });
});
