...
describe("routing evidence certification lineage", () => {
  ...
  it("blocks a claim from an unrelated native admission", () => {
    ...
    const claim = { claim_id: "claim-1", approval_id: "approval-1", capability_kind: "ipython", project_id: "project-1", goal_id: "goal-1", command_id: "command-1", action: "project.file.edit", target: "src/server.ts", policy_version: 1, budget_effect_cents: 0, consumed_at: "2026-09-08T12:00:00Z", admission_command_id: "different-admission" };
  ...
  it("blocks a claim after its bounded repetition window", () => {
    ...
    const claim = { claim_id: "claim-1", approval_id: "approval-1", capability_kind: "ipython", project_id: "project-1", goal_id: "goal-1", command_id: "command-1", action: "project.file.edit", target: "src/server.ts", policy_version: 1, budget_effect_cents: 0, consumed_at: "2026-09-08T12:00:00Z", admission_command_id: "command-1" };
    expect(evaluateRoutingEvidenceLineage({ goalId: "goal-1", projectId: "project-1", routingRows: [row(value)], nativeBindings: [binding], approvals: [approval], claims: [claim] }).blockers).toContainEqual(expect.objectContaining({ reason: "routing_evidence_unapproved_below_requirement" }));
  });
  ...
  it("checks approval expiry and revocation at capability claim time", () => {
    ...
    const claim = { claim_id: "claim-1", approval_id: "approval-1", capability_kind: "ipython", project_id: "project-1", goal_id: "goal-1", command_id: "command-1", action: "project.file.edit", target: "src/server.ts", policy_version: 1, budget_effect_cents: 0, consumed_at: "2026-09-08T13:00:00Z" };
    expect(evaluateRoutingEvidenceLineage({ goalId: "goal-1", projectId: "project-1", routingRows: [row(value)], nativeBindings: [binding], approvals: [approval], claims: [claim] }).blockers).toContainEqual(expect.objectContaining({ reason: "routing_evidence_unapproved_below_requirement" }));
