...
describeDatabase("Ensemble Router artifacts with PostgreSQL", () => {
  ...
  const routingInputs = (goalRef: string) => ({
    ...
    operationalOverlaySnapshot: { schemaVersion: 1, installationRef: "installation-1", projectRef: "project-1", goalRef, overlayVersion: 1, observations: [{ candidateRef: "candidate-1", measuredLatencyMs: 10, measuredCost: 1, failureRate: 0, timeoutRate: 0, providerErrorRate: 0, currentAvailability: true, accountBinding: "account-1", observedAt: "2026-09-08T12:00:00Z" }] }, approvalRef: null,
  });
