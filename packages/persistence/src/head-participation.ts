...
export interface ActivateHeadRequest {
  ...
  /** Stable permanent Head identity. The database verifies its Department binding. */
...
export class HeadActivationBindingConflictError extends Error {
  ...
  constructor(message = "HeadRoleId is not bound to the requested Department") {
    super(message); this.name = "HeadActivationBindingConflictError";
  }
}
...
export async function activateHeadParticipation(pool: Pool, request: ActivateHeadRequest, proof: GoalLeaseProof): Promise<GoalHeadParticipation> {
  ...
  if (request.departmentId.trim() === "" || request.reason.trim() === "") throw new RangeError("departmentId and reason are required");
  if (request.headRoleId !== undefined && request.headRoleId.trim() === "") throw new RangeError("headRoleId must be a non-empty string");
  const requester: HeadRequester = request.requester ?? { role: "Sane" };
  ...
  try {
    ...
    // Goal-scoped participation and role-scoped runtime bindings use one lock order.
    ...
    const existing = await client.query<ParticipationRow>(
      `SELECT goal_id, department_id, head_role_id, contract_id, status, active_session_ref
    ...
    if (active !== undefined && active.head_role_id !== targetHeadRoleId && request.headRoleId !== undefined) {
    ...
    const current = request.headRoleId !== undefined
      ? existing.rows.find((candidate) => candidate.head_role_id === targetHeadRoleId)
      : active ?? existing.rows.find((candidate) => candidate.head_role_id === targetHeadRoleId);
    ...
    const cycle = requester.role === "Head" && (
      ...
      await wouldCreateCycle(client, request.goalId, requesterHeadRoleId!, targetHeadRoleId)
    );
    if (cycle) {
    ...
    if (current?.status === "active") {
      row = current; outcome = "already_active";
      ...
      const updated = await client.query<ParticipationRow>(
        `UPDATE goal_head_participations SET status = 'starting', active_session_ref = NULL,
         ...
         RETURNING goal_id, department_id, head_role_id, contract_id, status, active_session_ref`,
        [request.goalId, targetHeadRoleId, request.contractId ?? null],
      );
      row = updated.rows[0]!; outcome = "reserved";
    } else {
      const inserted = await client.query<ParticipationRow>(
        `INSERT INTO goal_head_participations (goal_id, department_id, head_role_id, contract_id, status, active_session_ref)
         VALUES ($1, $2, $3, $4::uuid, 'starting', NULL)
         RETURNING goal_id, department_id, head_role_id, contract_id, status, active_session_ref`,
        [request.goalId, request.departmentId, targetHeadRoleId, request.contractId ?? null],
      );
...
export async function markHeadParticipationActive(
  ...
  if (goalId !== proof.goalId || !validProof(proof) || activeSessionRef.trim() === "") throw new StaleGoalLeaseError(goalId);
  return mutateParticipation(
    pool, goalId, departmentId, proof,
    `status = 'active', active_session_ref = $3, updated_at = transaction_timestamp()`,
    [activeSessionRef], "starting", headRoleId, activeSessionRef,
  );
}
...
async function mutateParticipation(
  ...
  try {
    ...
    if (current.rowCount !== 1) {
      if (requestedHeadRoleId !== undefined) throw new HeadActivationBindingConflictError("HeadRoleId is not bound to the requested Goal participation");
...
async function wouldCreateCycle(client: PoolClient, goalId: string, requesterHeadRoleId: string, targetHeadRoleId: string): Promise<boolean> {
  const result = await client.query(
    ...
    [goalId, requesterHeadRoleId, targetHeadRoleId],
  );
...
async function insertAttempt(
  ...
  outcome: "reserved" | "already_active" | "cycle_rejected" | "runtime_conflict" | "binding_conflict",
): Promise<void> {
  await client.query(
    ...
    [
      randomUUID(), request.goalId, request.departmentId, headRoleId,
