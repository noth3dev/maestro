...
export const COLUMNS = `candidate_id, lineage_id, version, parent_candidate_id, schema_version, project_id, goal_id, kind,
...
export function line(value: unknown, field: string, maximum = 256): asserts value is string {
  try {
...
export function authorValue(author: ImprovementCandidateAuthor): void {
  ...
  if (!UUID.test(author.operatorId))
    throw new ImprovementCandidatePersistenceError("Improvement Candidate operatorId must be a durable UUID");
}

export function normalizeInput(input: ImprovementCandidateInput): ImprovementCandidateInput {
  try {
...
export function operationRef(kind: "record" | "append" | "transition" | "evaluation", idempotencyKey: string): string {
...
export function inputFromRow(row: CandidateRow): ImprovementCandidateInput {
  return {
    schemaVersion: 1,
...
export function mapRow(row: CandidateRow): ImprovementCandidate {
  try {
    ...
    if (candidate.contentHash !== row.content_hash.trim())
      throw new ImprovementCandidatePersistenceError("Stored Improvement Candidate content hash does not match its input");
    if (candidate.contentHash.length !== 64)
      throw new ImprovementCandidatePersistenceError("Stored Improvement Candidate content hash is invalid");
    return candidate;
  } catch (error) {
...
export async function assertReadAccess(pool: Pool, row: CandidateRow, authorization: ImprovementCandidateReadAuthorization): Promise<void> {
  line(authorization?.operatorId, "Improvement Candidate read operatorId");
  if (!UUID.test(authorization.operatorId))
    throw new ImprovementCandidatePersistenceError("Improvement Candidate read operatorId must be a durable UUID");
  if (
    authorization.proof?.goalId?.toLowerCase() !== row.goal_id ||
    authorization.proof.ownerId === "" ||
    !/^\d+$/.test(authorization.proof.fencingToken)
  ) {
  ...
  if (result.rowCount !== 1)
    throw new ImprovementCandidatePersistenceError("Improvement Candidate read is not authorized for this project and Goal");
}

export async function readRow(client: Pick<Pool | PoolClient, "query">, candidateId: string, forUpdate = false): Promise<CandidateRow> {
  ...
  const result = await client.query<CandidateRow>(
    `SELECT ${COLUMNS} FROM improvement_candidates WHERE candidate_id = $1${forUpdate ? " FOR UPDATE" : ""}`,
...
export function payload(input: ImprovementCandidateInput): Record<string, unknown> {
  return {
...
export function assertIdempotentReplay(
  ...
  if (
    row.content_hash.trim() !== improvementCandidateContentHash(input) ||
...
export async function insertCandidate(
  ...
  if (canonicalHash === undefined || canonicalHash.trim() !== candidate.contentHash)
    throw new ImprovementCandidatePersistenceError("Database candidate content hash does not match the canonical input");
  const inserted = await client.query<CandidateRow>(
    ...
    [
      ...
      input.predictedEffect,
      JSON.stringify(input.expectedMetrics),
      JSON.stringify(input.protectedMetrics),
      JSON.stringify(input.scenarioSuite),
      input.scenarioSuiteHash,
      input.confidence,
      JSON.stringify(input.dataSufficiency),
      JSON.stringify(input.rollbackTarget),
      candidate.state,
      candidate.contentHash,
      candidate.authorId,
      author.operatorId,
...
export function ensureProofMatches(input: ImprovementCandidateInput, proof: GoalLeaseProof): void {
  if (input.goalId !== proof.goalId.toLowerCase())
    throw new ImprovementCandidatePersistenceError("Improvement Candidate Goal does not match its lease proof");
}

export async function appendFromPrevious(
  client: PoolClient,
  ...
  if (input.projectId !== previous.project_id || input.goalId !== previous.goal_id)
    throw new ImprovementCandidatePersistenceError("Improvement Candidate revision cannot change project or Goal");
  if (input.kind !== previous.kind)
    throw new ImprovementCandidatePersistenceError("Improvement Candidate revision cannot change candidate kind");
  if (input.target.roleId !== previous.target.roleId || input.target.taskClass !== previous.target.taskClass)
    throw new ImprovementCandidatePersistenceError("Improvement Candidate revision cannot change target role or task class");
  const candidateId = randomUUID();
  return insertCandidate(client, input, proof, author, operation, {
    candidateId,
    lineageId: previous.lineage_id,
    version: previous.version + 1,
    parentCandidateId: previous.candidate_id,
    state,
  });
}
