...
describeDatabase("Metronome challenges with PostgreSQL", () => {
  ...
  beforeEach(async () => { await pool.query("TRUNCATE goals, metronome_challenge_findings, metronome_challenges, metronome_findings, workers, mission_bundles, department_plan_revisions, department_plans, council_protocol_events, head_councils, goal_head_participations, task_contracts, evidence_records, goal_leases, outbox, goal_events, command_receipts, goal_controls RESTART IDENTITY CASCADE"); await bootstrapPermanentOrganization(pool); });
