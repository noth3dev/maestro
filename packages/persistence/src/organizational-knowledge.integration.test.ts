...
describeDatabase("organizational knowledge persistence", () => {
  ...
  it("requires Council approval and multiple episodes, then exposes global knowledge across projects", async () => {
    ...
    await promoteOrganizationalKnowledgeToProject(pool, { knowledgeId: proposed.knowledgeId, promoterRoleId: "head-engineering", departmentId: "engineering" });
    await expect(promoteOrganizationalKnowledgeToGlobal(pool, { knowledgeId: proposed.knowledgeId, promoterRoleId: "head-engineering", departmentId: "engineering", encoreCouncilRoundId: randomUUID(), corroboratingSourceIds: [evidenceId, evidenceId2], generalizedStatement: "Use bounded validation gates.", curatorRoleId: "head-security", noRawProjectContent: true, noPersonalInformation: true })).rejects.toThrow(/Council|approval/);
    ...
    await expect(promoteOrganizationalKnowledgeToGlobal(pool, { knowledgeId: proposed.knowledgeId, promoterRoleId: "head-engineering", departmentId: "engineering", encoreCouncilRoundId: wrongRoundId, corroboratingSourceIds: [evidenceId, evidenceId2], generalizedStatement: "Use bounded validation gates.", curatorRoleId: "head-security", noRawProjectContent: true, noPersonalInformation: true })).rejects.toThrow(/Council|approval|independent|Goal/);
    ...
    const global = await promoteOrganizationalKnowledgeToGlobal(pool, { knowledgeId: proposed.knowledgeId, promoterRoleId: "head-engineering", departmentId: "engineering", encoreCouncilRoundId: roundId, corroboratingSourceIds: [evidenceId, evidenceId2], generalizedStatement: "Use bounded validation gates.", curatorRoleId: "head-security", noRawProjectContent: true, noPersonalInformation: true });
