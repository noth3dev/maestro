...
describeDatabase("organizational knowledge persistence", () => {
  ...
  it("persists worker proposals separately and only exposes promoted knowledge", async () => {
    ...
    await expect(pool.query(`INSERT INTO organizational_knowledge
      ...
      VALUES ($1, 1, 1, $2, $2, $3, 'engineering', 'project_department', 'active', 'raw lesson', 'raw rationale', $4::jsonb, '[]', '["episode-a"]', 0.5, 1, false, NULL, NULL, NULL, 'department-promotion', NULL, 'worker', 'session:worker')`, [randomUUID(), projectId, goalId, JSON.stringify([evidenceId, evidenceId2])])).rejects.toThrow();
  ...
  it("does not expose a project lesson in another project", async () => {
    ...
    await grantProjectMembership(pool, operatorId, projectId);
    await expect(listOrganizationalKnowledge(pool, { operatorId, projectId, departmentId: "design" })).resolves.toEqual([]);
  });
  ...
  it("requires Council approval and multiple episodes, then exposes global knowledge across projects", async () => {
    ...
    await promoteOrganizationalKnowledgeToProject(pool, { knowledgeId: proposed.knowledgeId, promoterRoleId: "head-engineering", departmentId: "engineering" });
    await expect(promoteOrganizationalKnowledgeToGlobal(pool, { knowledgeId: proposed.knowledgeId, promoterRoleId: "head-engineering", departmentId: "engineering", encoreCouncilRoundId: randomUUID(), corroboratingSourceIds: [evidenceId, evidenceId2], corroboratingEpisodeIds: ["episode-a", "episode-b"], generalizedStatement: "Use bounded validation gates.", curatorRoleId: "head-security", noRawProjectContent: true, noPersonalInformation: true })).rejects.toThrow(/Council|approval/);
    ...
    await pool.query("INSERT INTO encore_council_rounds (round_id, goal_id, question, criteria, evidence_ids, trigger_reasons, reviewer_count) VALUES ($1, $2, 'wrong goal', '[]', $3::jsonb, '[]', 2)", [wrongRoundId, wrongGoalId, JSON.stringify([evidenceId, evidenceId2])]);
    ...
    await expect(promoteOrganizationalKnowledgeToGlobal(pool, { knowledgeId: proposed.knowledgeId, promoterRoleId: "head-engineering", departmentId: "engineering", encoreCouncilRoundId: wrongRoundId, corroboratingSourceIds: [evidenceId, evidenceId2], corroboratingEpisodeIds: ["episode-a", "episode-b"], generalizedStatement: "Use bounded validation gates.", curatorRoleId: "head-security", noRawProjectContent: true, noPersonalInformation: true })).rejects.toThrow(/Council|approval|independent|Goal/);
    const roundId = randomUUID();
    await pool.query("INSERT INTO encore_council_rounds (round_id, goal_id, question, criteria, evidence_ids, trigger_reasons, reviewer_count) VALUES ($1, $2, 'approve knowledge', '[]', $3::jsonb, '[]', 2)", [roundId, goalId, JSON.stringify([evidenceId, evidenceId2])]);
    ...
    const global = await promoteOrganizationalKnowledgeToGlobal(pool, { knowledgeId: proposed.knowledgeId, promoterRoleId: "head-engineering", departmentId: "engineering", encoreCouncilRoundId: roundId, corroboratingSourceIds: [evidenceId, evidenceId2], corroboratingEpisodeIds: ["episode-a", "episode-b"], generalizedStatement: "Use bounded validation gates.", curatorRoleId: "head-security", noRawProjectContent: true, noPersonalInformation: true });
  ...
  it("persists stale and contradicted decay as auditable revisions", async () => {
    ...
    const proposed = await proposeOrganizationalKnowledge(pool, proposal(), proof, { actorId: "worker", sessionRef: "session:worker" });
    const promoted = await promoteOrganizationalKnowledgeToProject(pool, { knowledgeId: proposed.knowledgeId, promoterRoleId: "head-engineering", departmentId: "engineering" });
    await expect(refreshOrganizationalKnowledge(pool, { knowledgeId: promoted.knowledgeId, now: "2026-09-20T00:00:00.000Z", lastSupportedAt: "2026-08-01T00:00:00.000Z", staleAfterMs: 86_400_000, actorId: "worker" })).rejects.toThrow(/restricted|system/);
    const stale = await refreshOrganizationalKnowledge(pool, { knowledgeId: promoted.knowledgeId, now: "2026-09-20T00:00:00.000Z", lastSupportedAt: "2026-08-01T00:00:00.000Z", staleAfterMs: 86_400_000, actorId: "knowledge-decay-system" });
  ...
  it("marks source-evidence loss unsupported and retires without deleting provenance", async () => {
    ...
    const proposed = await proposeOrganizationalKnowledge(pool, proposal(), proof, { actorId: "worker", sessionRef: "session:worker" });
    const promoted = await promoteOrganizationalKnowledgeToProject(pool, { knowledgeId: proposed.knowledgeId, promoterRoleId: "head-engineering", departmentId: "engineering" });
    await deleteEvidenceSource(pool, evidenceId, "source artifact was deleted", "evidence-operator");
