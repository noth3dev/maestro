...
describeDatabase("organizational knowledge persistence", () => {
  ...
  beforeEach(async () => {
    ...
    await pool.query("INSERT INTO evidence_records (evidence_id, correlation_id, command_id, project_id, goal_id, actor_id, sha256, byte_length, kind, media_type, retention) VALUES ($1, $2, $3, $4, $5, 'worker', $6, 10, 'lesson', 'text/plain', '30_days_after_goal_close')", [evidenceId, randomUUID(), randomUUID(), projectId, goalId, "a".repeat(64)]);
  ...
  it("persists worker proposals separately and only exposes promoted knowledge", async () => {
    ...
    await expect(pool.query(`INSERT INTO organizational_knowledge
      ...
      VALUES ($1, 2, 1, $2, $2, $3, 'engineering', 'worker_proposed', 'unsupported', 'Use a bounded validation gate.', 'It prevented recurrence.', $4::jsonb, '[]', '["episode-a"]', 0.5, 1, false, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 'adjudication', 'forged', 'head-engineering', 'forged-maintenance')`, [proposed.knowledgeId, projectId, goalId, JSON.stringify([evidenceId, evidenceId2])])).rejects.toThrow(/authorization|maintenance/);
    ...
    await expect(pool.query(`INSERT INTO organizational_knowledge (knowledge_id, revision, schema_version, source_project_id, project_id, source_goal_id, department_id, scope, status, statement, rationale, source_evidence_ids, source_digest_ids, episode_ids, confidence, freshness, generalized, promotion_marker, reason, created_by, source_session_ref) VALUES ($1, 1, 1, $2, $2, $3, 'worker_proposed', 'worker_proposed', 'unsupported', 'missing', 'missing', $4::jsonb, '[]', '["episode-a"]', 0.5, 1, false, 'source-loss', 'missing', 'worker', 'evidence:' || $5)`, [randomUUID(), projectId, goalId, JSON.stringify([randomUUID()]), evidenceId])).rejects.toThrow(/evidence|authorization|marker|source-loss/);
  ...
  it("marks source-evidence loss unsupported and retires without deleting provenance", async () => {
    ...
    await expect(pool.query("SELECT retention::text FROM source_evidence_loss_events WHERE evidence_id = $1", [evidenceId])).resolves.toMatchObject({ rows: [{ retention: "30_days_after_goal_close" }] });
    ...
    await expect(pool.query("SELECT retention::text FROM evidence_source_tombstones WHERE evidence_id = $1", [evidenceId])).resolves.toMatchObject({ rows: [{ retention: "30_days_after_goal_close" }] });
