...
interface KnowledgeRow {
  ...
  council_round_id: string | null; generalized_statement: string | null; curator_role_id: string | null; curator_operator_id: string | null; promotion_marker: string; reason: string | null; created_by: string; source_session_ref: string; created_at: Date; retention?: string;
}
const COLUMNS = "knowledge_id, revision, schema_version, source_project_id, project_id, source_goal_id, department_id, scope, status, statement, rationale, source_evidence_ids, source_digest_ids, episode_ids, confidence, freshness, generalized, council_round_id, generalized_statement, curator_role_id, curator_operator_id, promotion_marker, reason, created_by, source_session_ref, created_at, retention";
...
async function insertRevision(client: Pick<PoolClient, "query">, lesson: OrganizationalKnowledge, createdBy: string, sourceSessionRef: string): Promise<OrganizationalKnowledge> {
  ...
  const result = await client.query<KnowledgeRow>(
    ...
    [lesson.knowledgeId, lesson.revision, lesson.schemaVersion, lesson.sourceProjectId, lesson.projectId, lesson.sourceGoalId, lesson.departmentId, lesson.scope, lesson.status, lesson.statement, lesson.rationale, JSON.stringify(lesson.sourceEvidenceIds), JSON.stringify(lesson.sourceDigestIds), JSON.stringify(lesson.episodeIds), lesson.confidence, lesson.freshness, lesson.generalized, lesson.councilRoundId, lesson.generalizedStatement ?? null, lesson.curatorRoleId ?? null, lesson.curatorOperatorId ?? null, marker(lesson), lesson.reason, createdBy, sourceSessionRef, retention],
  );
...
export async function proposeOrganizationalKnowledge(pool: Pool, input: OrganizationalKnowledgeProposal, proof: GoalLeaseProof, author: OrganizationalKnowledgeAuthor, idempotencyKey?: string): Promise<OrganizationalKnowledge> {
  ...
  return withGoalAuthority(pool, proof, 63, async (client) => {
    ...
    if (idempotencyKey !== undefined && idempotencyKey.trim() === "") throw new OrganizationalKnowledgeError("proposal idempotency key is invalid");
    const operationRef = idempotencyKey === undefined ? author.sessionRef.trim() : `proposal:${idempotencyKey.trim()}`;
    ...
    return insertRevision(client, proposal, author.actorId.trim(), operationRef);
  });
}
...
export async function promoteOrganizationalKnowledgeToProject(pool: Pool, request: { readonly knowledgeId: string; readonly promoterRoleId: string; readonly promoterOperatorId: string; readonly departmentId: string; readonly proof: GoalLeaseProof; readonly idempotencyKey?: string }): Promise<OrganizationalKnowledge> {
  return withGoalAuthority(pool, request.proof, 91, async (client) => {
    ...
    if (request.idempotencyKey !== undefined && request.idempotencyKey.trim() === "") throw new OrganizationalKnowledgeError("promotion idempotency key is invalid");
    const operationRef = request.idempotencyKey === undefined ? `promotion:${request.promoterRoleId}` : `promotion:${request.idempotencyKey.trim()}`;
    ...
    return insertRevision(client, promoted, request.promoterRoleId, operationRef);
  });
}
...
export async function promoteOrganizationalKnowledgeToGlobal(pool: Pool, request: { readonly knowledgeId: string; readonly promoterRoleId: string; readonly promoterOperatorId: string; readonly departmentId: string; readonly proof: GoalLeaseProof; readonly encoreCouncilRoundId: string; readonly corroboratingSourceIds: readonly string[]; readonly corroboratingEpisodeIds: readonly string[]; readonly generalizedStatement: string; readonly curatorRoleId: string; readonly curatorOperatorId: string; readonly idempotencyKey?: string }): Promise<OrganizationalKnowledgePublic> {
  return withGoalAuthority(pool, request.proof, 92, async (client) => {
    ...
    const stored = await insertRevision(client, promoted, request.promoterRoleId, operationRef);
...
export async function refreshOrganizationalKnowledge(pool: Pool, request: { readonly knowledgeId: string; readonly now: string; readonly lastSupportedAt: string; readonly staleAfterMs?: number; readonly contradicted?: boolean; readonly proof: GoalLeaseProof; readonly operatorId: string; readonly idempotencyKey: string }): Promise<OrganizationalKnowledge | OrganizationalKnowledgePublic> {
  return withGoalAuthority(pool, request.proof, 89, async (client) => {
    ...
    const operationRef = `system:knowledge-decay:${request.idempotencyKey}`;
    if (current.sourceSessionRef === operationRef) return current.scope === "global" ? publicProjection(current) : current;
    ...
    const stored = await insertRevision(client, maintained, maintenanceAuthor, operationRef);
...
export async function retireOrganizationalKnowledge(pool: Pool, request: { readonly knowledgeId: string; readonly reason: string; readonly retiredBy: string; readonly proof: GoalLeaseProof; readonly operatorId: string }): Promise<OrganizationalKnowledge | OrganizationalKnowledgePublic> {
  return withGoalAuthority(pool, request.proof, 90, async (client) => {
    ...
    const stored = await insertRevision(client, retired, request.retiredBy, `retirement:${request.retiredBy}`);
    return stored.scope === "global" ? publicProjection(stored) : stored;
  });
}
