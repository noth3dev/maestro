...
describeDatabase("learned persona profile persistence", () => {
  ...
  beforeEach(async () => { await pool.query("TRUNCATE persona_task_class_adjustments, persona_profile_versions"); });
  ...
  it("versions learned profiles and retains each version's rationale", async () => {
    await storeLearnedPersonaProfileVersion(pool, profile(1));
  ...
  it("does not expose adaptive writes for permanent role core identity", async () => {
    ...
    await expect(pool.query("SELECT role_charter, capability_boundary, provenance FROM permanent_roles WHERE role_id = 'concertmaster'")).resolves.toMatchObject({ rows: [{ role_charter: expect.any(String), capability_boundary: expect.anything(), provenance: expect.anything() }] });
  });
});
