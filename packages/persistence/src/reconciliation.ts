...
export async function reconcileOnStartup(
  ...
  for (const row of nonterminalGoals) {
    ...
    if (row.state === "recovering") {
      ...
      results.push({ goalId: row.goal_id, projectId: row.project_id, priorState: row.state, outcome: "recovering", reasons, reconciledWorkerIds, reconciledHeadActivationCommandIds });
      continue;
    }
