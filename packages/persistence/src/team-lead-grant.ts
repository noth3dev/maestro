...
export async function spawnHelperWorker(pool: Pool, kernel: ExecutionKernelPort, grantId: string, proof: GoalLeaseProof, context: CouncilActorContext): Promise<Worker> {
  ...
  try {
    ...
    if (teamLead.rows[0]!.status === "unknown") throw new TeamLeadGrantError("Cannot spawn a helper from an unknown team-lead worker");
    ...
    const nextHelperNumber = priorHelpers.rows.length + 1;
    ...
    await client.query(
      `INSERT INTO workers (worker_id, council_id, department_id, plan_version, item_id, bundle_content_hash, attempt, execution_ref, invocation_ref, status, parent_worker_id, grant_id, owner_id, owner_fencing_token, owner_lease_expires_at, heartbeat_at, recovery_state)
       SELECT $1, council_id, department_id, plan_version, item_id, bundle_content_hash, $2, $3, $4, 'spawned', $5, $6, $7, $8::bigint, $9, transaction_timestamp(), 'none'
       FROM workers w1 WHERE worker_id = $5`,
      [workerId, nextHelperNumber, pendingExecutionRef, pendingInvocationRef, grant.worker_id, grantId, proof.ownerId, proof.fencingToken, ownerLeaseExpiresAt],
    );
    await client.query("COMMIT"); open = false;
  } catch (error) { if (open) await client.query("ROLLBACK"); throw error; } finally { client.release(); }
  ...
  try {
    return await bindWorkerInvocation(pool, workerId, spawned, proof);
  } catch (error) {
    // Compensation holds the same Goal/worker owner claim through provider
    // cancellation. A stale owner cannot cancel after takeover.
