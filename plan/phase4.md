# Phase 4 — Isolated Environments, Enrolled Devices, and Firefly Incidents

## Outcome

Extend the certified single-Goal system beyond its local disposable project. Workers gain reproducible task environments and narrowly enrolled device access. Firefly operates outside Maestro's primary failure domain and converts verified anomalies into bounded incident workflows.

No tool installation, device enrollment, or Firefly signal grants action authority by itself.
...
4. For each Goal, Sane requests the smallest grant: action types, project paths, applications, time, data, and network scope.
...
Firefly is a separate Node process with its own minimal configuration, health, credential, and append-only local buffer. It must continue detecting and reporting when Maestro's main control plane is unhealthy.
...
Firefly never patches, deploys, changes permissions, or spawns production workers. It emits authenticated, freshness-checked signals with:
...
10. Close with resolution, retained risk, false-positive result, and Firefly feedback.
...
- Firefly delivery uses an authenticated incident endpoint plus durable local retry buffer.
- Maestro accepts no raw Firefly command request; only evidence signals.
...
6. Implement Firefly process, health, buffer, signal schemas, authentication, and replay defense.
...
- Firefly sends the same signal repeatedly: update one incident, do not create duplicate Goals.
- Firefly is silent: report watchdog-health uncertainty, never infer no incidents.
- Firefly itself is compromised or stale: reject signal before organizational activation.
...
7. Firefly continues buffering while Maestro is stopped and delivers once healthy.
...
15. Firefly false positive becomes improvement evidence.
