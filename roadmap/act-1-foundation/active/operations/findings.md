...
- 2026-09-10 S2 review blockers found and fixed: arbitrary approval IDs in the first PostgreSQL fixture violated the journal approval FK; the fixture now creates a real approval and pending effect. The original `0071` migration constraint rejected the new rule; it was restored and moved to additive `0077`. Routing challenge evidence initially used `evidenceReferences: []`; routing evidence IDs are now validated as durable Goal-scoped references. Production stopped-boundary journaling initially collapsed runtime `error`/`unknown` results into `interruption`; stage outcome mapping and runtime boundary classification now fail closed.
- 2026-09-10 S2 integrity hardening: Metronome scanning initially bypassed canonical routing evidence validation and masked row/payload identity. It now uses `listRoutingEvidenceForGoal`, which validates the full durable payload, checks row identity, and preserves legacy `rejections` defaults. PostgreSQL regressions cover minimally-scored malformed evidence and route-row identity mismatch. The existing HTTP scan response remains findings-only by deliberate compatibility scope; loop `onTick`/`onApprovalObservation` is the complete observation surface.

- 2026-09-10 S2 independent review found a cross-project routing row gap after canonical payload/row validation: the routing table has no Goal/project FK. Added an explicit Goal project-scope comparison and a real PostgreSQL regression; the findings/challenge path remains fail-closed.

- 2026-09-10 S3 independent review found P1: `runEncoreCouncilReview` fell back to `kernel.getModelIdentity` when admission or `getExecutionBinding` was absent, allowing reviewer identity records without durable native evidence. Remediation fails closed before provider fan-out unless host-owned admission and the durable binding capability exist; successful reviewer IDs are read only from `native_execution_bindings`. The native binding conflict lookup also now passes only typed parameters actually used by its query, and the test fixture uses unique execution refs across rounds. Focused PostgreSQL (22/22), full no-DB (984/984, 417 skipped), and full PostgreSQL (1411/1411) passed; final independent review is PASS.

- 2026-09-10 Plan 3 S3b independent review blocker: the first implementation rendered conversation and activity as separate suffixes, left durable pending effects out of the decisions region, and placed status/decisions inside the scrollable viewport. It also allowed `user` tier text and a success glyph on error-coloured effects. Remediation adds cursor-ordered event history, replay of durable activity before reconnect, fixed-region components, pending-decision derivation, semantic glyph precedence, and working-turn/escape interaction. Re-review is required before merge.
- 2026-09-10 The repository root contains the user-owned untracked `testbed/` workspace. S3b validation is isolated to `.worktrees/tui-design-pass`; `testbed/` is excluded from all reads and writes.

- 2026-09-10 S3b PostgreSQL gate: the first full run used a pre-fix snapshot and failed one existing `tui-sse-reconnect.integration.test.ts` assertion expecting `goalId · draft`; the focused PostgreSQL rerun passed 2/2 after preserving that separator. A fresh full PostgreSQL run is required before merge.
- 2026-09-10 S3b review hardening: conversation and Goal cursors are independent sequences, so cross-stream chronology now uses durable `occurredAt`; activity replay/stream callbacks reject stale project generations; pending correlation uses `effectId`/command-index identity when present; streamed deltas collapse into one answer; and replayed decisions expose Ctrl+A detail review.

- 2026-09-10 S3b independent review failure: pending decisions used actor/action/target as a fallback identity, allowing concurrent identical effects to collapse or the wrong completion to clear a pending item; tier labels were inferred from outcome/classification. This was reproduced by RED tests.
- 2026-09-10 S3b remediation: activity parsing now accepts durable `effectId`/identity or `commandId:index` with numeric indexes, recognizes explicit tier fields, and skips pending decisions lacking either identity or tier. No provider or testbed surface was touched. Focused 40/40 and full no-DB 142/205-file, 1005/1422-test verification passed; fresh independent review remains required.

- 2026-09-10 S3b second review blocker: `entry.ts:165-168` placed an in-memory `pendingConfirmation` row into the same pending-decision list without identity and used `You` as a tier fallback. This violated universal fail-closed pending-row handling even though durable replay was corrected.
- 2026-09-10 S3b second remediation: identityless pending rows are filtered at the TUI boundary; durable activity rows carry their correlation key; critical confirmation summaries carry the exact command ID, and the same ID is reused for execution. Missing identity or tier no longer creates a pending decision row. Focused and full no-DB verification passed; independent re-review remains required.
...
- 2026-09-10 S3b review PASS with HANDOFF: the TUI consumes `GoalEvent` activity payloads, while the inspected control-plane effect path records capability-journal rows. User-owned live acceptance must verify the existing Plan 2 projection supplies effect action/target/classification/outcome, durable identity, and explicit tier; otherwise pending effect lines will remain absent. No provider or testbed changes were made.
...
- 2026-09-11 Plan 2 §S6c finding: the first pass of the observeWorker repair-hold transition did not recheck the repair approval's remaining repetition budget/expiry before moving a worker to `awaiting_repair`, so the real native-worker-acceptance test failed with the observed worker stuck at `awaiting_repair` instead of reaching `succeeded` after the single-count budget was already spent by the requeue. Fixed by checking `getCapabilityApproval` remaining count/expiry in `packages/persistence/src/worker.ts` before setting `awaiting_repair`; when the budget is exhausted or expired the worker goes straight to `succeeded` instead of holding open a session with no repair rounds left.

- 2026-09-11 Plan 2 §S6c finding: the first independent review (no-edit) of the merged S6c slice returned FAIL — RED 4 ("hold expires automatically") and RED 7 ("reconciles correctly on restart") were each proven only through direct calls to `expireAwaitingRepairWorker`/`recoverWorkerAfterRestart`, not through the real scheduled `metronome-loop.ts` pass or the real `reconcileOnStartup` orphan-journal path that production actually relies on for "automatic"/"on recovery" behavior. This was closed with two additive integration tests exercising only the real entry points; no production code changed. Lesson for future slices: when a RED test says "automatically" or "reconciles ... on recovery," the test must drive the actual scheduler/startup entry point, not the internal helper it calls, or an independent no-edit review will correctly flag the gap.

- 2026-09-11 Plan 3 §S4 `release-scenario-harness` re-assessment (unblocked, not yet resumed): now that plan-2 §S6b and §S6c are both merged, the slice's stated blocker is lifted, but code inspection shows two more concrete gaps beyond what the plan text already lists:
  1. `test/release-scenario/write-input.mjs`'s `mission` input kind builds a Mission Bundle substance with no `repairHold` field. Under §S6c's actual semantics, a worker with no `repairHold` goes straight to `succeeded` + kernel release the moment its first prompt completes — so `RUNBOOK.md` Step 10's `worker message` (the repair request) would hit a released, terminal worker in a real run, reproducing the exact bug §S6c was built to fix. `RUNBOOK.md` Steps 9/10 only guard on `status in (spawned|running)`, never `awaiting_repair`, so this defect would not be caught by re-running the current runbook.
  2. The CI-exercised path (`test/release-scenario/fake-control-plane-process.mjs` steps 9/10, driven by `run-fake-scenario.mjs`/`fake-provider.test.ts`) is a hand-rolled string/state-machine simulation of "quality_failed"/"repair_requested_through_maestro"/"quality_certified" events — it does not call the real `WorkerService`, real HTTP routes, or real `packages/persistence` worker/mission-bundle code at all. This is the same defect the plan's own review history already names ("string-based runbook.test.ts and fake HTTP process tests do not prove real WorkerService/API/PostgreSQL/provider execution"), and it has not been fixed since that finding was recorded — it looks structurally identical to the code that failed four prior independent reviews.
  Resuming §S4 honestly requires: (a) adding `repairHold` to the mission fixture and widening the Step 9/10 status guards to include `awaiting_repair`, and (b) replacing (or supplementing) `fake-control-plane-process.mjs`'s hand-rolled simulation with a CI-runnable harness that drives the real Control Plane HTTP surface + real PostgreSQL + a real (non-provider) `ExecutionKernelPort`/provider plugin the way `apps/control-plane/src/native-worker-acceptance.integration.test.ts` already does for a single worker — i.e., extending that same real-process pattern across all fourteen steps, not inventing a second, weaker one. That is a slice-sized rewrite of the fake-provider CI harness, comparable in size to §S6c's own work, and was not attempted this turn given the size and the four-times-failed review history; starting it without full worktree/RED/review rigor would risk a fifth failed review. Recommend treating this as its own bounded sub-effort before resuming §S4's merge/push steps.
...
## 2026-09-11 — Plan 6 S1 v13 independent review findings
- Fresh no-edit review of `19f075c` returned **REVIEW: FAIL**; focused domain tests passed 8/8, while PostgreSQL integration was not run because `MAESTRO_TEST_DATABASE_URL` is unset and the endpoint is unavailable.
- **High/blocker:** migration `0087_organizational_knowledge.sql` lines 120–127 reject missing evidence unless the immediately prior revision is already `unsupported`/`source-loss`, but `propagate_organizational_knowledge_evidence_loss` lines 208–218 creates the first such revision from an active row. The `deleteEvidenceSource` transaction therefore rolls back instead of appending unsupported lineage and deleting evidence.
- **High:** the SQL append-only trigger checks contiguous revisions and narrow scope lineage but does not compare immutable `source_project_id`, `source_goal_id`, `department_id`, `schema_version`, or related core identity/provenance fields with the prior revision.
- **High:** global curator standing/membership/independence is checked only in the TypeScript path. The DB authorization/trigger does not durably bind or enforce an independent curator.
- **High:** trigger-driven source-loss insertion omits `retention`, so it defaults to `project_lifetime` rather than preserving the prior revision's retention class.
- **Medium:** refresh computes an idempotency operation reference but inserts with the constant `system:knowledge-decay`, so a successful stale/contradicted refresh cannot be acknowledged on retry.
- **Medium:** proposal, project promotion, and evidence deletion do not provide complete stable replay/conflict contracts.
- **Medium:** database privacy/bounded-text checks do not cover every persisted text field and do not bound `generalized_statement` at the column level.
- **Medium:** proposal authorization binds identity and source references but not an immutable proposal payload hash, allowing an authorized issuer to insert a different payload if the table boundary is bypassed.
- Remediation must add RED regressions, then rerun focused tests, build/lint/diff-check, fresh independent review, and real PostgreSQL before merge.
- **Medium/high:** deleting evidence removes its `evidence_records` row without a durable tombstone or non-reuse constraint. `appendEvidenceMetadata` can reinsert the same `evidence_id` after source loss, while `source_evidence_loss_events` is keyed only by `event_id`; source identity can therefore be resurrected and reused. Add a never-reuse registry/tombstone or retain immutable metadata.
- **High:** `deleteEvidenceSource` authorizes any active operator with project membership/exact project role and a Goal lease, but not CEO approval or even a Department Head role, despite permanent source deletion being a CEO-critical boundary. `markKnowledgeUnsupportedForEvidence` and `markKnowledgeUnsupportedForDigest` similarly accept arbitrary project roles for durable status changes. Require the appropriate Head/CEO gate and add RED tests.

- 2026-09-13 Plan 6 §S1 final independent no-edit review at exact HEAD `49827ef` returned **REVIEW: FAIL** for one Gate 6 defect: `deleteEvidenceSource` checked the canonical permanent role table with `role_kind IN ('department_head', 'ceo')`, but the canonical CEO is `role_id=concertmaster`, `role_kind=concertmaster`; the supported `ceo` alias therefore failed both initial and retry prechecks. No additional general DB-hardening category was identified.
- 2026-09-13 Gate 6 finding resolved: evidence source deletion now uses the same explicit `ceo` → `concertmaster` role condition as the secured database boundary on both initial and retry paths. A real PostgreSQL regression covers deletion, provenance-preserving unsupported propagation, and idempotent retry. Targeted RED was observed (`1 failed`), then focused PostgreSQL verification passed `5 files / 29 tests`, with build/lint/diff-check clean.
...
- 2026-09-14 Plan 7-a §S3 scope correction resolved the previously recorded architecture blocker. The user-authorized thin organization route and API-client read path were added without persistence or model changes; Goal pressure remains explicitly excluded; registry next actions are state-valid. The corrected exact-HEAD review passed, and post-merge verification passed 244/244 files and 1686/1686 tests.

- 2026-09-14 Plan 7-a §S3 CI failure `34798344096` was a test-observation defect, not a production width defect: color-enabled CI made raw string length include ANSI SGR bytes. The minimal test-only visible-width correction was independently reviewed and passed post-merge verification in `daa1923` (244/244 files, 1686/1686 tests).

## 2026-09-14 — Plan 7 / Plan 7-c real-backing investigation blockers

- **Plan 7 §S7 persona inspection/editing:** blocked at the current repository boundary. `packages/persistence/src/persona-profile.ts:60-223` exposes durable profile functions, and `packages/persistence/src/improvement-candidate.ts:307,335,845` exposes candidate/history persistence, but there is no typed Control Plane/API-client bridge. `apps/control-plane/src/server.ts` only has the persona-evidence route; `packages/api-client/src/index.ts` has no profile/candidate methods; and `apps/carnegie/electron/apiBridge.ts` exposes none. Implementing the required candidate-only editing and real server rejection in the Carnegie-only slice would require inventing a contract/route or bypassing the server, so no code change was made.

- **Plan 7-c §S6 luthiery:** the investigation found no durable skill/tool registry with certification, rejection, hash, tag, or reuse state. Existing `allowedSkills`/`allowedTools` on Mission Bundles, permanent-role capability boundaries, the ephemeral runtime `ToolRegistry`, and Worker-output certifications are different records and cannot support the mockup's registry view. The implementation removes the misleading static rows and shows an honest unavailable state in `apps/carnegie/src/views/Luthiery.tsx`, with focused RED/GREEN coverage; commit `045263f` is pending independent review and merge verification.

- **Plan 7-c §S7 flashmob:** blocked at the current repository boundary. `apps/carnegie/src/views/Flashmob.tsx:7-32` and `FlashmobSession.tsx:4-43` are static/local-only; the only existing durable session is Goal-bound Conversation (`apps/control-plane/src/conversation-service.ts:12-22,26`, migration `0064_native_agent_runtime.sql:2-26`). No Flashmob session/profile table, provenance field, contract, API method, or route exists. A pre-Goal session and promote-to-Goal provenance therefore require new domain/persistence design, prohibited by this slice. No code change was made.
...
## 2026-09-15 — Plan 7-b §S7 read-depth route audit

The existing typed read responses were audited before expanding TUI rendering. Task Contract, Head Council participants/decision packet, Department Plan substance, Encore judgments/synthesis, Metronome challenge detail, certification lineage, and Goal budget envelope/reserved/spent values are already carried by their existing routes and are now rendered. The following requested fields are genuine route-level gaps and were not invented client-side: Head Council submitted briefs (the Head Council read response carries the immutable participant snapshot but no submitted brief list); Department Plan revision history (the existing response is the current version only); Worker mission/evidence payload (the `get` route returns `Worker`; the existing `observeWorker` route returns `WorkerObservation` and is now used by the panel for observability, but neither model carries mission/evidence-reference fields); Metronome findings (challenge-list returns challenges only); Certification evidence/findings (certification-list returns lineage only); and budget forecasts/department breakdown (GoalBudgetSummary returns envelope, reserved, and incurred spend only).
...
## 2026-09-15 — Plan 8 §S0 ensemble-router production-wiring scope blocker
- A read-only architecture audit against the post-Plan 7-e main ancestry (`52f62cf`) found that the stated S0 production path cannot be implemented honestly within the documented three-file scope.
- The current worker call chain is HTTP `apps/control-plane/src/server.ts` → `worker-service.ts` → `packages/persistence/src/worker.ts` → kernel. The only worker `ExecutionAdmission` is constructed inside persistence, where it directly uses `selectWorkerModel` and Mission Bundle `approvedModels`; it never calls `selectRoutedModel`. `native-admission.ts` provides the shared validation boundary, while `createPinnedNativeAdmission` is used for Head/Encore paths. The normal host admission path still rejects non-pin mode before a worker can be reached.
- The required durable pressure input is absent: Mission Bundle stores `taskDemand` and `approvedModels`, but no `WorkCharacter`; `WorkCharacter` exists only inside `RoutingEvidence`. Task Contract and Head Council records do not store E or an explicit Head uplift, the worker API has no E/pressure field, and no production call creates a Goal snapshot through `snapshotOperationalOverlayForGoalDurably`.
- The candidate join is also incomplete: the C snapshot can be read and `config/model_map.json` supplies A/B data, but operational overlay observations expose only opaque `candidateRef`/account values. No durable `candidateRef → modelRef` catalog exists. Do not synthesize refs from model slugs.
- Therefore no production source currently supplies the selector's complete A/B/C/E inputs. Adding arbitrary pressure, slug-derived candidates, or a placeholder catalog would falsify the route and violate the plan's real-PostgreSQL acceptance requirement. A truthful implementation needs a separately scoped durable E declaration/source, an explicit candidate catalog/mapping, and a callback seam from control-plane admission into persistence. The ensemble host/root admission rejection also needs an explicit product boundary decision.
- S0 implementation was not started. `native-admission.ts`'s shared validation function was not changed, and the Plan 8 §S1 freeze-tooling worktree remains untouched.

## 2026-09-15 — Plan 7-e §S4 post-merge verification
- On the dedicated disposable PostgreSQL container `maestro-s4-postmerge-postgres`, `npm run build`, `npm run lint`, `git diff --check`, and the serialized PostgreSQL command completed with exit `0`. The test run passed **291/291 files and 1941/1942 tests**, with one intentional skip, in 1072.04s (`/tmp/plan7e-s4-postmerge-pg-rerun.log`). The expected `git` stderr from negative invalid-revision assertions appeared during `server.test.ts`; that suite still passed 56/56.


## 2026-09-16 — Plan 8 §S7 routing-evidence redaction investigation
- Read `packages/domain/src/routing-evidence.ts`, `packages/persistence/src/ensemble-router-artifacts.ts`, `packages/persistence/src/worker.ts`, and the existing Discord/improvement-digest secret-handling helpers. The production worker derives routing evidence from the admission snapshot, then `recordRoutingEvidence` validated the shape but passed `JSON.stringify(value)` unchanged into the durable JSON column. Free-text task-demand rationales and routing rationale were not secret-screened at this boundary.
- RED proved a real boundary gap: a secret-like top-level rationale and nested task-demand rationale were accepted by `assertValidRoutingEvidence`; the persistence regression initially reached the fake SQL insert when the built domain package was stale. Rebuilding the package exposed the corrected boundary behavior.
- Minimal security fix: reuse the existing `redactDiscordSecretLikeText` detector in `routing-evidence` validation, recursively reject secret-like strings after structural validation, and let persistence fail before issuing SQL. The boundary rejects rather than silently rewriting because rewriting nested TaskDemand would invalidate its sealed `taskDemandHash` and make replay evidence dishonest.
- GREEN: `packages/domain/src/routing-evidence.test.ts` and `packages/persistence/src/ensemble-router-artifacts.test.ts` passed **14/14** after `npm run build`; no raw secret reached the fake persistence write. No provider-result identity or stale freshness claim is added.

## 2026-09-16 — Plan 7-e §S2 goal-less intake investigation

- The reported prompt is emitted by `apps/cli/src/tui/entry.ts` at the plain-text input branch: when the workspace is attached but `session?.goalId` is absent, it unconditionally appends `Select a Goal before sending a Concertmaster message` and does not create or send a conversation.
- The current conversation API cannot accept goal-less text. `packages/contracts/src/index.ts` requires `goalId` in both `ConversationSchema` and `CreateConversationInputSchema`; `packages/api-client/src/index.ts` exposes `createConversation(input)` with that required input; and `apps/control-plane/src/server.ts` parses the same strict schema before dispatch.
- The server service also requires the binding: `apps/control-plane/src/conversation-service.ts` verifies the Goal/project pair, stores `conversations.goal_id`, admits the model, and spawns the runtime with `context.goalId`. There is no project-scoped or goal-less conversation row, no server-side conversion of conversation text into a Task Contract draft, and no Concertmaster tool path that can create a Task Contract before a Goal exists.
- Existing TUI commands already separate project-level reads (`/goals list`, `/projection`) from Goal-scoped actions. Task Contract creation is project-scoped, but it currently requires structured `--substance-json`; no natural-language intake adapter exists below the TUI.
- Root cause: this is not only a misplaced prompt. Removing the TUI guard would hit a required backend `goalId` contract. A truthful goal-less flow needs an additive intake boundary that accepts project/session text, drafts or stores a Task Contract, and stops at explicit confirmation before Goal/Mission Bundle/worker execution. Building that missing server/domain persistence capability inside this UX fix would violate Plan 7-e §S2's scope rule and the current no-domain/authority/persistence constraint.
- Decision: record the gap and do not fabricate a Goal, placeholder conversation binding, or auto-approval. The minimal safe UX change must either route to an existing project-scoped intake surface (none found) or tell the operator how to start the structured Task Contract flow without saying that a Goal must already be selected.
...
## 2026-09-17 — UX loop compact dynamic-model recovery action visibility

- At compact widths below 16 rows, `/models list` projected the live catalog into the recovery dock, but an over-width provider/model identity caused `compactModelListAcknowledgement` to show only wrapped raw identity text. The visible result omitted the `/model use --model` action, so a beginner could not follow the recovery path without resizing or guessing.
- Candidates: (1) wrap the full command, (2) render an action-first `/model use --model` row followed by exact identity continuation lines, and (3) replace the raw identity fallback with resize guidance. Independent design critique ranked **2 > 1 > 3**, accepted #2, and rejected #3 because it hides the exact live identity and is not action-complete.
- RED updated the compact regression to require the action-first prefix, bounded continuation lines, exact identity preservation, and prefix wrapping below its width; the focused test failed before the winner implementation. GREEN keeps one-line commands when they fit, otherwise renders the command prefix first and wraps the live identity separately, preserving empty/unavailable catalog fallbacks and compact transcript semantics. Focused compact tests pass **1/1**; full TUI passes **56 files / 398 tests**, build, changed-file lint, and `git diff --check` pass; separate § 0.3 review returned **REVIEW: PASS**. Commit and push remain pending.
- Scope is pure TUI presentation in `apps/cli/src/tui/entry.ts` and `entry.test.ts`; no `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, provider, approval, conversation, or lifecycle behavior changed; no force-push.


## 2026-09-17 — UX loop Task Contract amend autocomplete

- `/task-contract amend` was advertised by the registry but had no autocomplete option map. At the attached/connected TUI, `amend --` returned no options even though the writer required explicit `--contract-id`, `--expected-version`, and `--substance-json`; `--command-id` is optional/generated. Manual partial submissions therefore failed sequentially with missing-option errors.
- Candidates: (1) add one explicit `task-contract amend` map entry, (2) generate completions from command-definition metadata, and (3) add action-specific usage guidance when completion is empty. Independent design critique ranked **1 > 2 > 3**, accepted #1, and rejected #3 as insufficiently actionable; #2 is a separate broader architecture follow-up.
- RED added `amend --` and `amend --expected-v` regressions and failed **1/17** with empty completions. GREEN adds the four truthful options without changing parser, structured payload, version checks, approval, or conversation behavior. Focused amendment tests pass **1/1**; full TUI passes **56 files / 399 tests**, build, changed-file lint, and `git diff --check` pass; separate § 0.3 review returned **REVIEW: PASS**. Commit `a3154d9` is pushed to `origin/main`; its CI is being monitored.
- Scope is the autocomplete map/test plus operation records; no `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, provider, approval, conversation, or lifecycle behavior changed; no force-push.


## 2026-09-17 — UX loop Task Contract select-roles autocomplete

- `/task-contract select-roles` was advertised by the registry and palette but had no autocomplete option map. At the attached/connected TUI, `select-roles --` returned no options even though the writer required explicit `--contract-id` and supported `--outside-evidence` and `--preview-needed` flags; submitting without the ID returned a missing-option error before any client call.
- Candidates: (1) add one explicit `task-contract select-roles` map entry, (2) generate completions from registry command metadata, and (3) add action-specific usage guidance when completion is empty. Independent design critique ranked **1 > 2 > 3**, accepted #1 for its narrow low-risk correction, and rejected #3 as non-actionable; #2 remains a broader drift-reduction follow-up.
- RED added `select-roles --` and `select-roles --outside` regressions and failed **1/18** with empty completions. GREEN adds the three truthful options without changing explicit identity, boolean semantics, parser, approval, or conversation behavior. Focused autocomplete tests pass **18/18**; full TUI passes **56 files / 400 tests**, build, changed-file lint, and `git diff --check` pass; separate § 0.3 review returned **REVIEW: PASS**. Commit `0ee8ded` is pushed to `origin/main`; CI `35177675821` is being monitored.
- Scope is the autocomplete map/test plus operation records; no `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, provider, approval, conversation, or lifecycle behavior changed; no force-push.


## 2026-09-17 — UX loop Task Contract launch autocomplete metadata

- `/task-contract launch` was visible in the registry/palette but had no autocomplete entry. At the TUI, `launch --` returned no options even though the writer required explicit `--contract-id`; submitting the action without that ID failed before any client call. This left the final Task Contract lifecycle action undiscoverable and could not safely infer the contract identity.
- Candidates: (1) add a launch-specific static map entry, (2) move action options into registry command metadata and generate autocomplete from that metadata, and (3) add an action-specific usage fallback. Independent design critique ranked **2 > 3 > 1**, accepted #2 because it removes registry/autocomplete drift at the source, and rejected #1 as another duplicate that can drift; #3 remains weaker because it does not provide normal option completion.
- RED added launch completion regressions and failed **1/19** with an empty result. GREEN moves the option catalog into registry-owned `CommandAction.options`, makes autocomplete read the matching action metadata, and adds the launch contract-id metadata while preserving all existing option completions. The metadata-source and launch tests pass **20/20**; full TUI passes **56 files / 402 tests**, build, changed-file lint, and `git diff --check` pass; separate § 0.3 re-review returned **REVIEW: PASS** after confirming the required operation-record docs are allowed. Commit `bf09030` is pushed to `origin/main`; CI `35178378642` is being monitored.
- Scope is command metadata/autocomplete presentation only; no `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, provider, approval, conversation, or lifecycle behavior changed; no force-push.


## 2026-09-17 — UX loop Goal lifecycle reason truthfulness

- Goal lifecycle autocomplete advertised `--reason` for `pause`, `resume`, `stop`, and `emergency-stop`, but `write-commands.ts` ignored it and strict `GoalControlInputSchema` did not accept it. A beginner could submit a rationale and believe it was recorded while the TUI sent only `projectId` and `expectedVersion`.
- Candidates: (1) remove `--reason` only from `pause`, (2) remove it from all four lifecycle metadata entries, and (3) implement reason end-to-end across contracts/API/server/domain/TUI. Independent design critique ranked **2 > 1 > 3**, accepted #2 as the smallest coherent truthful fix, and rejected #3 as an unrelated cross-layer capability expansion.
- RED added a four-action regression and failed **1/21** because `pause --rea` still suggested the unsupported option. GREEN removes `--reason` from all four registry-owned autocomplete metadata entries while preserving explicit Goal/version, optimistic concurrency, auth, approval, cancellation, and conversation behavior. Focused autocomplete tests pass **21/21**; full TUI passes **56 files / 403 tests**, build, changed-file lint, and `git diff --check` pass; separate § 0.3 review returned **REVIEW: PASS**. Commit `5d4e838` is pushed to `origin/main`; CI `35179008685` is being monitored.
- Scope is registry-owned TUI autocomplete metadata/test only; no `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, provider, approval, conversation, or lifecycle behavior changed; no force-push.


## 2026-09-17 — UX loop Head activation input discoverability

- `/head activate` is palette-visible, but registry-owned autocomplete had no `head activate` metadata while `write-commands.ts` requires `--activation-json` before calling `activateHead`; with a selected Goal, `activate --` and `activate --a` returned no completions and submit returned `Missing required option --activation-json`.
- Candidates: (1) add narrow registry-owned options `--goal-id`, `--activation-json`, and `--command-id`, (2) generate options from a shared CLI schema/contract mapping, and (3) add head-specific empty-completion/submit guidance. Independent design critique ranked **1 > 3 > 2**, accepted #1 as the smallest direct discoverability fix, and rejected #2 as broader refactor risk and #3 as too late to provide autocomplete. The JSON payload remains parser-validated; autocomplete does not imply it can construct or validate JSON.
- RED added `activate --` and `activate --a` regressions and failed **1/22** because both returned no options. GREEN adds the three truthful registry metadata options without changing parser, explicit Goal selection, command identity, auth, approval, cancellation, or conversation behavior. Focused autocomplete tests pass **22/22**; full TUI passes **56 files / 404 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate § 0.3 review returned **REVIEW: PASS**. Commit `68e78bf` is pushed to `origin/main`; CI `35179064173` completed successfully.
- Scope is registry-owned TUI autocomplete metadata/test only; no `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, Muze, §S11, or G9 changes; no force-push.


## 2026-09-17 — UX loop Council creation input discoverability

- `/council create` is palette-visible, but registry-owned autocomplete had no action metadata while `write-commands.ts` requires `--council-json`; with a selected Goal, `create --` and `create --c` returned no completions and submit returned `Missing required option --council-json` before any client call.
- Candidates: (1) add narrow registry-owned options `--goal-id`, `--council-json`, and `--command-id`, (2) broaden the metadata-source refactor to generate all action options, and (3) add action-specific usage fallback. Independent design critique ranked **1 > 3 > 2**, accepted #1 as the smallest truthful direct discoverability fix, and rejected #2 as disproportionate cross-action scope and #3 as too late for interactive completion. The JSON remains parser/schema validated; metadata does not imply autocomplete can construct or validate it.
- RED added `create --` and `create --c` regressions and failed **1/23** because both returned no options. GREEN adds the three truthful registry metadata options without changing parser, selected Goal binding, command identity, auth, approval, cancellation, or conversation behavior. Focused autocomplete tests pass **23/23**; full TUI passes **56 files / 405 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate § 0.3 review returned **REVIEW: PASS**. Commit `86abec7` is pushed to `origin/main`; CI `35179860504` completed successfully.
- Scope is registry-owned TUI autocomplete metadata/test only; no `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, Muze, §S11, or G9 changes; no force-push.


## 2026-09-17 — UX loop Council read identity discoverability

- `/council get` is palette-visible, but registry-owned autocomplete had no action metadata while `read-commands.ts` requires explicit `--council-id`; `get --` and `get --c` returned no completions and submit returned `Missing required option --council-id` before any client call.
- Candidates: (1) add one registry-owned `council get: [--council-id]` entry and regression, (2) generate options from authoritative command metadata/schemas, and (3) add action-specific missing-option guidance. Independent design critique ranked **1 > 2 > 3**, accepted #1 as the narrow truthful fix, and rejected #2 as broad parser/schema scope and #3 as too late to repair inline completion. Explicit council identity remains required; no inference is added.
- RED added `get --` and `get --c` regressions and failed **1/24** because both returned no options. GREEN adds the truthful registry metadata entry without changing read execution, explicit identity, auth, approval, cancellation, or conversation behavior. Focused autocomplete tests pass **24/24**; full TUI passes **56 files / 406 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate § 0.3 review returned **REVIEW: PASS**. Commit `cac0c37` is pushed to `origin/main`; CI `35180350579` completed successfully.
- Scope is registry-owned TUI autocomplete metadata/test only; no `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, Muze, §S11, or G9 changes; no force-push.


## 2026-09-17 — UX loop Council brief submission discoverability

- `/council submit-brief` is palette-visible, but registry-owned autocomplete had no action metadata while `write-commands.ts` requires explicit `--council-id`, `--department-id`, and `--brief-json`; `--command-id` remains optional/generated. `submit-brief --` and `submit-brief --c` returned no completions and submit failed first with `Missing required option --council-id` before any client call.
- Candidates: (1) add narrow registry-owned metadata for the four truthful options, (2) generate options from an authoritative metadata/schema source shared with writer/CLI, and (3) add bounded action-specific empty-completion guidance. Independent design critique ranked **1 > 2 > 3**, accepted #1 as the smallest direct discoverability fix, and rejected #2 as broad refactor risk and #3 as weaker non-inline guidance. Identity stays explicit; autocomplete only names the raw JSON input and does not construct or validate it.
- RED added `submit-brief --` and `submit-brief --dep` regressions and failed **1/25** because the action returned no options. GREEN adds `--council-id`, `--department-id`, `--brief-json`, and `--command-id` metadata without changing parser, JSON/schema validation, request construction, auth, approval, cancellation, or conversation behavior. Focused autocomplete tests pass **25/25**; full TUI passes **56 files / 407 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate § 0.3 review returned **REVIEW: PASS**. Commit `c91a7d4` is pushed to `origin/main`; CI `35180983299` completed successfully.
- Scope is registry-owned TUI autocomplete metadata/test only; no `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, Muze, §S11, or G9 changes; no force-push.


## 2026-09-17 — UX loop Council reveal identity discoverability

- `/council reveal` is palette-visible, but registry-owned autocomplete had no action metadata while `write-commands.ts` requires explicit `--council-id`; `reveal --` and `reveal --c` returned no completions and submit failed with `Missing required option --council-id` before any client call.
- Candidates: (1) add registry-owned metadata `--council-id` and `--command-id`, (2) generate options from an authoritative command/writer schema, and (3) add reveal-specific empty-completion/submit guidance. Independent design critique ranked **1 > 2 > 3**, accepted #1 as the smallest truthful repair, and rejected #2 as broader schema/CLI risk and #3 as late non-inline guidance. Explicit council identity, optional/generated command identity, auth, approval, cancellation, and conversation boundaries remain unchanged.
- RED added `reveal --` and a disambiguated `reveal --cou` regression and initially failed **1/26** because no options were advertised. GREEN adds the two truthful metadata options; the partial-prefix assertion uses `--cou` because both options legitimately share the `--c` prefix. Focused autocomplete tests pass **26/26**; full TUI passes **56 files / 408 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate § 0.3 review returned **REVIEW: PASS**. Commit `ca946d8` is pushed to `origin/main`; CI `35181551585` completed successfully.
- Scope is registry-owned TUI autocomplete metadata/test only; no `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, Muze, §S11, or G9 changes; no force-push.


## 2026-09-17 — UX loop Council decision input discoverability

- `/council decide` is palette-visible, but registry-owned autocomplete had no action metadata while `write-commands.ts` requires explicit `--council-id` and `--packet-json`; `--command-id` remains optional/generated. `decide --` and `decide --c` returned no completions and empty submit failed first with `Missing required option --council-id` before any client call.
- Candidates: (1) add narrow registry-owned metadata `--council-id`, `--packet-json`, and `--command-id`, (2) unify autocomplete with an authoritative command/writer/schema metadata source, and (3) add bounded action-specific empty-completion usage guidance. Independent design critique ranked **1 > 3 > 2**, accepted #1 as the smallest truthful fix, and rejected #2 as broad refactor risk and #3 as weaker late guidance. Explicit Council identity, raw JSON validation, optional/generated command identity, auth, approval, cancellation, and conversation boundaries remain unchanged.
- RED added `decide --` and `decide --pac` regressions and failed **1/27** because the action returned no options. GREEN adds the three truthful metadata options without changing parser, JSON/schema validation, request construction, or server behavior. Focused autocomplete tests pass **27/27**; full TUI passes **56 files / 409 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate § 0.3 review returned **REVIEW: PASS**. Commit `57e8bdb` is pushed to `origin/main`; CI `35182373859` completed successfully.
- Scope is registry-owned TUI autocomplete metadata/test only; no `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, Muze, §S11, or G9 changes; no force-push.


## 2026-09-17 — UX loop Department Plan family input discoverability

- `/department-plan create`, `get`, and `revise` are palette-visible, but registry-owned autocomplete had no action metadata. `create --`, `get --`, and `revise --` returned no options even though `write-commands.ts`/`read-commands.ts` require explicit Council and Department identifiers, create/revise require JSON plan input, and revise requires a reason; revise also accepts expected version while command identity remains optional/generated.
- Candidates: (1) add only `department-plan create` metadata, (2) add truthful registry metadata for the complete family (`create`, `get`, `revise`) with family regressions, and (3) replace per-action metadata with an authoritative command-spec/schema source shared by dispatch and autocomplete. Independent design critique ranked **2 > 1 > 3**, accepted #2 as the best scoped fix for consistent family discoverability, conditionally accepted #1 only as a create-only emergency, and rejected #3 as a separate high-risk architecture change. Do not invent options for symmetry; explicit IDs, JSON/schema validation, idempotency, auth, approval, cancellation, and conversation boundaries remain unchanged.
- RED added create/get/revise completion regressions and failed **1/28** because the family returned no options. GREEN adds only truthful family metadata: create `--council-id`, `--department-id`, `--plan-json`, `--command-id`; get the two identifiers; revise those identifiers plus `--expected-version`, `--plan-json`, `--reason`, and `--command-id`. Focused autocomplete tests pass **28/28**; full TUI passes **56 files / 410 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate § 0.3 review returned **REVIEW: PASS**. Commit `2f20c2f` is pushed to `origin/main`; CI `35185748069` is being monitored.
- Scope is registry-owned TUI autocomplete metadata/test only; no `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, Muze, §S11, or G9 changes; no force-push.


## 2026-09-17 — UX loop Mission Bundle input discoverability

- `/mission-bundle create` and `get` are palette-visible, but registry-owned autocomplete had no action metadata. `create --` and `get --` returned no options although create requires explicit `--council-id`, `--department-id`, `--item-id`, and `--bundle-json`, and get requires `--council-id`, `--department-id`, `--plan-version`, and `--item-id`; create's `--command-id` remains optional/generated.
- Candidates: (1) add narrow static registry metadata for both actions, (2) derive autocomplete from an authoritative shared command-spec/schema, and (3) add bounded action-specific usage fallback. Independent design critique ranked **2 > 1 > 3**, accepted #2 as preferred only when the shared spec also drives palette, completion, parsing/validation, help, and policy; accepted #1 as a narrow interim fix; and rejected #3 as the primary repair. We choose #1 to keep this finding low-risk and behavior-preserving. Explicit identity, JSON/schema validation, idempotency, auth, approval, cancellation, and conversation boundaries remain unchanged.
- RED added create/get completion regressions and failed **1/29** because both actions returned no options. GREEN adds only exact option metadata for create (`--council-id`, `--department-id`, `--item-id`, `--bundle-json`, `--command-id`) and get (`--council-id`, `--department-id`, `--plan-version`, `--item-id`). Focused autocomplete tests pass **29/29**; full TUI passes **56 files / 411 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate § 0.3 review returned **REVIEW: PASS**. Commit `cbdd6fb` is pushed to `origin/main`; CI `35186316153` is being monitored.
- Scope is registry-owned TUI autocomplete metadata/test only; no `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, Muze, §S11, or G9 changes; no force-push.


## 2026-09-17 — UX loop Worker spawn input discoverability

- `/worker spawn` is palette-visible, but registry-owned autocomplete had no action metadata. `spawn --` and `spawn --c` returned no options although `write-commands.ts` requires explicit `--council-id`, `--department-id`, and `--worker-json`; `SpawnWorkerInputSchema` also requires `planVersion` and `itemId` inside the JSON payload, while `--command-id` remains optional/generated.
- Candidates: (1) add narrow static registry metadata `--council-id`, `--department-id`, `--worker-json`, and optional `--command-id`, (2) derive options from an authoritative shared worker-spawn action spec driving parser/handler/help/policy, and (3) add a bounded usage hint when completion is empty. Independent design critique ranked **2 > 1 > 3**, accepted #2 as best long-term only if scoped to one authoritative worker-spawn spec, accepted #1 as the best narrow low-risk fix, and rejected #3 as primary. We choose #1 for this finding to preserve behavior and avoid cross-layer refactoring. Explicit IDs, JSON/schema validation including plan/item fields, idempotency, auth, approval, cancellation, and conversation boundaries remain enforced by existing runtime paths.
- RED added `spawn --` and `spawn --wor` regressions and failed **1/30** because the action returned no options. GREEN adds exact registry metadata including optional/generated command identity without changing dispatch, schema validation, auth, approval, cancellation, or conversation behavior. Focused autocomplete tests pass **30/30**; full TUI passes **56 files / 412 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate § 0.3 review returned **REVIEW: PASS**. Commit `74536d4` is pushed to `origin/main`; CI `35186905697` is queued.
- Scope is registry-owned TUI autocomplete metadata/test only; no `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, Muze, §S11, or G9 changes; no force-push.


## 2026-09-17 — UX loop Worker accept input discoverability

- `/worker accept` is palette-visible, but registry-owned autocomplete had no action metadata. `accept --` returned no options although `write-commands.ts` requires explicit `--worker-id` and `--reason` before calling `acceptWorker`; command identity remains existing generated behavior.
- Candidates: (1) add static registry metadata `worker accept: [--worker-id, --reason]` with focused regressions, (2) add metadata for every Worker action and generate autocomplete from one per-action Worker contract, and (3) put the required syntax in the palette description or an input prompt while leaving autocomplete unchanged. Independent design critique ranked **1 > 2 > 3**, accepted #1 as the direct low-risk repair, and rejected #2 as broader scope and #3 as non-composable weaker guidance. Explicit Worker identity, idempotency, auth, approval, cancellation, and conversation boundaries remain enforced by existing runtime paths.
- RED added `accept --` and `accept --rea` regressions and failed **1/31** because the action returned no options. GREEN adds only the two truthful metadata options without changing required-input validation, dispatch, generated command identity, auth, approval, cancellation, or conversation behavior. Focused autocomplete tests pass **31/31**; full TUI passes **56 files / 413 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate § 0.3 review returned **REVIEW: PASS**. Commit `50db481` is pushed to `origin/main`; CI `35186952520` completed successfully.
- Scope is registry-owned TUI autocomplete metadata/test only; no `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, Muze, §S11, or G9 changes; no force-push.


## 2026-09-17 — UX loop Worker get identity discoverability

- `/worker get` is palette-visible, but registry-owned autocomplete had no action metadata. `get --` and `get --wor` returned no options even though `read-commands.ts` requires explicit `--worker-id` before calling the authenticated `getWorker` path.
- Candidates: (1) add `worker get: [--worker-id]` metadata and a focused regression, (2) hide/remove the action from the palette until argument entry exists, and (3) add a dedicated command schema/argument prompt for explicit ID routing. Independent design critique ranked **3 > 1 > 2**, accepted #3 as the best guided UX if kept on the existing authenticated/cancellable read path, accepted #1 as the narrowest valid repair, and rejected #2 because it removes a supported capability. We choose #1 to keep scope surgical; no identity inference, authorization bypass, or conversation leakage is introduced.
- RED added `get --` and `get --wor` regressions and failed **1/32** because the action returned no options. GREEN adds only `--worker-id` metadata without changing required-input validation, read-only/idempotent behavior, auth, cancellation, or conversation boundaries. Focused autocomplete tests pass **32/32**; full TUI passes **56 files / 414 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate § 0.3 review returned **REVIEW: PASS**. Commit `6b89293` is pushed to `origin/main`; CI `35188833159` is in progress.
- Scope is registry-owned TUI autocomplete metadata/test only; no `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, Muze, §S11, or G9 changes; no force-push.


## 2026-09-17 — UX loop Event cursor discoverability

- `/events list` is palette-visible and `read-commands.ts` accepts optional `--after` cursor input, defaulting to `"0"`, but registry-owned autocomplete had no `events list` metadata. `list --` returned no completion even though `--after=123` is accepted and used for incremental reads.
- Candidates: (1) add static registry metadata `events list: [--after]` with a focused regression, (2) remove `--after` and always read from cursor `0`, and (3) replace hand-maintained read metadata with a shared registry/handler source. Independent design critique ranked **1 > 3 > 2**, accepted #1 as the smallest truthful fix, rejected #3 for this narrow finding as broader anti-drift architecture, and rejected #2 because it discards supported pagination semantics. Read-only cursor behavior, auth, cancellation, idempotency, and conversation boundaries remain unchanged.
- RED added `list --` and `list --aft` regressions and failed **1/33** because the optional cursor returned no completion. GREEN adds only `--after` metadata without changing read dispatch or cursor defaults. Focused autocomplete tests pass **33/33**; full TUI passes **56 files / 415 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate § 0.3 review returned **REVIEW: PASS**. Commit `7541481` is pushed to `origin/main`; CI `35190489908` is in progress.
- Scope is registry-owned TUI autocomplete metadata/test only; no `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, Muze, §S11, or G9 changes; no force-push.


## 2026-09-17 — UX loop Worker cancellation identity discoverability

- `/worker cancel` is palette-visible, but registry-owned autocomplete had no action metadata. `cancel --` and `cancel --wor` returned no options although `write-commands.ts` requires explicit `--worker-id` before calling the authenticated `cancelWorker` path with attached project context and generated command identity.
- Candidates: (1) add static metadata `worker cancel: [--worker-id]` with a focused regression, (2) advertise `--worker-id`, `--project-id`, and `--command-id` while teaching precedence, and (3) make registry metadata the single source with required-option parity tests and safe context/default exceptions. Independent design critique ranked **1 > 3 > 2**, accepted #1 as the immediate fix, accepted #3 only as bounded follow-up hardening, and rejected #2 because exposing context/idempotency internals invites ambiguity and can undermine auth, project scoping, and generated command identity. Existing cancellation, auth, idempotency, and conversation boundaries remain unchanged.
- RED added `cancel --` and `cancel --wor` regressions and failed **1/34** because the required worker identity returned no completion. GREEN adds only `--worker-id` metadata without changing cancellation dispatch or runtime validation. Focused autocomplete tests pass **34/34**; full TUI passes **56 files / 416 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; first § 0.3 review was invalid because it stopped before inspection, and independent retry returned **REVIEW: PASS** after inspecting the exact diff. Commit `b6d7812` is pushed to `origin/main`; CI `35191059193` completed successfully.
- Scope is registry-owned TUI autocomplete metadata/test only; no `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, Muze, §S11, or G9 changes; no force-push.


## 2026-09-17 — UX loop Worker observation identity discoverability

- `/worker observe` is palette-visible, but registry-owned autocomplete had no action metadata. `observe --` and `observe --wor` returned no options although `write-commands.ts` requires explicit `--worker-id` before calling `observeWorker`.
- Candidates: (1) add static metadata `worker observe: [--worker-id]` with a focused regression, (2) move option metadata onto `CommandAction` definitions and generate autocomplete from the single registry source, and (3) add a special `worker observe` completion branch in `autocomplete.ts`. Independent design critique ranked **1 > 2 > 3**, accepted #1 as the smallest truthful repair, and rejected #2 as a broader registry migration and #3 as brittle duplicated command knowledge. Explicit worker identity, auth, approvals, cancellation, idempotency, and conversation boundaries remain unchanged.
- RED added `observe --` and `observe --wor` regressions and failed **1/35** because the required worker identity returned no completion. GREEN adds only `--worker-id` metadata without changing observe dispatch or runtime validation. Focused autocomplete tests pass **35/35**; full TUI passes **56 files / 417 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate § 0.3 review returned **REVIEW: PASS**. Commit `0cc1e35` is pushed to `origin/main`; CI `35192382419` completed successfully.
- Scope is registry-owned TUI autocomplete metadata/test only; no `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, Muze, §S11, or G9 changes; no force-push.


## 2026-09-17 — UX loop Worker listing goal-scope discoverability

- `/worker list` is palette-visible, but registry-owned autocomplete had no action metadata. `list --` and `list --g` returned no options although `read-commands.ts` routes `worker:list` through `selectedGoal`, which accepts attached Goal context or requires explicit `--goal-id` when no Goal is selected.
- Candidates: (1) add static metadata `worker list: [--goal-id]` with a focused regression, (2) add shared metadata for goal-scoped reads with a consistency test, and (3) derive `--goal-id` in autocomplete from handler `selectedGoal`/`requiresGoal` semantics. Independent design critique ranked **2 > 1 > 3**, accepted #2 as the preferred longer-term bounded design, accepted #1 as the narrow fallback selected for this surgical fix, and rejected #3 because it couples autocomplete to runtime semantics. Explicit Goal scope, attached-context fallback, auth, approval, cancellation, idempotency, and conversation boundaries remain unchanged.
- RED added `list --` and `list --g` regressions and failed **1/36** because the Goal scope option returned no completion. GREEN adds only `--goal-id` metadata without changing selected-goal resolution or read dispatch. Focused autocomplete tests pass **36/36**; full TUI passes **56 files / 418 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate § 0.3 review returned **REVIEW: PASS**. Commit `cfb6f71` is pushed to `origin/main`; CI `35195167285` completed successfully.
- Scope is registry-owned TUI autocomplete metadata/test plus operation records only; no `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, Muze, §S11, or G9 changes; no force-push.


## 2026-09-17 — UX loop Worker certification input discoverability

- `/worker certify` is palette-visible, but registry-owned autocomplete had no action metadata. `certify --` and `certify --wor` returned no options although `write-commands.ts` requires explicit `--worker-id` and `--certification-json` before validating the certification payload.
- Candidates: (1) add static metadata `worker certify: [--worker-id, --certification-json]` with a focused regression, (2) make one typed action-spec source own required option metadata for both autocomplete and handler validation, and (3) remove `certify` from the palette until typed metadata is wired. Independent design critique ranked **1 > 2 > 3**, accepted #1 as the smallest truthful repair, accepted #2 only as a follow-up, and rejected #3 because it hides a valid operation. Existing explicit identity, project/auth path, approvals, cancellation, generated command-id/idempotency, and conversation boundaries remain unchanged.
- RED added `certify --` and `certify --wor` regressions and failed **1/37** because neither required option returned a completion. GREEN adds only the two metadata options without changing handler validation or dispatch. Focused autocomplete tests pass **37/37**; full TUI passes **56 files / 419 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate § 0.3 review returned **REVIEW: PASS**. Commit and push remain pending.
- Scope is registry-owned TUI autocomplete metadata/test plus operation records only; no `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, Muze, §S11, or G9 changes; no force-push.


## 2026-09-17 — UX loop Git goal branch input discoverability

- `/git goal-branch` is palette-visible, but registry-owned autocomplete had no action metadata. `goal-branch --` returned no options although `write-commands.ts` requires `--repository-path`, `--branch-name`, and `--base-revision`; Goal scope still comes from selected context or explicit `--goal-id`.
- Candidates: (1) add static metadata `git goal-branch: [--repository-path, --branch-name, --base-revision]` with a focused regression, (2) co-locate option metadata on the `goal-branch` `CommandAction`, and (3) establish a typed required-option contract per handler with generated autocomplete and palette parity tests. Independent design critique ranked **3 > 2 > 1**, accepted #3 as the strongest long-term design, accepted #2 as an interim option, and rejected #1 as detached metadata; we choose #1 for this narrow behavior-preserving loop item. Selected Goal/project auth, generated command identity, approval, cancellation, idempotency, and conversation boundaries remain unchanged.
- RED added `goal-branch --` and `goal-branch --repo` regressions and failed **1/38** because required branch inputs returned no completions. GREEN adds only the three metadata options without changing handler validation or dispatch. Focused autocomplete tests pass **38/38**; full TUI passes **56 files / 420 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate § 0.3 review returned **REVIEW: PASS**. Commit and push remain pending.
- Scope is registry-owned TUI autocomplete metadata/test plus operation records only; no `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, Muze, §S11, or G9 changes; no force-push.


## 2026-09-17 — UX loop budget Goal-scope autocomplete

- `/budget get` is palette-visible, but its required Goal selector was absent from registry-owned autocomplete metadata. `get --` returned no options although `read-commands.ts` routes `budget:get` through `selectedGoal`, which accepts attached Goal context or requires explicit `--goal-id`; without a selected Goal the runtime truthfully returns `No Goal is selected; use --goal-id or /goal select`.
- Candidates: (1) add static metadata `budget get: [--goal-id]`, (2) add `requiresGoal: true` and synthesize `--goal-id` from that bounded metadata, and (3) make option metadata declarative or generated from checked read contracts with a parity invariant. Independent design critique ranked **1 > 2 > 3**, accepted #1 as the smallest truthful repair, and rejected #2 as broader behavior and #3 as larger refactor scope.
- RED added `get --` and `get --g` regressions and failed **1/39**; GREEN adds only `--goal-id` metadata without changing attached-Goal fallback, Goal identity, authentication, authorization, cancellation, idempotency, or conversation boundaries. Focused autocomplete tests pass **39/39**; full TUI passes **56 files / 421 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate § 0.3 review returned **REVIEW: PASS**. Commit and push are pending. No `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, Muze, §S11, or G9 changes; no force-push.


## 2026-09-17 — UX loop certification alias autocomplete

- `/certification certify` is palette-visible, but its required Worker identity and certification payload were absent from registry-owned autocomplete. `certify --` returned no options although `write-commands.ts` routes `certification:certify` through the certification branch, which requires `--worker-id` and `--certification-json` before the existing authenticated `certifyWorker` path.
- Candidates: (1) add explicit metadata `certification certify: [--worker-id, --certification-json]`, (2) share one readonly certification-options tuple between `worker certify` and this alias, and (3) add a canonical-alias lookup in registry construction/autocomplete. Independent design critique ranked **1 > 2 > 3**, accepted #1 as the smallest explicit alias repair, and rejected #2/#3 for broader refactor or less declarative coupling.
- RED added `certify --` and `certify --cert` regressions and failed **1/40**; GREEN adds only the two alias metadata options without changing certification validation, explicit Worker identity, project/auth scope, approvals, cancellation, command-id/idempotency, or conversation boundaries. Focused autocomplete tests pass **40/40**; full TUI passes **56 files / 422 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate § 0.3 review returned **REVIEW: PASS**. Commit and push are pending. No `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, Muze, §S11, or G9 changes; no force-push.


## 2026-09-17 — UX loop certification list Goal-scope autocomplete

- `/certifications list` and its singular `/certification list` alias are palette-visible, but both lack the required Goal selector in registry-owned autocomplete. `list --` and `list --g` returned no options although `read-commands.ts` handles both keys through `selectedGoal`, which accepts attached Goal context or requires explicit `--goal-id`; manually typing it reaches `listCertifications`.
- Candidates: (1) add both explicit map entries `certifications list: [--goal-id]` and `certification list: [--goal-id]`, (2) share one readonly `GOAL_SCOPE_OPTIONS` tuple across both aliases, and (3) mark both actions `requiresGoal: true` and synthesize `--goal-id` in autocomplete. Independent design critique confirmed one alias-family finding, ranked **1 > 2 > 3**, accepted #1 as the smallest direct parity repair, and rejected #2/#3 for broader refactor or generic behavior risk.
- RED added plural and singular `list --`/`list --g` regressions and failed **1/41**; GREEN adds only the two Goal option metadata entries without changing Goal identity, attached-context fallback, authentication, authorization, approvals, cancellation, idempotency, or conversation boundaries. Focused autocomplete tests pass **41/41**; full TUI passes **56 files / 423 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate § 0.3 review returned **REVIEW: PASS**. Commit and push are pending. No `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, Muze, §S11, or G9 changes; no force-push.


## 2026-09-17 — UX loop conditional Worker certification autocomplete

- `/worker certify-conditional` is palette-visible but has no action-input autocomplete. Its write handler requires `--worker-id`, `--kind` (`security` or `safety_compliance`), and `--certification-json`; without these, runtime errors are truthful but the visible action gives no discoverable path.
- Candidates: (1) add explicit metadata `worker certify-conditional: [--worker-id, --kind, --certification-json]`, (2) co-locate required option metadata with each registry action and derive autocomplete from that source, and (3) hide the action from the palette until metadata exists. Independent design critique ranked **1 > 2 > 3**, accepted #1 as the smallest direct repair, identified #2 as the stronger long-term anti-drift design, and rejected #3 because it hides a supported operation.
- RED added `certify-conditional --` and `--kin` regressions and failed **1/42**; GREEN adds only the three metadata options without changing Worker/project identity, authentication, authorization, approvals, cancellation, command-id/idempotency, or conversation boundaries. Focused autocomplete tests pass **42/42**; full TUI passes **56 files / 424 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate § 0.3 review returned **REVIEW: PASS**. Commit and push are pending. No `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, Muze, §S11, or G9 changes; no force-push.


## 2026-09-17 — UX loop Git worker-worktree autocomplete

- `/git worker-worktree` is palette-visible but has no action-input autocomplete. Its write handler requires `--worker-id` and `--worktree-path` before calling `createWorkerWorktree`; `--command-id` is an accepted optional idempotency override, while project context remains injected from the authenticated TUI context.
- Candidates: (1) add explicit metadata `git worker-worktree: [--worker-id, --worktree-path, --command-id]`, (2) make route/handler contracts the single metadata source with required-vs-optional markers, and (3) hide the action from the palette until metadata exists. Independent design critique ranked **2 > 1 > 3**, accepted #1 as the correct bounded repair, identified #2 as the longer-term anti-drift design, and rejected #3 because it hides a wired capability.
- RED added `worker-worktree --` and typed-prefix regressions and failed **1/43**; GREEN adds only the three truthful metadata options without changing Worker/project identity, authentication, authorization, approvals, cancellation, command-id/idempotency, or conversation boundaries. Focused autocomplete tests pass **43/43**; § 0.3 review, full revalidation, commit, and push are pending. No `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, Muze, §S11, or G9 changes; no force-push.


## 2026-09-17 — UX loop Git department-branch autocomplete

- `/git department-branch` is a live palette-visible write action but has no action-input autocomplete. The TUI handler requires `--council-id` and `--department-id`, injects authenticated project context, and uses the optional generated/explicit `--command-id`; direct parsing reaches the existing handler and client call.
- Candidates: (1) add static metadata `git department-branch: [--council-id, --department-id, --command-id]`, (2) move this tuple into the registry action definition, or (3) introduce an authoritative typed command-spec source shared by registry/autocomplete and handler validation. Independent design critique ranked **1 > 2 > 3**, selecting #1 as the proportional direct repair, reserving #2 for repeated drift, and rejecting #3 as an oversized separate refactor.
- RED adds `department-branch --` and typed-prefix regressions; GREEN will add only truthful metadata without changing council/department/project identity, authentication, authorization, approvals, cancellation, command-id/idempotency, or conversation boundaries. No excluded files are touched.


## 2026-09-17 — UX loop Metronome challenge autocomplete

- `/metronome challenge` is a live palette-visible write action but has no action-input autocomplete. Its handler requires `--reason`, JSON `--finding-ids`, and JSON `--evidence-references`, resolves selected Goal context or accepts explicit `--goal-id`, and uses the shared optional `--command-id` idempotency override. A manual command reaches `raiseMetronomeChallenge`, so this is discoverability drift rather than a dead route.
- Candidates: (1) add static metadata `metronome challenge: [--goal-id, --reason, --finding-ids, --evidence-references, --command-id]`, (2) co-locate these options in the action definition and add registry parity checks, or (3) add handler/schema-backed autocomplete with JSON-shape examples. Independent design critique ranked **2 > 1 > 3**, identifying #2 as the durable design but selecting #1 for this bounded low-risk repair and rejecting #3 as excessive coupling.
- RED will add `challenge --` and typed-prefix regressions; GREEN will add only truthful completion metadata without changing Goal identity, project/auth scope, JSON validation, approvals, cancellation, command-id/idempotency, or conversation boundaries. No excluded files are touched.


## 2026-09-17 — UX loop Metronome challenges list Goal autocomplete
...
## 2026-09-17 — UX loop Metronome resolve autocomplete

- `/metronome resolve` is a live palette-visible write action but has no action-input autocomplete. Its handler requires `--challenge-id` and `--reason`, accepts optional explicit `--command-id` through the shared idempotency helper, and a real parsed/mock-client path reaches `resolveMetronomeChallenge`; empty input returns `Missing required option --challenge-id`.
- Candidates: (1) add static metadata `metronome resolve: [--challenge-id, --reason, --command-id]`, (2) move option metadata onto each `CommandAction` with parity checks, or (3) add a special usage fallback without map metadata. Independent design critique ranked **2 > 1 > 3**, selecting #1 as the proportional behavior-preserving repair and rejecting #3 because it lacks composable completion.
- RED will add `resolve --` and typed-prefix regressions; GREEN will add only truthful completion metadata without changing challenge identity, reason validation, project/auth scope, approvals, cancellation, idempotency, or conversation boundaries. No excluded files are touched.


## 2026-09-17 — UX loop Metronome safe-pause autocomplete
...
## 2026-09-17 — UX loop plural workers get identity autocomplete

- `/workers get` is a palette-visible compatibility alias defined with `get`, and `read-commands.ts` handles both `worker:get` and `workers:get` with required `--worker-id`, but the literal `actionOptions` map contains only the repaired `workers list` alias and no `workers get` entry. The plural alias therefore returns no `--worker-id` completion.
- Candidates: (1) add static metadata `workers get: [--worker-id]`, (2) share a readonly Worker-ID tuple between singular and plural aliases, or (3) canonicalize the `workers` alias before metadata lookup and dispatch. Independent design critique ranked **1 > 2 > 3**, selecting #1 as the smallest compatibility-preserving repair, with #2 as a limited anti-drift improvement and #3 as broader behavior coupling.
- RED failed **1/51**; GREEN adds only the `workers get: [--worker-id]` metadata. Focused autocomplete tests pass **51/51**; full TUI passes **56 files / 433 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate §0.3 review returned **REVIEW: PASS**. Commit `0e747358` is pushed to `origin/main`; CI is queued. Required identity validation, project/auth scope, approvals, cancellation, idempotency, and conversation boundaries remain unchanged. No excluded files are touched.


## 2026-09-17 — UX loop Git goal-revision Goal autocomplete

- `/git goal-revision` is palette-visible and in `TUI_WRITE_COMMAND_KEYS`; its handler calls `selectedGoal`, which consumes explicit `--goal-id` before selected context fallback, but `actionOptions` has no `git goal-revision` entry. The supported explicit scope option is therefore undiscoverable.
- Candidates: (1) add static metadata `git goal-revision: [--goal-id]`, (2) introduce a shared Goal-scope option group for every `selectedGoal` action, or (3) add a generic `requiresGoal` autocomplete fallback and mark this action. Independent design critique ranked **1 > 3 > 2**, selecting #1 as the smallest match for the current literal registry, with #3 as a future semantic approach and #2 as broadest synchronization risk.
- RED failed **1/52**; GREEN adds only the `git goal-revision: [--goal-id]` metadata. Focused autocomplete tests pass **52/52**; full TUI passes **56 files / 434 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate §0.3 review returned **REVIEW: PASS**. Commit `07b24e7a` is pushed to `origin/main`; CI is queued. Goal selection, project/auth scope, approvals, cancellation, idempotency, and conversation boundaries remain unchanged. No excluded files are touched.


## 2026-09-17 — UX loop Metronome scan Goal autocomplete

- `/metronome scan` is a palette-visible supported write action in `TUI_WRITE_COMMAND_KEYS`; its live handler calls `selectedGoal`, consuming explicit `--goal-id` before context fallback, but `actionOptions` has no `metronome scan` entry. The supported explicit Goal option is therefore undiscoverable.
- Candidates: (1) add static metadata `metronome scan: [--goal-id]`, (2) introduce a shared readonly Goal-scope tuple for selected-Goal actions, or (3) add a generic autocomplete fallback driven by a `requiresGoal`/scope marker. Independent design critique ranked **1 > 3 > 2**, selecting #1 as the smallest current-design repair, with #3 as a future semantic approach and #2 as the broadest refactor for one omission.
- RED failed **1/53**; GREEN adds only the `metronome scan: [--goal-id]` metadata. Focused autocomplete tests pass **53/53**; full TUI passes **56 files / 435 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate §0.3 review returned **REVIEW: PASS**. Commit `cc07dfd7` is pushed to `origin/main`; CI is queued. scan behavior, Goal selection, project/auth scope, approvals, cancellation, idempotency, and conversation boundaries remain unchanged. No excluded files are touched.


## 2026-09-17 — UX loop Encore review input autocomplete

- `/encore review` is palette-visible and supported in `TUI_WRITE_COMMAND_KEYS`; its live handler calls `selectedGoal`, parses required `--review-json`, and uses optional/generated `--command-id`, but `actionOptions` has no `encore review` entry. These supported inputs are undiscoverable.
- Candidates: (1) add static metadata `encore review: [--goal-id, --review-json, --command-id]`, (2) co-locate options on the `CommandAction` definition and derive autocomplete from it, or (3) introduce a typed shared action spec for autocomplete and handler validation. Independent design critique ranked **1 > 2 > 3**, selecting #1 as the proportional behavior-preserving repair, with #2 as bounded anti-drift follow-up and #3 as oversized for one omission.
- RED failed **1/54**; GREEN adds only the `encore review: [--goal-id, --review-json, --command-id]` metadata. Focused autocomplete tests pass **54/54**; full TUI passes **56 files / 436 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate §0.3 review returned **REVIEW: PASS**. Commit `bfcef03d` is pushed to `origin/main`; CI was superseded by the subsequent push. Review JSON validation, Goal selection, project/auth scope, approvals, cancellation, idempotency, and conversation boundaries remain unchanged. No excluded files are touched.


## 2026-09-17 — UX loop Git status Goal autocomplete

- `/git status` is a palette-visible live read action; its handler calls `selectedGoal`, consuming explicit `--goal-id` before attached context fallback, but the literal `actionOptions` map has no `git status` entry. The supported explicit Goal selector is therefore undiscoverable.
- Candidates: (1) add static metadata `git status: [--goal-id]`, (2) introduce a shared readonly Goal-scope tuple for selected-Goal actions, or (3) add a generic `requiresGoal`-driven autocomplete fallback. Independent design critique ranked **1 > 3 > 2**, selecting #1 as the smallest current-design repair, with #3 as a future semantic approach and #2 as the broadest refactor for one omission.
- RED failed **1/55**; GREEN adds only the `git status: [--goal-id]` metadata. Focused autocomplete tests pass **55/55**; full TUI passes **56 files / 437 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate §0.3 review returned **REVIEW: PASS**. Commit `96eab6fa` is pushed to `origin/main`; CI was superseded by the subsequent push. Read behavior, Goal selection, project/auth scope, approvals, cancellation, idempotency, and conversation boundaries remain unchanged. No excluded files are touched.


## 2026-09-17 — UX loop Encore Council list Goal autocomplete

- `/encore-council list` is palette-visible and read-supported; its handler calls `selectedGoal`, consuming explicit `--goal-id` before attached context fallback, but the literal `actionOptions` map has no `encore-council list` entry. The supported explicit Goal selector is therefore undiscoverable.
- Candidates: (1) add static metadata `encore-council list: [--goal-id]`, (2) share a readonly Goal-scope option tuple across selected-Goal actions, or (3) remove explicit Goal input and rely only on selected context. Independent design critique ranked **1 > 3 > 2**, selecting #1 as the smallest compatibility-preserving repair, with #3 as a safer future structural direction and #2 as a behavior-regressing targeting change.
- RED failed **1/56**; GREEN adds only the `encore-council list: [--goal-id]` metadata. Focused autocomplete tests pass **56/56**; full TUI passes **56 files / 438 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate §0.3 review returned **REVIEW: PASS**. Commit `120ebc62` is pushed to `origin/main`; CI is superseded by the subsequent push. Read behavior, Goal selection, project/auth scope, approvals, cancellation, idempotency, and conversation boundaries remain unchanged. No excluded files are touched.


## 2026-09-17 — UX loop Concertmaster report get Goal autocomplete

- `/concertmaster-report get` is palette-visible and has a live read handler, but `actionOptions` defines only `concertmaster-report generate`; `read-commands.ts:319-322` calls `selectedGoal(command, context)`, so explicit `--goal-id` is supported by the handler but undiscoverable in autocomplete.
- Candidates: (1) add static metadata `concertmaster-report get: [--goal-id]`, (2) add an empty metadata entry and rely only on selected context, or (3) require explicit `--goal-id` in the read handler. Independent design critique ranked **1 > 2 > 3**, selecting #1 as the smallest behavior-preserving repair; #2 needlessly hides a supported override and #3 regresses context fallback.
- RED failed **1/57**; GREEN adds only the `concertmaster-report get: [--goal-id]` metadata. Focused autocomplete tests pass **57/57**; full TUI passes **56 files / 439 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate §0.3 review returned **REVIEW: PASS**. Commit `5eda66b2` is pushed to `origin/main`; CI is superseded by the subsequent push. Read behavior, Goal selection, project/auth scope, approvals, cancellation, idempotency, and conversation boundaries remain unchanged. No excluded files are touched.


## 2026-09-17 — UX loop Improvement digests list Goal autocomplete

- `/improvement-digests list` is palette-visible and read-supported (`registry.ts:347-351`, `read-commands.ts:84-90`); its live handler (`read-commands.ts:324-329`) calls `selectedGoal` and then a project-scoped client read, but `actionOptions` has no entry. Explicit `--goal-id` is therefore supported but undiscoverable.
- Candidates: (1) add static metadata `improvement-digests list: [--goal-id]`, (2) also advertise ignored `--project-id` for registry consistency, or (3) add `--goal-id` plus broader `requiresGoal` metadata. Independent design critique ranked **1 > 3 > 2**, selecting #1 as the smallest truthful repair; #2 creates a false no-op contract and #3 is broader than this omission.
- RED failed **1/58**; GREEN adds only the `improvement-digests list: [--goal-id]` metadata. Focused autocomplete tests pass **58/58**; full TUI passes **56 files / 440 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate §0.3 review returned **REVIEW: PASS**. Commit `63deb477` is pushed to `origin/main`; CI is superseded by the subsequent push. digest read behavior, Goal selection, project/auth scope, approvals, cancellation, idempotency, and conversation boundaries remain unchanged. No excluded files are touched.


## 2026-09-17 — UX loop Critical action approve-and-run autocomplete

- `/critical-action approve-and-run` is palette-visible as a critical action (`registry.ts:112-117`) and live in `TUI_WRITE_COMMAND_KEYS`; `write-commands.ts:526-529` dispatches it. The handler consumes `--goal-id` through `selectedGoal`, required `--action`, `--target`, `--version`, `--budget-effect-cents`, and `--expires-at`, plus optional/generated `--command-id`, but `actionOptions` has no entry.
- Candidates: (1) add exact static metadata `critical-action approve-and-run: [--goal-id, --action, --target, --version, --budget-effect-cents, --expires-at, --command-id]`, (2) co-locate this option tuple on `CommandAction` and derive autocomplete from it, or (3) introduce a typed command spec shared by registry, autocomplete, and handler validation. Independent design critique ranked **2 > 1 > 3**; #1 is selected for this bounded fix because #2 requires migration of nearby metadata and #3 is disproportionate to one omission.
- RED failed **1/59**; GREEN adds only the exact `critical-action approve-and-run: [--goal-id, --action, --target, --version, --budget-effect-cents, --expires-at, --command-id]` metadata. Focused autocomplete tests pass **59/59**; full TUI passes **56 files / 441 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate §0.3 review returned **REVIEW: PASS**. Critical approval, Goal selection, project/auth scope, version/expiry/budget validation, cancellation, idempotency, and conversation boundaries remain unchanged. No excluded files are touched.

## 2026-09-17 — UX loop Approval alias autocomplete

- `/approval approve-and-run` is a live palette action (`registry.ts:271-273`) and its exact `${name} ${action}` lookup (`registry.ts:356-365`) has no literal `approval approve-and-run` metadata. `write-commands.ts:109-115` includes the alias, and the shared handler (`write-commands.ts:526-529`) consumes `--goal-id`, `--action`, `--target`, `--version`, `--budget-effect-cents`, `--expires-at`, plus optional/generated `--command-id` (`write-commands.ts:119`).
- Candidates: (1) add a direct alias map entry with the exact seven options, (2) define one readonly `CRITICAL_APPROVAL_OPTIONS` tuple and reference it from both canonical and alias entries, or (3) resolve an explicit alias map to the canonical metadata key. Independent design critique ranked **2 > 1 > 3**, selecting #2 as the best small drift-prevention repair; #1 is clearest but duplicates values, and #3 adds indirect lookup logic.
- RED failed **1/60**; GREEN shares only the readonly `CRITICAL_APPROVAL_OPTIONS` tuple between canonical and alias metadata. Focused autocomplete tests pass **60/60**; full TUI passes **56 files / 442 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate §0.3 review returned **REVIEW: PASS**. Dispatch, critical approval, Goal selection, project/auth scope, validation, cancellation, idempotency, and conversation behavior remain unchanged. No excluded files are touched.


## 2026-09-17 — UX loop Git goal-branch scope and idempotency autocomplete

- `/git goal-branch` is palette-visible and live in `TUI_WRITE_COMMAND_KEYS`; its handler consumes explicit `--goal-id` through `selectedGoal` and `--command-id` through `commandId`, but registry metadata exposed only branch inputs. These two supported options were undiscoverable.
- Candidates: (1) update the literal metadata to `[--goal-id, --repository-path, --branch-name, --base-revision, --command-id]`, (2) compose the entry from immutable Goal-scope/idempotency option constants plus branch inputs, or (3) derive autocomplete from typed handler-declared option metadata. Independent design critique ranked **1 > 2 > 3**, selecting #1 as the smallest exact behavior-preserving repair; #2 reduces future drift but adds abstraction for one entry, and #3 has the largest scope and regression risk.

## 2026-09-21 — Plan 5 status sync and actual-cost integration coverage gap closed

- Reviewed `execution/plan-5.md` against the actual repository state. Slices S1–S5 (`ccee3019` isolation proof, `531f4b5a` capacity inventory/queue, `c0bccc3f` safe pause/resume, `7f43a92d` Portfolio Council decision, `d2bff458` scenario harness) are merged, but `roadmap/act-1-foundation/phase-05-concurrent-goals-portfolio.md` and `docs{,/en,/ko}/05-roadmap-and-phase-status.md` still carried the pre-S1 2026-09-08 status text describing Portfolio Council and capacity reservations as absent. Corrected all four documents plus a superseded-pointer in `roadmap/act-1-foundation/active/phase5-execution-slices.md` to describe the merged S1–S5 state while explicitly not claiming the §5 HANDOFF live acceptance, which remains open and unchecked in `execution/PENDING_LIVE_CHECKS.md`.
- Closed the `packages/persistence/src/actual-cost.ts` integration-test gap plan-5 §1 flagged (unit-tested against a mocked pool only, no real-PostgreSQL gate, unlike sibling `budget-reservation.ts`/`capacity-reservation.ts`). Added `actual-cost.integration.test.ts` covering: recording and ordered listing, idempotent replay of an identical command, rejection of a reused command id with different content, rejection under a stale fencing token with the ledger left unchanged, the `goal_actual_costs_immutable` trigger rejecting direct UPDATE/DELETE, and cross-Goal isolation of the ledger.
- Verification performed this session: `npx tsc -b` (full workspace build) passes clean; `npx vitest run --exclude '**/*.integration.test.ts' --exclude '**/*.property.test.ts'` passes 2034/2044 (9 skipped, unrelated to this change) with one pre-existing unrelated failure in `packages/local-backend/src/surface.test.ts` (a barrel-export snapshot stale against uncommitted, in-progress `apps/carnegie`/`local-bootstrap.ts` work already present in the working tree before this session — not touched, out of plan-5 scope, flagged to the user). No `MAESTRO_TEST_DATABASE_URL` was available in this environment (no Docker/Postgres), so the new `actual-cost.integration.test.ts` and all sibling `*.integration.test.ts`/phase5-scenario suites ran in their documented skip mode only; real-PostgreSQL execution of the new test remains a follow-up when a database is available. No live/production acceptance of plan-5 is claimed.

## 2026-09-22 — plan-E8: native ChatGPT Codex OAuth + Responses API provider (no external `codex` binary required, real tool-calls)

**Why:** plan-E7's live acceptance run discovered that Maestro's only ChatGPT Codex integration (`packages/model-provider-openai/src/codex-app-server.ts`, spawning an external `codex app-server` subprocess and speaking its JSON-RPC protocol) hard-rejects any tool-bearing turn by design (`runTextTurn`: `if (input.tools.length > 0) throw ...`). Since the goal-less Concertmaster conversation always grants the `task-contract:create` tool, this made Codex categorically unable to complete that flow. Investigating `~/prime-agent` (a separate, unrelated CLI project on this machine used only as a read-only structural reference, same as the Prime Agent benchmark already cited elsewhere in this repo's plans) showed the same ChatGPT Codex OAuth grant fully supports tool calls when the *Responses API* (`https://chatgpt.com/backend-api/codex/responses`) is called directly with the OAuth access token, bypassing `codex app-server` entirely. The operator asked for this ported into Maestro natively — not by shelling out to Prime Agent or depending on any external `codex` executable being installed.

**What was built** (worktree `.worktrees/e8-codex-responses`, branch `e8-codex-responses`, based on `main`, not yet merged/committed to `main`):

- `packages/model-provider-openai/src/codex-oauth.ts` — `CodexOAuthClient`: drives the ChatGPT Codex OAuth PKCE flow directly (same public `client_id` and endpoints the `codex` CLI itself uses — `auth.openai.com/oauth/{authorize,token}`, redirect `http://localhost:1455/auth/callback`), with a real local HTTP callback server, no external binary. Structurally compatible with `GatewayOptions.codex` (same `startChatGptLogin`/`loginStatus`/`cancelLogin`/`close`/`logout` shape as `CodexAppServerClient`), plus a native-only `getCredentials(loginId)` for retrieving the real token pair to persist, and `refresh(refreshToken)`.
- `packages/model-provider-openai/src/codex-responses.ts` — `CodexResponsesProvider`/`createCodexResponsesPlugin`: a real `ModelProviderPort` calling the Responses API directly over SSE, with real tool support (`tools`, `tool_choice: "auto"`, `parallel_tool_calls: true` in the request; `response.output_item.added|done` / `function_call_arguments.delta` parsed into `ModelToolCall`s and emitted as `tool-proposed`). Capabilities include `tool-calls`, unlike the app-server path's catalog.
- `apps/model-gateway/src/codex-token-resolver.ts` — `createCodexAccessTokenResolver`: reads the stored token pair, transparently refreshes it inside a buffer window before expiry, and re-persists the refreshed pair; concurrent calls for the same account share one in-flight refresh (refresh tokens rotate on use, so two parallel refreshes with the same stale refresh token would otherwise race).
- `apps/model-gateway/src/gateway.ts`: `accountLoginStatus` now persists the *real* token pair (via `credentials.bind()`) when the configured `codex` client exposes `getCredentials()` (the native path), falling back to the previous metadata-only `bindManaged()` for the legacy app-server path (which keeps its own credential secret in the external process, by design). Persists once per succeeded login, not on every status poll.
- `apps/model-gateway/src/credential-store.ts`: relaxed `parseStoredCredential`'s invariant, which previously rejected *any* `managed-subscription` credential carrying a secret (`"Managed accounts have no Maestro secret"` — true for the app-server path, no longer true for native OAuth, which must persist a real token pair). Also hardened `list()` so one corrupted/incompatible keychain entry can no longer throw and break `/v1/models` for every other provider of that operator.
- `apps/model-gateway/src/main.ts`: **Codex support now defaults to the native path** (no `MAESTRO_CODEX_APP_SERVER_COMMAND` or external binary required). Set that env var to explicitly opt into the legacy app-server subprocess bridge instead, or `MAESTRO_CODEX_DISABLE=true` to disable Codex support entirely.

**Independent review (code-review skill, `high`) found six real issues, all fixed before this entry, each with a RED→GREEN test:**
1. **Critical:** `toResponsesInput()` skipped every `role: "tool"` message *before* extracting its tool-result content, so `agent-runtime.ts`'s `toolMessage()` output (always `role: "tool"`) never reached the wire — function_call_output was silently dropped on every multi-turn tool exchange, defeating the entire point of this plan. Fixed by removing the early skip (the existing `text !== ""` guard already prevents ever emitting a bogus `role:"tool"` text item).
2. **Critical:** the new real-secret `managed-subscription` credential failed `parseStoredCredential`'s validation on any process restart (in-memory cache masked it same-process), and the uncaught exception inside `list()`'s per-binding hydrate loop broke `/v1/models` for *every* provider of that operator, not just Codex. Fixed both (see above); round-trip covered by a fresh-instance test simulating restart.
3. `CodexOAuthClient` had no `logout()`, so `logoutAccount` always failed under the new default config. Added a documented no-op (nothing to revoke server-side without a published Codex token-revocation endpoint; the credential store's own `revoke()` already clears the persisted secret).
4. `startChatGptLogin()` resolved with an authUrl before the local callback server had actually bound port 1455, so a concurrent second login could get a doomed URL if the port was already taken. Fixed by awaiting the real `listening` event (or rejecting on `EADDRINUSE`) before returning.
5. Two concurrent token resolutions racing near expiry could both spend the same (rotating) refresh token. Fixed with an in-flight-flight map keyed by `accountRef`, registered synchronously before any `await` so a same-tick concurrent call reliably observes it.
6. `accountLoginStatus` re-persisted the full token pair (a real keychain write) on every single status poll while already succeeded, not only on the pending→succeeded transition. Fixed with a per-`loginId` persisted-once guard.

**Verification:** focused suites for all new/changed files pass (`codex-oauth.test.ts` 8/8, `codex-responses.real-http.test.ts` 5/5 against a real local HTTP/SSE server — not mocked fetch, `codex-token-resolver.test.ts` 6/6, `credential-store-managed-secret.test.ts` 2/2, `gateway.test.ts`/`main.test.ts` updated for the new default-on Codex registration). Full `npx tsc -b --force` (whole workspace) passes clean; changed-file `eslint` passes; full non-integration `npx vitest run --exclude '**/*.integration.test.ts' --exclude '**/*.property.test.ts'` passes 311/312 files, 2147/2156 tests (9 skipped), with the one failing file being the same pre-existing, unrelated `packages/persistence/src/ipython-session-journal.test.ts` `ECONNREFUSED` hazard already recorded in `task_plan.md`'s Errors-encountered table (a stale `MAESTRO_TEST_DATABASE_URL` surviving in this session's shell snapshot, pointing at a Postgres container that no longer exists — not caused by this change).

**Not yet done:** a real live OAuth login + tool-bearing conversation turn through this new path (the actual point of building it) — that requires the operator to complete a fresh browser consent, since this is a completely separate credential store from the app-server path's `~/.codex/auth.json`. Tracked separately; see `execution/PENDING_LIVE_CHECKS.md`.
