...
- RED added `list --` and `list --g` regressions and failed **1/36** because the Goal scope option returned no completion. GREEN adds only `--goal-id` metadata without changing selected-goal resolution or read dispatch. Focused autocomplete tests pass **36/36**; full TUI passes **56 files / 418 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate § 0.3 review returned **REVIEW: PASS**. Commit `cfb6f71` is pushed to `origin/main`; CI `35195167285` remains in progress.
