...
- RED added `observe --` and `observe --wor` regressions and failed **1/35** because the required worker identity returned no completion. GREEN adds only `--worker-id` metadata without changing observe dispatch or runtime validation. Focused autocomplete tests pass **35/35**; full TUI passes **56 files / 417 tests**, build including renderer typecheck, changed-file lint, and `git diff --check` pass; separate § 0.3 review returned **REVIEW: PASS**. Commit `0cc1e35` is pushed to `origin/main`; CI `35192382419` remains in progress.
- Scope is registry-owned TUI autocomplete metadata/test only; no `testbed/`, `apps/carnegie/src`, backend/domain/authority/persistence, Muze, §S11, or G9 changes; no force-push.
