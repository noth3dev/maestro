...
## 2026-09-19 — Dogfood loop: Pillar 1 real TUI capture and blind review

- Built `scripts/tui-capture.mjs` and RED/GREEN coverage in `scripts/tui-capture.test.ts`. Focused suite passes 8/8; `node --check scripts/tui-capture.mjs` passes; `git diff --check` is clean.
- Real tmux evidence: `help` at `80x24` completed with two identical ANSI/plain hashes. Full matrix completed at `80x24`, `120x40`, and `200x50`, each with color and `NO_COLOR`; all six cases completed two-run reproducibility checks. Artifacts are under `.artifacts/e4-pillar1-matrix` in the isolated worktree.
- Capture safety: generated owned sessions use `randomUUID`, caller-selected sessions are not accepted, child launch uses `env -i` with an allowlist and isolated temporary `HOME`, default captures do not inherit Maestro credentials, and cleanup errors are surfaced.
- Re-read references: `openai/codex` TUI guidance requires real interactive PTY use and separate text/Enter writes; its snapshot keeps 80-column content explicit. `PrimeIntellect-ai/prime-agent` requires rendered lines to stay within width and provides virtual-terminal coverage; `NousResearch/hermes-agent` tests real PTY passthrough and isolated homes; `anomalyco/opencode` uses repeatable visual capture and focus regression scenarios.
- Blind critique verdict: NEEDS-CHANGES. Captures show a context-free `80x24` state, a repeated warning plus blank non-actionable prompt, unmarked/unselected sidebar rows, truncation at 120/200 columns, inconsistent warning copy, large dead whitespace, and an over-wide empty-state card. The next layout finding will start with the disconnected/empty state and must preserve this six-case capture gate. No live authenticated flow was claimed in this entry.


## 2026-09-19 — Dogfood loop: E4 setup-state and live conversation recheck

- Correction to the prior incomplete record: the setup-required/layout finding was not closed until all three pillars were run. Pillar 1 is now backed by reproducible six-case `home` captures in `.artifacts/e4-pillar1-final-layout` and `.artifacts/e4-pillar1-post-layout` at `80x24`, `120x40`, and `200x50`, in both color and `NO_COLOR`. Each `capture.json` reports `reproducible: true`; the frames retain the warning, session state, `No activity yet.`, restart guidance, composer, and footer without clipping.
- Pillar 1 also includes the `/models list` input evidence under `.artifacts/e4-pillar3-models-input`. The command helper now closes autocomplete with `Escape`, waits 50 ms, then submits with `Enter`; the helper capture contains the live model catalog, including `openai-codex/gpt-5.6-luna`. This fixes the capture sequence, not production command semantics.
- Pillar 2 was independently reviewed from rendered frames only. Verdict: `PASS`. The reviewer confirmed bounded command wrapping, readable color and `NO_COLOR` frames, usable 80-column compact layout, full wider layouts, and no broken final state after the transient reconnect notice. The live help frames are not byte-identical across color and `NO_COLOR` at 120x40/200x50 because live sidebar/activity rows differ; the reviewer judged geometry and legibility unaffected and treated this as timing/state variance, not a color failure. Exact reviewed frames include `.artifacts/e4-pillar1-post-layout/{80x24,120x40,200x50}-{color,no-color}/home.txt`, `.artifacts/e4-pillar1-live-matrix/{80x24,120x40,200x50}-{color,no-color}/help.txt`, and `.artifacts/e4-pillar3-conversation/{model-selected.txt,agent-response.txt}`. The reviewer had no local reference checkout and invented no reference claims.
- Pillar 3 completed through the real connected Maestro session without logout or credential mutation: `/models use --model openai-codex/gpt-5.6-luna`, `/session new`, then a natural-language turn. `.artifacts/e4-pillar3-conversation/agent-response.txt` records conversation `d1486ed9-bf53-45eb-a883-a013c82820ad` and the actual response: `Ready for a safe, read-only local Maestro task; filesystem writes and external actions are unavailable without authorization.` This is the first verified live agent response; prior `model none` captures remain non-success evidence.
- Reference structure was re-read and recorded in `.artifacts/e4-pillar1-final-layout/reference-structure.json`: Codex separate PTY text/Enter writes, Prime width-bounded render/focus contracts, Hermes fixed-size and fail-closed capture, and OpenCode repeated visual capture plus composer-focus regression coverage. Focused capture tests remain 8/8; `node --check scripts/tui-capture.mjs` and `git diff --check` pass.
- Remaining gates: live matrix reproducibility is still not claimed because its dynamic captures are intentionally one-pass; the Carnegie build remains blocked by the pre-existing missing `@playwright/test` dependency and implicit-`any` errors in `apps/carnegie/src/accessibility.playwright.ts`; the worktree changes and artifacts still require final review and commit/push discipline. No E4 completion claim is made until those gates are reconciled.


## 2026-09-19 — Dogfood loop: blocked setup screen layout closure

- Re-read the four reference structures for this layout finding: Codex real-TTY guidance and width-bounded snapshots; Prime TUI line-width contract and virtual terminal; Hermes PTY passthrough/isolated-home tests; OpenCode visual-stability capture and terminal focus regression.
- Real captures exposed and then rechecked the setup-required/new-session state at all six required cases. The first blind review found false sidebar/composer affordances; the second found a dead transcript viewport; the third found an ungrouped duplicate warning.
- Three candidate rounds were independently critiqued. Selected the subtractive blocked-state gate, then bounded/centered panel treatment, then single-message cleanup. RED/GREEN evidence: blocked predicate and panel tests were added; final focused suite passes 79/79, CLI `tsc -b` passes, changed-file ESLint passes, and `git diff --check` is clean.
- Final real matrix: `.artifacts/e4-pillar1-final-single-message` contains 80x24, 120x40, and 200x50 color/NO_COLOR frames. All six completed two-run reproducibility checks. The final panel is 72 columns wide, centered at x=4/24/64 and vertically balanced at y=9/17/22; no clipping or wrapping observed.
- Final blind review verdict: **PASS**. The fresh critic saw one bounded six-line setup message, truthful restart instructions, no fake controls, equal margins, and identical readable color/NO_COLOR output.
- Live Pillar 3 status: real credential/session files were present, but a non-destructive `--live /help` smoke exited before capture and produced no verified live result. This remains explicitly listed in `execution/PENDING_LIVE_CHECKS.md`; no logout, revoke, publish, approval, or write action was attempted.
- Full `npm run build` remains blocked by the pre-existing missing `@playwright/test` dependency in Carnegie renderer typecheck; root/CLI `tsc -b` passed.


## 2026-09-19 — E4 verification recheck
...
## 2026-09-19 — Dogfood loop: live startup and read-only session verification

- Live diagnosis: a fixed 1.5/3-second delay did not prove that the authenticated TUI was interactive. The helper now keeps the 1.5-second initial delay and, for `--live`, waits for the bounded `Session attached` readiness marker before scenario steps; missing readiness fails closed and kills the owned session.
- RED/GREEN: readiness appeared only after repeated pane probes in the fake-tmux regression; the timeout regression proves cleanup and no scenario step before readiness. The focused capture suite passes 15/15; palette tests pass 5/5 (20/20 total). Contradictory `--reproducible --no-reproducibility` flags fail clearly.
- The command-submit regression is now observable: after `/models list`, the helper sends a second Enter only when the first probe still shows the exact command in the composer, then polls for composer clearance. No-completion commands submit once; stuck commands fail with a bounded error.
- Real live `/models list` returned the model catalog, and the following natural-language turn returned `Yes, I can handle a safe, read-only local task.` without `Unexpected argument: Reply`. Evidence: `.artifacts/e4-pillar3-live-help-conversation/{models,conversation}.{txt,ansi.txt}`.
- The explicit `--no-reproducibility` matrix mode captured six live `/help` cases in `.artifacts/e4-pillar1-live-help-final3`; each result is `reproducible: false`. The live frames preserve color/NO_COLOR hashes and show the composer at all sizes. The first blind review found the inventory's vertical truncation at 80x24/120x40; the selected scroll-cue fix adds `More commands above · PgUp/PgDn scroll` as plain text. A fresh blind review is **PASS** for all six cases and the debug frames.
- Pillar 3 also exercised the cue: `.artifacts/e4-pillar3-live-help-pageup-verify/{help-end,help-pageup,help-pagedown}.txt` shows the live `/help` list, `PageUp` reaches the earlier `Commands:` beginning, and `PageDown` returns to the tail/composer. Dynamic authentication differences are recorded as transient, not layout claims.
- The live evidence was compared against the current Codex, Prime Agent, Hermes Agent, and OpenCode reference structures recorded in `.artifacts/e4-pillar3-live-help-reference-structure.json`; no unobserved reference behavior is claimed. Broader Goal creation, approval, Council/DepartmentPlan/Worker, publish, and release-runbook flows remain pending.


## 2026-09-19 — Dogfood loop: live command catalog readability

- Fresh real 80x24 `/help` capture found a dense unheaded command wall. Three candidates were considered: (A) remove the catalog behind a short hint, (B) retain the catalog with READ/WRITE/CRITICAL grouping plus bounded rows and truthful scrollback, or (C) add only another scroll indicator. Independent design critique ranked **B > A > C**.
- RED added grouping, bounded-line, and at-a-glance assertions. GREEN keeps the full catalog in transcript scrollback, renders one command per bounded row, compresses local `OTHER` commands, and keeps a tail-visible `At a glance` summary for READ/WRITE/CRITICAL representatives. The cue now says `Catalog above · PgUp/PgDn scroll`; it does not invent page state.
- Real live verification from committed source `df3c26ec`: `.artifacts/e4-postcommit-live-help` is reproducible at 80x24, and `.artifacts/e4-postcommit-live-help-matrix` contains six cases (80x24, 120x40, 200x50, color and `NO_COLOR`), each explicitly `reproducible: false`. The pre-commit artifacts are superseded and are not used as final evidence.
- Fresh blind review is **PASS**: no clipping or wrapping, visible group summary, truthful scroll cue, separated status/composer, and monochrome readability.
- Final focused gate: `scripts/tui-capture.test.ts` 15 tests plus `apps/cli/src/tui/commands/palette.test.ts` 5 tests = **20/20**, root `tsc -b`, changed-file ESLint, `node --check scripts/tui-capture.mjs`, and `git diff --check` pass.
- Re-read the current TUI and hierarchical-execution docs and live root structures for `openai/codex`, `PrimeIntellect-ai/prime-agent`, `NousResearch/hermes-agent`, and `anomalyco/opencode`; the observed structure snapshot is `.artifacts/e4-reference-structure-current.json`. No reference behavior is claimed beyond the recorded structure.


## 2026-09-19 — Dogfood loop: narrow-sidebar overlay recovery

- Pillar 1 and blind design review confirmed a real 80x24 defect: the docked sidebar was hidden and `Ctrl+B`/`Down` produced no visible focus movement. The independent critique ranked a transient full-width overlay above a footer-only warning because keyboard users must retain at-width access to the views. The current reference structure snapshot `.artifacts/e4-pillar3-live-help-reference-structure.json` was re-read for fixed-size capture, line-width, focus/cursor, and key-boundary guidance; it does not claim an existing narrow-overlay implementation.
- RED/GREEN changes add a narrow-screen sidebar overlay. `Ctrl+B` opens it without changing the dock visibility preference; `Down` moves `› search` to `› inbox`; `Enter` activates the selected row; `Escape` restores the composer. The docked sidebar behavior remains unchanged at 120x40 and 200x50. The overlay omits the repeated status block so its `up/dn` and `esc` hints remain visible at 80x24.
- Adversarial code review found and the final patch closed two regressions before delivery: modal-owned main-pane content now suppresses the overlay, and the overlay uses the same click-region dispatch as the docked sidebar. The main footer now keeps the `ctrl+b sidebar` discovery cue in both narrow overlay and wide docked modes; predicate and hint tests cover these paths.
- Fresh live evidence is in `.artifacts/e4-pillar1-live-sidebar-overlay` for all six 80/120/200 color/NO_COLOR cases, intentionally marked one-pass/non-reproducible. Every frame is width-bounded; 80x24 shows visible search→inbox focus movement and both keyboard hints; larger captures retain docked behavior. `.artifacts/e4-pillar1-live-sidebar-overlay-enter` records live Enter activation and Escape restoration in both 80x24 color modes.
- Fresh blind review is **PASS**. It verified all 18 plain frames, focus movement, footer hints, composer restoration, ANSI/plain agreement, and line-width limits. Focused verification passes 150/150 TUI tests, CLI `tsc -b`, changed-file ESLint, and `git diff --check`; the known full-build Carnegie `@playwright/test` blocker remains unchanged.


## 2026-09-19 — Dogfood loop: narrow sidebar focus overlay

- Real live `Ctrl+B` captures exposed the narrow-width behavior at 80x24 and the normal-width behavior at 120x40. At 80 columns, the focused navigation surface becomes full-width and removes the cramped composer; at 120 columns, the normal conversation/composer remains coherent.
- Three candidates were compared: (A) keep the full-width navigation-only overlay with visible `esc unfocus · ctrl+b` recovery, (B) keep a dock beside the composer at 80 columns, or (C) hide the sidebar and let input fall through. Independent design critique ranked **A > B > C**; the subtractive C was rejected because it hides navigation capability without a recovery path.
- GREEN behavior is covered by the lifecycle sidebar tests: focus, up/down movement, Enter activation, Esc recovery, width restoration, and narrow Ctrl+B toggle. The real color and `NO_COLOR` 80x24 frames show the selected row and recovery legend.
- Fresh blind review is **PASS**: no clipping, clear focus ownership, explicit recovery, intentional width adaptation, and acceptable navigation-only blank space.
- Initial verification: loop suite 11/11, `tsc -b`, changed-file ESLint, and `git diff --check` passed before the final committed delivery.
...
## 2026-09-19 — Dogfood loop: nav destination page validation

- Pillar 1 captured all six sidebar destinations (`home`, `inbox`, `channel`, `evidence log`, `billing`, `luthiery`) at `80x24`, `120x40`, and `200x50`, in color and `NO_COLOR`: 36/36 live capture cases completed and 72 plain-frame integrity checks passed. Evidence: `.artifacts/e4-pillar1-live-nav-pages/matrix.json` and the per-destination `focused.txt`/`after.txt` frames. Dynamic captures remain one-pass and are not claimed reproducible.
- The matrix proves frame capture, focus/sidebar geometry, and bounded rendering; it does **not** prove 36 semantic navigation successes. The original after frames contain historical literal `C-b C-b` conversation text, `Model turn outcome is unavailable`, and dynamic auth/resumability/history errors. A clean explicit-key 80x24 revalidation (`C-b`, repeated `DOWN`, `Enter`, `Escape`) directly showed Billing and Evidence Log destination output, and separately confirmed the Goal-scoped Channel empty state; search/inbox remain auth/history-limited and Luthiery truthfully reports its future Phase 9 registry limitation. These are evidence limits, not fabricated page success or demonstrated product defects.
- The destination model was checked against the re-read docs; this validation adds no channel behavior or creation flow. `roadmap/act-1-foundation/phase-02-hierarchical-execution.md`, `roadmap/act-1-foundation/specs/2026-09-09-tui-design.md`, and `docs/02-hierarchical-orchestration.md` were re-read.
- The four current reference structures were re-read and recorded in `.artifacts/e4-reference-structure-nav-pages-current.json`: Codex interactive PTY boundaries, Prime width/focus/overlay contracts, Hermes fixed-size fail-closed capture, and OpenCode repeated visual capture plus composer-focus restoration.
- Pillar 2 blind review is **PASS**, recorded in `.artifacts/e4-pillar2-nav-pages-blind-review.md`: the rendered frames showed explicit focus, distinct page bodies where live state settled, bounded narrow/wide frames, and no layout change required. No product code changed for this validation finding.


## 2026-09-19 — Dogfood loop: authenticated color/NO_COLOR parity recheck

- Pillar 1 ran a fresh live `200x50` paired capture in color and `NO_COLOR`, with two frames per mode. All four plain frames are exactly 50 lines, at most 200 columns, and match their recorded hashes; initial and settled frames are byte-identical within each mode. Evidence: `.artifacts/e4-live-auth-parity-recheck/matrix.json` and the four mode frames.
- The cross-mode content is not semantically comparable in this run: color shows repeated authentication-unavailable status content, while `NO_COLOR` shows the draft Goal/worker/budget state and a different event/UUID presentation. The lower session/composer area, focus prompt, box, project/event lines, and row bounds remain stable. This does not demonstrate a color-rendering defect or backend parity.
- Three handling options were considered: change rendering to force cross-mode content equality, repeat after backend state normalizes, or accept geometry parity while keeping semantic parity open. The blind review selected the evidence-only third option, with normalization as the only safe follow-up; no product code changed.
- Pillar 2 blind review is **PASS**, recorded in `.artifacts/e4-pillar2-auth-parity-blind-review.md`: no clipping, overflow, focus drift, or layout drift was found. Dynamic backend state remains an explicit limitation.
- Pillar 3 used the real connected Maestro session and preserved the read-only agent conversation visible in both modes; no logout, credential mutation, approval, write, development delegation, or pipeline execution was attempted for this parity-only check, so broader live development gates remain unclaimed. Reference structure evidence was re-read for capture/focus contracts in `.artifacts/e4-reference-structure-color-parity-recheck.json`.


## 2026-09-19 — Release runbook deterministic harness recheck

- Re-ran the deterministic release-scenario harness instead of claiming the live provider gate: the fourteen-step fake-provider path passed through `test/release-scenario/fake-provider.test.ts` (2/2), fixture safety passed (3/3), and runbook contract checks passed (3/3), for 8/8 tests. The duplicate persisted-effect successor-reconciliation case also passed. Evidence: `.artifacts/e4-release-fake-tests.log`.
- This verifies the disposable fixture mechanics and fake-provider assertions only. No live provider, Goal creation, approval, worker execution, publish, remote push, or external side effect was attempted. The live Plan 3/4/5 gates remain open because the runbook requires explicit disposable-root, API-token, project, and model environment inputs that are not present in this session.
- The full build gate was rechecked with `set -o pipefail`: root/CLI `tsc -b` reached the Carnegie renderer, then failed with the known missing `@playwright/test` module and dependent implicit-`any` diagnostics in `apps/carnegie/src/accessibility.playwright.ts` (exit 2). Evidence: `.artifacts/e4-build-heartbeat-recheck.log`. No source workaround or dependency installation was made.


## 2026-09-20 — Dogfood loop: nav evidence-quality correction

- Re-read the full E4 loop plan, the Phase 2/channel/TUI docs, and the four reference repository structures. The reference comparison again supports explicit PTY key boundaries (Codex), width/focus/overlay contracts (Prime), fail-closed fixed-size capture (Hermes), and repeated visual capture with composer restoration (OpenCode). Snapshot: `.artifacts/e4-reference-structure-nav-recheck.json`.
- Review of the prior nav matrix found that `ok: true` only proved process/capture completion. The original after frames contain historical literal `C-b C-b`, `Model turn outcome is unavailable`, and dynamic auth/resumability/history errors; they cannot support a broad claim of 36 semantic navigation successes.
