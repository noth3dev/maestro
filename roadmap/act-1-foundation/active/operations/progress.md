...
- Task 4 Step 1 started after the Task 1.5 checkpoint: added RED coverage for durable Goal loading, initial selection, explicit selection persistence, selection fallback when a Goal disappears, refresh/reconnect reloads, and project scoping. The planned `goal-operations.ts` module is intentionally still absent, so the focused Task 4 command is RED only in `goal-operations.test.ts` while existing Goal-control/Dashboard tests remain green.
