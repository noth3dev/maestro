...
- 2026-09-15 Plan 8 §S4 migration parity: RED reproduced on pre-0104 base `3f83728`: the production `runMigrations` path applied only through migration `0103`, so no audit privacy triggers existed. Added `test/phase8-data/migration-parity.integration.test.ts` to verify production application of `0104`, both audit triggers, safe inserts, bearer-content rejection, and second-run idempotency. Current focused PostgreSQL verification passed **1/1**; independent exact-diff no-edit review returned **REVIEW: PASS**; `npm run lint` and `git diff --check` passed; package build passed after linking the existing workspace `apps/cli/node_modules` dependency directory in the disposable worktree. No production implementation changed.
