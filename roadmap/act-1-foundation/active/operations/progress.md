...
- Pillar 1 captured all six sidebar destinations (`home`, `inbox`, `channel`, `evidence log`, `billing`, `luthiery`) at `80x24`, `120x40`, and `200x50`, in color and `NO_COLOR`: 36 live cases and 72 plain-frame integrity checks passed. Evidence: `.artifacts/e4-pillar1-live-nav-pages/matrix.json` and the per-destination `focused.txt`/`after.txt` frames. Dynamic captures remain one-pass and are not claimed reproducible.
- The artifact set contains focused/after captures for all six destinations across `80x24`, `120x40`, and `200x50` in color and `NO_COLOR`; focused frames show selection and after frames show destination output where live state settled. At 80x24, settled live bodies exposed distinct destinations for conversation/home, the Goal-scoped channel result, Events, Billing, and Luthiery; the inbox destination capture completed but its body was limited by a live conversation-resumability error. Several wide cases had the same resumability/auth/history limits. These are recorded as evidence limits, not fabricated page success.
