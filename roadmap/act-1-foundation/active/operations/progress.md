...
- 2026-09-15 Plan 7-e §S3 Codex auto-detection and login diagnosability: added PATH-based executable discovery (explicit `MAESTRO_CODEX_APP_SERVER_COMMAND` remains authoritative), verified the actual spawned gateway environment, and propagated bounded spawn-vs-transport detail while preserving `provider_unavailable`. Focused verification passed **87 passed / 1 intentional skip**; build, ESLint, and `git diff --check` passed. The real installed Codex gate invoked `codex login status` but remains blocked by unavailable credentials (`Not logged in`, exit `1`; app-server `authMode: null`); the prior transient authenticated result is not reproducible, so no live-provider acceptance is claimed.
