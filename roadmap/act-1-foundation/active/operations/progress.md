...
- Adversarial code review found and the final patch closed two regressions before delivery: modal-owned main-pane content now suppresses the overlay, and the overlay uses the same click-region dispatch as the docked sidebar. A narrow footer cue (`ctrl+b sidebar`) is also visible after Escape; predicate and hint tests cover these paths.
...
- Verification: loop suite 11/11, `tsc -b`, changed-file ESLint, and `git diff --check` pass. The change remains uncommitted pending §0.3 review.
...
- Follow-up simplification: a footer-only `ctrl+b sidebar` hint was visible in unit output but not in the full-width overlay frame, while the overlay already exposes `esc unfocus · ctrl+b`; the hint-only shell/view edits were removed. The final change keeps only the tested overlay predicate, layout routing, and lifecycle focus behavior. Aggregate focused verification after simplification passed 3 files / 79 tests, `tsc -b`, changed-file ESLint, and `git diff --check`.
