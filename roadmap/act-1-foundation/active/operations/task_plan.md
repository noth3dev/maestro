...
- **Phase 1:** native-only composition, no-Prime scan, Model Gateway HTTP acceptance, Control Plane + PostgreSQL + native Worker acceptance, S1/S2/S3 implementation evidence, and the clean main serialized PostgreSQL run (**189/189 files, 1,258/1,258 tests**, exit 0) are evidenced. G6 is closed by `apps/control-plane/src/tui-sse-reconnect.integration.test.ts`, `apps/cli/src/tui/activity-stream.ts`, and `packages/api-client/src/index.ts`, with main revalidation log `/tmp/maestro-s3-main-revalidation.log` and CI run `34293072671` at SHA `2fa6168`. Final phase acceptance remains open until the Phase 2 host-tool live path and shared boundary are independently verified.
- **Phase 2:** hierarchical building blocks and PostgreSQL evidence exist, but phase acceptance is reopened. The approved next slice is a Prime-style persistent IPython host tool for local files, Git, tests, shell, and local environment changes, with independent → Department Head → Encore Council → user approval tiers, selectable repetition scope, two full-access modes, live stop, and whole-block highest-risk handling. Production `ToolRegistry` now contains one typed `ipython` definition, and the read-only persistent bridge is authority-composed with numeric policy/control context. Native admissions still default to `allowedTools: []`; local writes, approvals, durable effect evidence, and live worker acceptance remain closed/open; workers remain text/evidence-only.
- **Phase 3:** Metronome, Encore, certification/reporting, CLI/API parity, conversation, and restart/fencing evidence exist. TUI parity/reconnect evidence is complete through the Phase 1 G6 gate. First usable release remains unaccepted until the Phase 2 host-tool live flow and independent release review pass together.
...
- **Documentation checkpoint:** the native host-tool hardening, native model-policy enforcement, Council timing correction, and Carnegie GUI branding commits are on synchronized `main`/`origin/main`. Prime remains structural reference only; no Prime runtime or adapter is production code. Plan 1 S4 records the completed G6 evidence in the Phase 1 status documents and this canonical block.
- **Immediate work:** complete the remaining reopened Phase 1 Ensemble Router gates before Phase 2 router or approval/effect implementation: production selector/native-admission wiring, fixed-model evidence migration, and the native admission/live gates. A–E metric schemas, pure A↔D/B/C selector logic, eight-axis scoring rubric, recipes, continuous pressure function, four bands, baseline/overlay persistence, structured routing-evidence rejection persistence, and database immutability fences now exist. Do not claim Phase 1–3 acceptance until the artifact, admission, and live gates pass.
