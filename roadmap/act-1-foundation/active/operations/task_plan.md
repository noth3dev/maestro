...
- **Phase 2 status:** S6 is complete at the code/test/live-evidence level in this worktree. Overall Phase 2 is not yet accepted because the production write-command/API surface and independent release review remain open.
- **Phase 3 status:** existing certification/reporting and read parity remain available, but production write-command routes for Task Contract/Council/Plan/worker/effect lifecycle are still the next usability gap. Do not claim a usable release until those writes, CLI/API parity, and release review are proven.
- **Phase 4+:** external capabilities, device/browser/deployment access, payments, and future phases remain separately gated and are not enabled by S6.
- **Verification after documentation update:** `npm run build`, `npm run lint`, and `git diff --check` all passed.
- **Immediate next step:** create the required local S6 checkpoint commit after direct diff review; then implement the smallest production write-command slice needed to drive one approved local Worker/IPython Goal through authenticated API/CLI surfaces. Keep remote push/merge and external provider login separate.
...
**Current execution status:** 1A is partially implemented: the typed tool, per-session kernel factory/queue, collision-safe Goal session identity, bounded result envelope, fail-closed production registration, host-owned command/tool-call/session identity, and focused tests exist. The production live acceptance remains open. 1B now has the versioned frame parser, ready handshake validation, out-of-band host responses, JSON-lines framing, busy/interrupt/child-close handling, injected process-kernel seam, bounded read-only host router, and an authority-backed UTF-8 workspace-file adapter with symlink/sensitive-path/output-limit checks. The real-child-tested Python bootstrap now exists with AST/dunder guards, stale-helper/session checks, handshake gating, bounded TypeScript interrupt handling, child-death evidence, a parent-death watchdog seam with parent identity checks, and per-call host binding rotation. The Git revision adapter over an injected `GitPort` now exists and is tested with canonical path checks. A production-owned detached Python channel now supplies a minimal environment, parent PID/identity, bounded stderr, and process-group teardown; durable effect evidence, production Control Plane Git composition, orphan/restart journaling, and live acceptance remain open.
...
The fresh full `npm run check` caught one timing-sensitive real-child lifecycle test: it killed the Python child after a fixed 100 ms delay, which occasionally expired during handshake and produced `handshake_timeout` instead of `child_closed`. The test now waits until the execute frame is observed before terminating the child, preserving the intended mid-cell death assertion. This is a test-harness timing correction; production behavior was unchanged.


## 2026-09-08 — process adapter type-check correction

The first full-check attempt stopped in `tsc -b` because Node's `ChildProcess` type keeps `pid` and stdio nullable even for the non-null stream return type. The adapter now narrows PID/stdin/stdout/stderr explicitly and throws before exposing a channel when any required handle is unavailable. Focused adapter/bootstrap tests pass after the correction.
...
- Main now includes migrations [`0072_ensemble_router_artifacts.sql`](../../../../packages/persistence/migrations/0072_ensemble_router_artifacts.sql), [`0073_routing_evidence_rejections.sql`](../../../../packages/persistence/migrations/0073_routing_evidence_rejections.sql), and [`0074_ensemble_router_snapshot_append_only.sql`](../../../../packages/persistence/migrations/0074_ensemble_router_snapshot_append_only.sql), plus the exported [`packages/persistence/src/ensemble-router-artifacts.ts`](../../../../packages/persistence/src/ensemble-router-artifacts.ts) adapter.
- C operational overlays and immutable per-Goal snapshots are durably stored and integrity-checked. Routing evidence now persists structured rejection reasons, compares its JSON and relational representations on read, preserves legacy 0072 rows without UPDATE, and has a real PostgreSQL integration gate.
...
## 2026-09-11 — Plan 6 S1 live remediation after v13 review

`knowledge-store-and-promotion` remains open after fresh independent `REVIEW: FAIL` at `19f075c`. The active remediation order is: (1) make the first source-loss transition valid and retention-preserving; (2) enforce immutable core identity/provenance across every appended revision; (3) durably bind independent curator authorization; (4) restore refresh and mutator replay/idempotency semantics; (5) complete SQL privacy and generalized-text bounds; and (6) bind proposal authorization to the exact payload. Each item requires a RED regression before its fix. Do not merge, delete the worktree, push, or start S2 until a fresh no-edit `REVIEW: PASS` and real PostgreSQL proof exist.
...
## 2026-09-13 — Plan 6 S1 final Gate 6 remediation
