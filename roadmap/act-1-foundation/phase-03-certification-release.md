...
> **Current status (2026-09-11):** Plan 2 (§S1–§S6c) is fully merged, closing the Phase 2 host-tool dependency this document previously blocked on — the persistent IPython host tool, four-level approval hierarchy, worker target binding/repair, and the `awaiting_repair` hold/requeue lifecycle all exist in production. `execution/plan-3.md` §S1–§S4 and §S3b are merged: routing evidence gates certification, Metronome observes approvals, Council records actual reviewer diversity, the TUI design pass landed, and §S4's release-scenario harness includes the repair-hold fixture and runbook. **One honest limitation remains recorded against §S4:** its CI fake-provider harness for the Step 9→10 repair sequence is a hand-rolled simulation, not real `WorkerService`/PostgreSQL/HTTP execution — see `roadmap/act-1-foundation/active/operations/findings.md`. Phase 3 acceptance now rests entirely on the **user-owned** fourteen-step live-provider scenario (`test/release-scenario/RUNBOOK.md`); no further agent-run slice is pending. §S5 (`phase3-status-sync`, documentation only) has not started and is gated on that live run passing. No Phase 3 release acceptance is claimed from implementation evidence alone.
>
> Historical entries elsewhere in this repository's history describing an earlier "13 live-gate steps" exit-gate closure predate the current fourteen-step scenario this document defines and are superseded by it — do not treat them as current acceptance.
...
Metronome consumes durable events and evidence from:
...
Metronome may attach evidence, challenge, request a bounded correction, or request a safe pause. It cannot choose creative product direction or certify its own challenge as resolved.

### Selective Encore Council
...
- uncertainty is high;
- Metronome or a Head submits a valid challenge;
...
4. Metronome certifies process integrity, lineage, authority, budget, evidence completeness, and no unresolved blocker.
5. Concertmaster reports success only when all required certifications bind to the same Task Contract identity and integrated revision.
...
- Concertmaster's final report.

## Concertmaster final report
...
1. Implement the Metronome event consumer and deterministic rule catalog.
...
9. Implement Concertmaster milestone and final reporting.
...
- Metronome process stops: execution continues only within configured observation-loss tolerance; beyond it, safely pause affected Goals.
- Metronome repeats a challenge: deduplicate by Goal, rule, evidence identity, and active plan version.
...
1. Every Metronome rule fires on a seeded violation and remains silent on a valid fixture.
...
2. Concertmaster creates the Task Contract through minimal intake.
