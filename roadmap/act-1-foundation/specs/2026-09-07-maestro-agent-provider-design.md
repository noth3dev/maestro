# Carnegie Agent Runtime and Model Provider Design
...
Allow Carnegie to keep its natural-language control surface while replacing both the model provider and the agent runtime without changing Control Plane authority, durable approval, worker fencing, lease, idempotency, or evidence rules.
...
- **Agent runtime:** the orchestration layer that owns sessions, tool dispatch, child invocations, cancellation, observation, and lifecycle. Examples: the existing Prime Agent runtime and the new Carnegie-owned runtime.
- **Natural-language control:** an authenticated Carnegie conversation in which a model interprets user text, uses only the tools exposed by Carnegie, and reaches the existing typed API/Control Plane path. It is not direct model authority.
...
The runtime choice is an internal implementation detail. `prime-agent on` uses the Prime Agent adapter. `prime-agent off` uses the Carnegie-owned common runtime with the selected direct model provider. The toggle never disables natural-language control.

If the selected model provider or the requested Prime Agent mode is unavailable, Carnegie reports a truthful unavailable/setup-required state. It never silently falls back to another provider or runtime.
...
CEO text
  ...
  -> Carnegie tool/policy validator
  -> authenticated Carnegie API client
...
The model cannot call PostgreSQL, persistence modules, arbitrary shell, filesystem, MCP, or child-agent APIs directly. The runtime only exposes registered Carnegie tools. Tool implementations remain host-side and use the existing authenticated API client or existing provider-neutral adapters.
...
The existing `ExecutionKernelPort` remains the Control Plane's provider-neutral worker boundary. The Prime Agent adapter and the Carnegie-owned runtime both implement it. `MaestroAgentRuntime` owns the common loop and delegates only model I/O to `ModelProviderPort`; Prime Agent remains an optional compatibility implementation selected only by the internal `prime-agent` toggle.
...
The Carnegie-owned runtime provides:
...
A direct OpenAI/Claude model therefore gets the same effective capabilities through Carnegie's runtime rather than needing Prime Agent's RLM implementation.
...
Prime Agent-specific behavior is an adapter detail. RLM child IDs, Prime session IDs, and provider credentials must not cross the domain boundary; only opaque Carnegie references and normalized observations may cross it.
...
- a new opaque Carnegie invocation reference;
...
- Process restart: use the existing worker fencing/reconciliation policy. The Carnegie runtime must not claim resume/reconnect until it has a durable provider/session binding; otherwise it reports unavailable/unknown.
...
3. Implement the Carnegie-owned runtime with one provider adapter first, using the common tool/child/cancellation contract.
...
- Prime Agent can be toggled without disabling natural-language control; `off` selects the Carnegie-owned runtime and never silently falls back.
...
- No provider-specific semantics in `@carnegie/domain`.
