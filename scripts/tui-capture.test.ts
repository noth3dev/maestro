...
describe("tui capture pipeline", () => {
  ...
  it("starts the first capture when tmux has no server socket yet", async () => {
    const calls: string[][] = [];
    let created = false;
    ...
    const runTmux: Run = async (args) => {
      ...
      if (args[0] === "has-session") {
        if (firstHasSession) {
          ...
          throw Object.assign(new Error("tmux has-session failed"), {
      ...
      if (args[0] === "capture-pane") return { stdout: args.includes("-e") ? "\u001b[31mframe\u001b[0m\n" : "frame\n" };
    ...
    expect(result.frames[0]).toMatchObject({ name: "frame", plain: "frame\n" });
    expect(calls.some((call) => call[0] === "new-session")).toBe(true);
    expect(calls.at(-1)?.[0]).toBe("kill-session");
    expect(calls.some((call) => call[0] === "kill-server")).toBe(false);
  });

  it("does not swallow an unrelated missing socket error", async () => {
    let newSessionCalls = 0;
    const runTmux: Run = async (args) => {
      if (args[0] === "has-session") {
        throw Object.assign(new Error("tmux has-session failed"), {
          stderr: "error connecting to /var/lib/app.sock (No such file or directory)\n",
        });
      }
      if (args[0] === "new-session") newSessionCalls += 1;
      return { stdout: "" };
    };
    ...
    ).rejects.toThrow("tmux has-session failed");
