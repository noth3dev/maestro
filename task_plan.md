...
- Building this test found and fixed a real production defect (see `findings.md`/`progress.md` same date): `limitsFor()` in `packages/agent-runtime/src/agent-runtime.ts` sent an unclamped `providerTimeoutMs`/`wallTimeMs` that failed the real Model Gateway wire schema for any Mission Bundle `timeCeiling` over 10 minutes -- i.e. effectively every realistic mission in this codebase's own fixtures. Fixed with a defensive clamp and a dedicated regression test; full clean single-worker real-PostgreSQL rerun after the fix: 155/155 files, 1051/1051 tests, 0 failed.
