...
## Operating protocol (read this first, every session — resume, checkpoints, subagents)

This section exists so any session or subagent picking up this project mid-stream — after an
interruption, a context reset, or a brand-new session days later — behaves identically to the
session before it. Read this section before doing anything else, then read "Next step" below.

### A. Session resume checklist (in order)
1. Read `task_plan.md` in full (this file) — phases, status markers, "Next step", and this
   protocol.
...
1. **Model selection.** Default to inheriting the current session's working model (no `model=`
   override) unless the user explicitly names a different model. If the user names a model,
   resolve it with `rlm.find_models(...)` and use the exact returned selector.
2. **Detect a dead/rate-limited child immediately, don't wait it out.** After spawning, check
   ...
   rlm.delete_subagent(handle.name)`) and respawn the same task, either on the inherited default
   model or after the reset window, rather than waiting further or treating its "completed without
   reply" notification as a real result.
3. **Thinking level.** Use `high` for audits/hardening/security work by default; the caller (this
   session) decides and may raise to `max` for a single especially deep pass. Do not leave it at
   the model's default for anything auditing correctness/security.
...
5. **Always tell an audit child what NOT to re-report.** Point it at the current open findings in
   this file's "Phase 5 remediation plan" (or successor) section so parallel audits don't waste a
   turn re-discovering the same known gaps under a different name.
...
### E. Commit checkpoint policy (existing, restated)
...
### F. Local tooling already installed for this project (do not reinstall/reconfigure blindly)
...
- Documentation lives in `docs/` (architecture/roadmap/guide, committed) with an explicit
  ...
  only — keep that note in sync with the remediation plan below; do not let the two drift apart.
