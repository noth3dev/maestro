...
let state;
try { state = JSON.parse(await readFile(statePath, "utf8")); } catch { state = { provider: "fake", lastStep: 0, generation: 1, goalId: randomUUID(), bundleId: randomUUID(), events: [], certifications: [], approvals: [], effects: [], providerCalls: 0 }; }
...
const evidence = async () => {
  ...
  return { bundle: { bundleId: state.bundleId, goalId: state.goalId, content, hash }, certifications: { certifications: state.certifications }, report: { reportId: state.reportId ?? (state.reportId = randomUUID()), goalId: state.goalId, success: state.certifications.some((item) => item.verdict === "passed"), blockers: [], ceoRequest: "Repair the discount calculation", whatChanged: "Disposable target repaired", userVisibleBehaviorPassed: state.certifications.some((item) => item.verdict === "passed"), participatingDepartments: ["engineering"], keyDecisions: state.events.map((item) => item.name), dissent: [], independentValidation: state.certifications.map((item) => item.verdict), costCents: 0, budgetCents: 0, incidents: [], knownLimitations: ["Fake provider; live run is user-owned"], criticalActionAwaitingApproval: state.approvals.some((item) => item.status === "pending"), evidenceBundleId: state.bundleId } };
};
const server = createServer(async (request, response) => {
  try {
    ...
    else if (step === 10) { await provider("repair", target); const result = await provider("test", target); if (result.result.exitCode !== 0) throw new Error("repair did not pass"); const revision = (await provider("revision", target)).result.revision; state.effects.push({ id: "repair:1", revision }); state.certifications.push({ verdict: "passed", revision }); observation = event("quality_certified", { verdict: "passed", integratedRevision: revision }); }
    else if (step === 11) { const effectCount = state.effects.filter((item) => item.id === "repair:1").length; const duplicateWrites = Math.max(0, effectCount - 1); state.generation += 1; observation = event("restart_reconciled", { generation: state.generation, duplicateWrites, effectCount }); }
    else if (step === 12) { await provider("remote", target); const report = JSON.parse(await readFile(`${target}/fixtures/remote-push-attempt.json`, "utf8")); state.approvals.push({ action: "git.remote.push", status: "pending" }); observation = event("ambiguous_action", { escalated: true, remotePush: report.status, networkInvoked: report.networkInvoked }); }
    else if (step === 13) { const modeResults = []; for (const mode of ["full-access-read", "full-access-write"]) { await provider("mode", target, { mode }); await provider("remote", target); const report = JSON.parse(await readFile(`${target}/fixtures/remote-push-attempt.json`, "utf8")); modeResults.push({ mode, status: report.status, networkInvoked: report.networkInvoked }); } observation = event("forbidden_effect", { modeResults }); }
    else if (step === 14) { const result = await evidence(); state.lastEvidence = result; observation = event("evidence_dump", { readOnly: true, bundleReportMatch: result.bundle.bundleId === result.report.evidenceBundleId, providerCalls: state.providerCalls }); }
