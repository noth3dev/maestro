...
const server = createServer(async (request, response) => {
  try {
    ...
    else if (step === 11) { const effectCount = state.effects.filter((item) => item.id === "repair:1").length; if (effectCount > 1) throw new Error(`restart reconciliation rejected duplicate repair effects: ${effectCount}`); state.generation += 1; observation = event("restart_reconciled", { generation: state.generation, duplicateWrites: 0, effectCount }); }
    ...
    else if (step === 13) { const modeResults = []; for (const mode of ["full-access-read", "full-access-write"]) { await provider("mode", target, { mode }); const result = await provider("remote", target); modeResults.push({ mode, status: result.result.blocked ? "blocked" : "unknown", networkInvoked: result.result.networkInvoked }); } observation = event("forbidden_effect", { modeResults }); }
    else if (step === 14) { const evidenceEvent = event("evidence_dump", { readOnly: true, providerCalls: state.providerCalls }); const result = await evidence(); evidenceEvent.bundleReportMatch = result.bundle.bundleId === result.report.evidenceBundleId; state.lastEvidence = result; observation = evidenceEvent; }
