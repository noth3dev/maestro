...
if (kind === "contract") {
  ...
  value = { projectId, substance: { role: "execution", profileRef: "release-scenario", goalBrief: "Inspect the seeded discount defect and report; do not repair before Quality review.", taskDemand, approvedModels: ["test/model-a"], allowedSkills: ["implementation"], allowedTools: ["read", "write"], allowedPaths: [parsed.values.target ?? "TARGET"], environment: ["node"], authorityBoundary: ["target only"], externalServiceBoundary: ["none"], dataBoundary: "target only", costCeiling: "local", timeCeiling: "short", retryCeiling: 0, workerCeiling: 1, deliverable: "seeded-defect test evidence", evidenceRequirements: ["test output"], validationCriteria: ["npm test"], terminationConditions: ["pass or fail"] } };
} else if (kind === "worker") {
