...
if (kind === "contract") {
  ...
  value = { projectId, substance: { role: "execution", profileRef: "release-scenario", goalBrief: "Repair the discount target.", taskDemand, approvedModels: ["test/model-a"], allowedSkills: ["implementation"], allowedTools: ["read", "write"], allowedPaths: [parsed.values.target ?? "TARGET"], environment: ["node"], authorityBoundary: ["target only"], externalServiceBoundary: ["none"], dataBoundary: "target only", costCeiling: "local", timeCeiling: "short", retryCeiling: 0, workerCeiling: 1, deliverable: "passing target test", evidenceRequirements: ["test output"], validationCriteria: ["npm test"], terminationConditions: ["pass or fail"] } };
} else if (kind === "worker") {
  value = { projectId, planVersion: 1, itemId: parsed.values.item ?? "discount-repair", model: "test/model-a" };
} else if (kind === "review") {
